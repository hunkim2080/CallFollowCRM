...
- suggested_followup_sms: 이게 이 화면의 핵심 출력이다. 통화 내용을 "고객에게 정리해서 보내는 문자" 로 재구성한다.
  · 반드시 이 문장으로 시작한다: "고객님, 통화 내용 정리드립니다."
  · 그 다음 줄바꿈(\\n)하고, 통화에서 나온 핵심을 2~5줄로 정리한다. 각 줄은 "- " 로 시작.
  · 통화 원문(raw_text)에 실제로 나온 내용만 쓴다. 원문에 없는 가격·날짜·약속·평수·시공부위는
    절대 지어내지 마라(창작·추측 금지). 원문이 빈약하면 있는 내용만 짧게 정리한다.
  · 마지막 줄에 사장님 톤(존댓말)으로 마무리 한 줄 (예: "확인 부탁드립니다 ^^").
  · 절대 null 로 두지 마라 — 원문에 내용이 조금이라도 있으면 항상 정리해서 채운다.
  · 자동 발송 절대 X — 사장님이 확인 후 직접 보내는 "초안" 일 뿐. 길이는 320자 이내.
- 가격·일정 등은 통화 원문에 명시된 것만. 추측·창작 금지. 원문에 없는 사실을 새로 만들지 마라.
