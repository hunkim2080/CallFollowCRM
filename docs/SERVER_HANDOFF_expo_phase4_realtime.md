...
### G. 고객 웹 흐름·입력 개선 (사장님 2026-07-23, 실사용) — 요청
전부 고객 웹(`/expo/c`) 몫.
- **G1. QR 찍으면 고객정보 먼저**: 고객이 계약서 QR 스캔 → **바로 "고객정보를 입력해주세요"**(성함·연락처·동/호수·타입) 화면 → 다 입력하면 → 상담사와 **계약서(상품·금액)를 같이 보는** 화면으로. (지금은 viewer가 먼저 뜸 → 순서 바꿔달라.)
- **G2. 동/호수 분리 입력 + 숫자패드**: 한 칸에 "714동 128호" 말고 **`[   ]동 [   ]호` 두 칸으로 분리**. **숫자패드(inputmode=numeric)만** 뜨게. (호는 숫자, 동도 숫자 기본 — 필요시 동은 문자 허용 여부 판단.)
- **G3. 계약서 약관 노출 확인**: 사장님이 "계약서에 약관이 안 보인다" 함. **앱 네이티브 계약서는 이제 room terms 노출하게 수정함(android).** 웹 영수증(`/expo/r`)·고객 viewer 에도 room terms 가 실제로 뜨는지 **확인** 요망(추가147에 넣었다지만 실렌더 점검).

### F 보강 (2026-07-23): OCR 이미지 소스 = 사진촬영 / 앨범첨부 **둘 다 선택**
- 사장님: OCR 넣을 때 "사진을 찍을지, 기존 이미지(앨범)를 첨부할지 **선택 가능**해야". 
- 앱: [사진 촬영](카메라) · [앨범에서 선택](PickVisualMedia) 둘 다 제공 → 서버 OCR endpoint로 업로드. (서버 endpoint는 이미지 1장 받으면 소스 무관.)
