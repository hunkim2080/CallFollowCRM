...
### 남은 우선순위 (효과順)
1. **위 0개 가이드 구현** + 0개 경로 실기 검증(녹음 끄고 → 0개 → 가이드 → 켜고 → 다시찾기).
2. 홈 "시작 준비" 체크리스트(SetupCheckCard) 확장 — 건너뛴 항목 재권유(녹음·가격표·템플릿, done 감지자 존재).
3. 가격표·답장 마법사 안 원클릭.
4. 진짜 신규설치 end-to-end 워크(앞 화면 톤·복잡도까지 어르신 관점).
