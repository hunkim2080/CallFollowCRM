...
## 2026-08-14 21:07 · android (완료·S9 검증) — 챗 입력줄 개편 + 이렇게답해보세요 새 띠 + 다음답변AI추천
사장님 프로토-우선 확정 → ChatScreen.kt 구현. 실제 S9+ 눈검증 완료(크래시0·한글깨짐0).
- **입력줄 ⊕ (인스타식)**: 상시 3칩 제거 → ⊕ 왼쪽 풍선 팝업(견적작성/내일정확인/문구넣기). ✨다듬기·보내기(종이비행기)는 글 있을 때만 톡. canSend=글||사진.
- **이렇게답해보세요 = 닫힌 얇은 띠**: "AI 답안 N개 보기 ▲"→탭하면 스르륵 펼침(카드 2개, 화살표 회전). 생성중=스피너 띠. 실패·stale·통화끝남·빈=통째 숨김. **↻재생성 제거·항상 take(2)**. 법적 면책 원문 유지.
- **⊕ "다음 답변 AI 추천"**(4번째): 1개 생성→입력칸 삽입(커서끝+키보드). regenerateSuggestions 재사용.
- 무해/독립: 서버 API 인터페이스 변화 없음(기존 /suggestions·/prepare-reply 그대로). cowork 영향 없음.
- 다음(예정): **통화 상세** — 통화카드 태그화+상세시트(전문 표시)+발신/수신/부재중(코랄). 탭재생은 Whisper 세그먼트(시각) 저장 필요 = **cowork 2단계**. **통화 보정(Sonnet)은 유료버전 백로그 보류**(비용).
- commit: (아래).

## 2026-08-14 21:24 · android (완료·S9 검증) — 통화 상세 1단계 (카드 컴팩트 + 통화 전문 살림)
CallSegment 리팩터. 실제 S9+ 눈검증 완료(크래시0·한글깨짐0).
- 통화 카드 = **컴팩트**(아이콘+발신/수신/부재중·시각 + 태그 + ▲) → **탭하면 인라인 펼침**(요약·전문·재생·후속·수정). 부재중=코랄·펼침없음. "80% 먹던" 카드 해결.
- 태그 = summary 구조화 필드(space·problem·schedule) 상위 3개. 없으면 헤더만.
- **통화 전문** = 숨겨져 있던 transcriptText 살림 → 펼침에 텍스트 블록 + "받아쓰기 대략치" 안내.
- 색 = **딥 티얼(답한통화)·코랄(부재중)**, 메시지 파랑과 구분(Gemini 조언 반영). 프로토 08352d6e.
- 서버 인터페이스 변화 없음(무해).
- **다음(cowork 2단계):** 탭재생 = Whisper 받아쓸 때 **세그먼트(시각+텍스트) 저장**해야 앱이 카톡식 말풍선+문장 탭→그 대목 재생 가능. (에이닷 텍스트 통화는 시각 없어 탭재생 X). **통화 보정(Sonnet)은 유료 백로그 보류.**
- commit: (아래).

## 2026-08-14 21:40 · android → cowork (요청) — Ollama 통화 보정 품질 테스트
사장님 결정: 통화 받아쓰기(STT) 오타·"들리는 대로" 를 정리하는 **보정 단계**를 **유료(Sonnet) 말고 로컬 Ollama(qwen2.5:7b, 이미 맥미니에 있음)로 먼저 테스트**(공짜). 품질 보고 채택 결정.
- 요청: STT transcript → Ollama 보정(오타·띄어쓰기·자연스러운 한국어, **뜻 유지·지어내기 금지**) 프롬프트로 실제 통화 몇 통 돌려 before/after 공유. 봐줄 만하면 채택(공짜) / 부족하면 Gemini Flash(거의 공짜).
- 참고: Ollama는 전에 통화'요약'에 썼다 화자구분 정확도로 제외된 이력 — 근데 보정(오타 고치기)은 더 쉬운 태스크. `call_ollama_json` 이미 있음.
- 맥미니 디스크: `.paperclip` 삭제(~300GB)로 여유 생겼을 것(옛 "97%"는 stale). android서 ssh 지금 타임아웃(꺼짐/네트워크) — 켜지면 df 확인.

## 2026-08-14 22:27 · android → cowork (핸드오프·우선순위 UP) — 통화 전문 "세그먼트(시각+화자)" 저장 = 프로토 카톡식
사장님이 앱 통화 전문 보고 "프로토(카톡 좌우 말풍선)랑 다르다" 놀람. 이유 = 앱은 transcript 를 **통짜 텍스트**로만 가짐(Whisper 세그먼트 버림). 프로토대로 하려면 아래 데이터가 필요 — Ollama 보정 테스트와 **한 묶음**으로 부탁.
- **필요 데이터(계약):** `call_summaries` 에 세그먼트 배열 저장(예: `transcriptSegmentsJson`) = `[{start_ms, speaker("나"/"손님"), text}, ...]`.
  - **시각(start_ms):** faster-whisper `segment.start` 그대로 저장(지금 버리는 중). → 앱이 문장 탭→그 대목부터 재생.
  - **화자(나/손님) = 함정:** faster-whisper 는 **화자 분리 안 됨.** → **Ollama 보정할 때 문맥으로 화자까지 추정**(오타정리 + 화자, 한 번에). 100% 정확 안 해도 됨("대략치" 앱에 이미 안내). **좌우 말풍선엔 이게 있어야 함.**
...
## 2026-08-15 09:10 · cowork
2주 정기 사이트 기능검토(자동) — 최근 릴리즈 기능을 마케팅 페이지에 반영.
...
## 2026-08-15 18:11 · android → cowork ⭐[새 기능] 웹 '글 만들기'(블로그·인스타·스레드+해시태그) + 마이페이지(PC)
사장님 신규 기능. 우리만의 독보적 스킬 = "진짜 상담 원본(문자+통화+메모)으로 마케팅 글 자동 생성". PC 웹 뷰어에 얹음.

**프로토(사장님 확정, 1:1로):**
- 글 만들기 = `claude.ai/code/artifact/406087d0-01c2-49ec-95b9-f3bcb2fdce42`
- 마이페이지 = `claude.ai/code/artifact/4b33650f-e4c4-4e80-8449-d177b6a091a0`

**A. 웹 '글 만들기' (시공 완료 고객 기준)**
- 위치: PC 웹 뷰어, **시공 완료** 현장. 각 현장 카드에 (주소↔시공일 사이) **현장 페르소나 한 줄** + **[✨ 글 만들기]** 버튼.
- 페르소나 한 줄 = 통화요약(부위·문제·예산·일정)+문자 흐름+메모 → "어떤 고민 → 어떻게 시공" 자동 1줄.
- [글 만들기] 누르면 **한 대화로 3플랫폼**을 각 톤에 맞춰 생성(누를 때 생성=미리 X, 비용 절약):
  - 📝 **블로그**(네이버): 1000~1300자 스토리텔링(고민→상담→시공→결과).
  - 📷 **인스타**: ~380자 감성 캡션(줄바꿈·이모지) + **해시태그 15개**. 본문/해시태그 **따로 복사**(해시태그는 첫 댓글용).
  - 🧵 **스레드**: ~230자 대화하듯(톤 확 낮춤) + 해시태그 1~2개.
  - 해시태그 = 지역·부위·공정에서 자동 추출.
- **익명화 필수**: 이름·전화·**동/호수**·정확 주소 제거 → "동탄·34평 아파트" 수준. (다운로드 파일명 동/호수 빼기와 같은 원칙)
- 사진 자동 첨부 제안(이미 웹뷰어에 있는 시공 전·후 사진).

**B. 생성 엔진 = 사장님 본인 Gemini 키 (핵심)**
- PC 웹 글 생성은 **각 owner 본인 Gemini API 키**로 호출 → **AI 비용 사장님 본인 부담**(우리 비용 X). 폰 앱 핵심(추천답변·요약)은 그대로 시공막내가 부담 — 구분.
- 키는 마이페이지에서 붙여넣음 → 서버가 **owner별 암호화 저장** → 글생성 시 그 키로 Gemini 호출. (앱은 관여 X, 전부 웹↔서버)
...
3. **문자 대화**: ⚠️ **아직 서버에 없음 = 결정 필요.** 문자는 폰 Room DB 에만 있음.
   ...
   - **android 제안 계약**: 완료 게이트 별도 엔드포인트 `POST /api/web/customer-content`
     = `{owner_phone, customer_digits, conversation_text}` (필요시 memo 도 여기로 이관 가능).
     앱이 **완료된 고객만** 변경 시 debounce push + 기존 완료 고객 **백필 1회**.
   - **코워크가 더 나은 계약 있으면 제안** → 확정되면 android 가 그 계약대로 conversation push 배선(지금은 memo 만 완료).

**D. 마이페이지 (PC 전용 UI) — 프로토 4b33650f**
- **PC에서만** 열림, 데스크탑 레이아웃(topbar+좌측 사이드바+콘텐츠). 로그인한 사장님용.
- 섹션:
  1. **로그인 & 보안**: 로그인한 **핸드폰번호 전체 표시**(누르면 최근 로그인 이력) · 현재 기기 · 최근 로그인 이력(기기·지역·시간) · **모르는 기기 즉시 로그아웃** · 동적 보안 배지(모르는 기기 있으면 "⚠️ 확인 필요", 없으면 "이 기기 안전"). = QR 인증(방금 90121cd/17:03 막은 것)의 사용자 얼굴.
  2. **AI 키 (내 비용)**: 본인 Gemini 키 붙여넣기 · 마스킹 표시(뒤 4자리) · 이번달 사용량 카드 · **키 받는 법 3단계**(aistudio.google.com/apikey) · 암호화 저장 안내.
  3. **블로그 톤**: 따라할 **"글 주소"(콘텐츠 URL)** 붙여넣기 — ⚠️사장님: **글마다 말투 다를 수 있어 특정 글 URL**(블로그 홈 X). 서버가 그 글 내용을 읽어(fetch) 톤 학습 → 글생성 시 그 스타일로. 배운 톤 badge 표시.
  4. **앱 연결**: 폰 앱과 연결 상태.
...
## 2026-08-15 20:44 · cowork ✅ 웹 QR 로그인 보안 = B 확정 (사장님 결정)
- 사장님 결정 = **B (보안 유지)**. WEB_AUTH_REQUIRE_TOKEN=1 그대로. A(env=0, owner_phone 신뢰 복귀)는 미적용 — 막은 구멍 다시 안 엶.
- 서버 변경 없음(현 상태가 곧 B). 기존 유저는 웹 로그인 첫 시도 시 앱 재인증(SMS 자동읽기 1탭)으로 토큰 발급 → 이후 정상.
- android: 앱 interceptor fix(9cd519f, /api/web/ 401 은 앱세션 안 죽임) 이미 완료 → B대로 진행하면 됨. 추가 조율 불필요.
- 남은 대형작업(별개): 웹 글만들기 마이페이지 — 프로토 HTML(docs/mypage_PROTO.html) 대기 중(사장님이 넣어주면 착수). 그 사이 원하면 백엔드(Gemini 키 암호화 저장) 선행 가능.
## 2026-08-15 21:56 · android → cowork ⭐[사장님 요청·우선순위↑] 웹뷰어에 '글 만들기'+'마이페이지' 깔끔하게 붙이기
오늘 웹 QR 로그인 뚫려서(아래 참고) 사장님이 실제 si0in.kr/web 을 봄 → "블로그·마이페이지 UI 는 왜 없냐, 웹에 깔끔하게 붙여달라" 직접 요청. **우선순위 올려서 붙여주세요.** (android 는 server/ 영역이라 손 못 댐 — 코워크가 붙임)

**어디에 (지금 웹뷰어 레이아웃 기준):**
1. **글 만들기** = 우측 현장 상세 패널. **주소("서울 서대문구…1204호") 바로 아래, 시공일 위**에 →
   - **현장 페르소나 한 줄**(통화요약+문자+메모로 "고민→결정" 자동 1줄) + 그 옆 **[✨ 글 만들기]** 버튼.
   - 누르면 3플랫폼 탭(블로그 길게 / 인스타 짧게+해시태그15 / 스레드 대화체+해시태그2) 생성. 익명화(동/호수·이름·전화 제거). 프로토 그대로 = claude.ai/code/artifact/406087d0-01c2-49ec-95b9-f3bcb2fdce42
2. **마이페이지** = 우측 상단 **"이 PC 로그인됨"(막 아바타) 클릭 → 마이페이지**. PC 전용 레이아웃. 번호 전체+로그인이력·동적 보안배지·본인 Gemini 키 붙여넣기·블로그 톤(콘텐츠 URL). 프로토 = claude.ai/code/artifact/4b33650f-e4c4-4e80-8449-d177b6a091a0
...
## 2026-08-15 23:53 · android → cowork 🎨 마이페이지 ↔ 뷰어 '일관성' (사장님 지적)
사장님: 실제 뷰어(si0in.kr/web)랑 마이페이지 프로토가 헤더·스타일이 따로 논다 → "일관성?".
- android 조치: `docs/mypage_PROTO.html` 헤더를 **뷰어와 일관되게** 수정 — 상단 바 제거 → **펄 라벨 "🖥️ 시공막내 웹 · 마이페이지"**(뷰어 "🖥️ 시공막내 웹 · 시공 사진" 과 동일 톤) + 큰 제목 + 부제 + "← 사진 캘린더로 돌아가기". 카드/팔레트는 이미 동일 토큰.
- ⭐ **근본 요청(코워크):** 마이페이지 구현 시 **뷰어와 같은 셸/헤더를 공유**해주세요 — 펄 라벨·카드 스타일·"이 PC 로그인됨 · 마이페이지 ›" nav 를 한 컴포넌트로. **뷰어가 기준.** 두 페이지가 한 앱처럼 보여야 함(사장님 민감).
- 참고: 뷰어에 글만들기 카드 이미 붙은 것 확인함(빠르네요 👍). 마이페이지도 같은 결로.

## 2026-08-15 23:57 · cowork 마이페이지 헤더 뷰어와 일관 (사장님 '일관성' 지적, 프로토 2022047)
- 변경(_MYPAGE_HTML): 상단 별도 topbar 제거 → 뷰어 톤 펄 라벨 '🖥️ 시공막내 웹 · 마이페이지' + '← 사진 캘린더로 돌아가기' + 부제(pilltop/pill/backlink/subtitle). 뷰어(si0in.kr/web)와 같은 결. topAv/topWho JS 제거.
- 검증: ast OK, 마커 확인.
...
## 2026-08-16 09:34 · android → cowork 🔴 사장님 실사용 피드백 3건 (배포된 마이페이지·글만들기)
사장님이 배포판 직접 써보고 지적. 프로토 반영했으니 코워크 웹뷰어/마이페이지도 맞춰주세요.
1. 🔴 **Gemini 키 저장 안 됨 (제일 급함)** — 마이페이지에서 키 붙여넣고 [연결] 눌러도 '미연결' 그대로, 저장 안 되는 듯. **이거 안 되면 글만들기 아예 못 씀.** 서버 키 저장 엔드포인트 + 저장 후 UI '연결됨' 갱신 디버그 부탁. (사장님 실제 키로 재현됨)
2. **글 만들기 = 플랫폼별 따로 생성으로 개편** — 지금 한 번에 블로그·인스타·스레드 3개 다 뽑는데, 사장님: "안 쓰는 사람도 있으니 각각 눌러서 그것만 열려야". → **프로토 개편**(406087d0): 플랫폼 3개 각 [✨ 만들기] 버튼, 누른 것만 생성(그 플랫폼만 Gemini 호출=비용·시간 절약). 뷰어의 generate-content 를 platform 파라미터 받게(bl/ig/th 개별 생성) 바꿔주세요.
3. **마이페이지 좌측 nav 가 '버튼처럼'** 보임(사장님 지적) → 메뉴답게 개편(프로토 4b33650f): 'MENU' 라벨 + 활성 항목 좌측 강조바 + 로그아웃 앞 구분선. 버튼 박스 느낌 제거.
- 프로토 최신: 글만들기=406087d0(docs/content_studio_PROTO.html) · 마이페이지=4b33650f(docs/mypage_PROTO.html). §0 대로 1:1.

## 2026-08-16 12:40 · android → cowork 글 재료에 '협업 사장 메모' 추가 (사장님 요청)
사장님: 협업 사장(팀)이 현장에 남긴 댓글/메모도 글 재료로 가져와야 함. 단, 한 줄씩 쳐서 문장이 아닐 수(단편·키워드) 있음.
- **가능·앱작업 없음**: 협업 사장 메모 = 이미 서버에 있음(team notes, /api/team/notes · owner_phone+customer_phone). 
- **요청(코워크)**: generate-content 의 _web_gather_materials 에 **team notes 도 4번째 재료로 추가** (통화요약·메모·문자대화 + 협업사장 메모).
- **단편 처리**: 프롬프트에 "협업 현장 메모는 한 줄 단편·키워드일 수 있음 → 맥락으로 이해해 자연스럽게 반영(그대로 나열 금지)" 지시. Gemini 는 단편 잘 소화함.
- 프로토(와이드 v2)의 '이 현장은' 탭에 재료 4종으로 반영: da57e77b. 익명화는 기존대로.

## 2026-08-16 13:17 · cowork 🔴fix 통화 2버그 (사장님 실통화 로그: 3396→589자 요약사고 + 화자 전부'?')
- 실측(stdout): exaone 보정 완료 (3396→589자) = 긴 통화를 요약해 전문 83% 손실 · 화자분리 206발화지만 라벨 전부'?'(4000자 넘어 포기).
- fix1 _ollama_correct_transcript: 보정 결과가 원문의 70% 미만이면 원문 유지(요약사고 방지). → 긴 통화는 원문 그대로(요약은 Gemini/Haiku가 따로).
- fix2 _ollama_label_speakers: 4000자 넘으면 전부'?' 하던 것 → ~3000자 단위 배치로 나눠 라벨링. 긴 통화(발화 많음)도 화자 정상.
- 검증: TestClient — 요약사고→원문유지·정상보정 반영·206발화 배치라벨 '?'0개(2배치). ast OK.
- ⚠️ 배포: bash server/deploy_phase1.sh + 새 통화 재처리(옛 캐시통화는 force_refresh 나 새 통화라야 반영). 탭재생 seekTo 는 앱측 배선 확인 필요(서버 start_ms 는 화자분리 발화에 포함).

## 2026-08-16 13:28 · cowork → android (요청) 통화 '다시 요약(재처리)' 버튼 위치/노출
- 사장님이 실통화(01090361119)로 통화 2버그 발견 → cowork fix(19e7d98: 보정 요약사고 70% 가드 + 화자라벨 긴통화 배치). 배포하면 **새 통화**는 정상.
- 문제: **이미 처리된 그 통화**를 고치려면 앱에서 녹음 재업로드=재요약(force_refresh) 뿐(서버는 오디오 미보관). 사장님이 "다시 요약 어디 있냐" 못 찾음.
- 🔴 요청: 통화 카드/상세에서 **'다시 요약(재처리)' 버튼 위치를 사장님께 안내**하거나, 안 보이면 **눈에 띄게** 해주세요. (cached=true 시 '다시 요약?' 흐름은 있다고 했는데 사장님 못 찾는 중.)
- 참고: 서버 캐시만 고치는 건 앱이 로컬 사본 표시라 무의미 → 재요약(재업로드)만이 그 통화 반영. 탭재생 seekTo 도 이 재처리 후 start_ms 로 확인.

## 2026-08-16 13:52 · android (신규 폴더 `extension/`) 크롬 확장 MVP 착수 — 네이버 블로그 넣기
사장님 "ㄱㄱ" → 글만들기의 마지막 구간(네이버 자동 발행) = 크롬 확장으로. 기획서 docs/EXTENSION_PLAN_PROTO.html.
- **신규 폴더 `extension/`** (app/·server/ 아님. 영역 안 겹침). MV3. commit 7a20044.
- **MVP 범위**: 글(텍스트)만 네이버 스마트에디터에 자동 삽입(content.js). popup 에 붙여넣기→[넣기]. 사진 자동은 2단계.
...
## 2026-08-16 · android(extension/) ✅ 크롬 확장 완성 (0.16.0) — 나란히까지
버튼 하나로 **제목·본문·굵게·인용구·구분선·소제목·사진([n]자리)·나란히(se-imageStrip)·임시저장** 전부 자동. 사장님 실기 전부 검증(사진 나란히 2·3·2장 성공, 7장 보존=SEO안전).
- 나란히=CDP로 사장님 크롬(9222) 붙어 드래그 실측 튜닝→확정(무버 '보이는 윗부분' 잡기, 잡기점 화면밖이 버그였음). SEO 위해 사진 개별유지+나란히 둘다.
...
## 2026-08-16 · android → 🔴 cowork 웹 2건 (사장님 blocked — 확장 불러오기 테스트 막힘)
사장님이 확장 [바로 넣기]/[불러오기] 테스트하려면 웹에서 글을 먼저 만들어야 하는데, 웹에서 막힘:
...
## 2026-08-16 · cowork(server) → 🔴 통화요약 무한반복(계속 요약중) fix
사장님 신고: 통화(…1119) 요약이 계속 돎.
- **원인**: `/api/call-audio-summary`에 **중복 방지(dedup)가 없음**. 요약이 오래 걸리면(STT+exaone보정+화자라벨+gemini) 앱/Cloudflare가 타임아웃 → 같은 통화를 **재시도**. 각 재시도가 새 STT+요약 작업을 만들고 `_whisper_lock` 뒤에 줄줄이 쌓임 → 같은 통화가 끝없이 다시 요약됨.
- **fix**: `(phone,started_at_ms)`별 `asyncio.Lock`(`_call_summary_lock`) 추가. 처리 전체를 락으로 감싸고, **락 안에서 캐시 재확인** → 앞선 요청이 방금 캐시에 넣었으면 즉시 반환. 재시도들은 락에서 대기하다 캐시 HIT로 즉시 끝남 → **딱 1건만 실제 요약**, 중복 큐잉 사라짐.
...
- commit: __C__
## 2026-08-17 · android → 🔴 cowork 웹 개선 3건 (사장님 지적 — 3열 뷰어 배포 후 점검)
3열 뷰어·[연결] 배포 확인됨(materials 404→401, Gemini키 '연결됨'으로 뜸). 사장님이 둘러보며 3건 지적 — 전부 웹/서버(코워크) 영역이라 코드 줄 짚어 넘김. 확장/android 는 손댈 것 없음(확장은 완료건 글만 pull).

1. 🟡 **[테스트] 버튼 문구 = 개발자말, 일반 회원 어색.** 사장님: "테스트가 아니라 정상 연결 체크(내 키 제대로 연결됐나 확인)".
   - 위치: 마이페이지 AI키 카드 `testKey` (main.py:27683) + 결과 토스트 "통과 ✓"/"실패".
   - 추천: 버튼 **[테스트] → "연결 확인"** (같은 카드 [연결]/"연결됨"과 말 맞춤). 토스트 "통과 ✓"→"잘 돼요 ✓", "실패 — 키 확인"→"안 돼요 · 키 다시 확인".

2. 🟡 **마이페이지 '기기 24대 로그인'** = 한 PC에서 정상 연결확인 몇 번 한 게 24개로 쌓임(사장님 "이게 맞냐").
   - 원인: `web_authorize` (main.py:26424) 가 로그인마다 새 `sid` row INSERT, user_agent/기기 dedup 없음. "24대" 문구도 사실 사장님 PC 하나인데 겁줌.
   - 추천: ①같은 브라우저(user_agent+기기쿠키)면 기존 세션 UPDATE(새 row 안 쌓기) ②오래된 세션 자동 정리/캡 ③"24대 로그인" 문구 완화.

3. 🔴 **'시공 완료건만 블로그 글 생성' 규칙이 안 걸려 있음** (사장님 핵심 규칙 — 진행중은 시공후 사진·후기 없어 글 못 씀).
   - 현재: 웹 버튼(main.py:27209)·`genContent`(27215)·서버 `generate-content`(27817) 전부 **완료 여부 안 봄** → 진행중 현장도 글 생성됨.
   - 완료 플래그 `c.completed` 는 웹이 이미 보유(27047·27072) → 게이트 간단.
   - 추천(2중): ①웹—진행중이면 [글 만들기] 버튼 비활성 + "✅ 시공 완료 후 글을 만들 수 있어요" 안내 ②서버—`generate-content` 가 완료 아니면 400 거부("시공 완료건만 글 생성 가능"). 웹 우회 방지.
...
## 2026-08-17 · cowork(server) → ✅ 웹 3건(핸드오프 164bef6) 반영
- 3. 🔴 **시공 완료건만 글 생성**(사장님 우선): ①서버 `generate-content` 가 완료 아니면 **400**("시공 완료 후 글을 만들 수 있어요")—웹 우회 차단. ②웹 뷰어 오른쪽 [글 만들기] 버튼을 진행중이면 **비활성+안내**. (검증: 진행중 400·완료 200)
- 1. 🟡 **[테스트]→"연결 확인"**: 마이페이지 AI키 카드 버튼 문구 + 토스트 "연결 확인 중…"/"잘 돼요 ✓"/"안 돼요 · 키 다시 확인"으로 회원 친화.
- 2. 🟡 **기기 24대 세션 중복**: `web_login_status` 가 로그인마다 새 sid 쌓던 것 → **같은 기기(user_agent)면 기존 세션 UPDATE 재사용** + 30일 초과 세션 자동 정리. (이제 같은 PC 반복 연결확인해도 안 쌓임)
...
## 2026-08-17 · cowork(server) → ✅ 감사 저위험 관찰 4건 정리(#7·#9 수정 / #11·#13 검토·수용)
- ✅ **#7 화자라벨 속도**: `_ollama_label_speakers` 배치를 순차 await → **병렬(동시 3개 상한, Semaphore)** 로. gather 로 순서 보존(탭재생 start_ms 정렬 유지). 긴 통화 라벨링 지연 대폭↓. (검증: 75발화/3배치 → 순서보존·'?' 0)
- ✅ **#9 끝8 조인충돌**: 사이트/재료 조회에 **전체번호 정확일치 우선 + 끝8 suffix 폴백** 추가(`web_site`·`_web_gather_materials`). 끝8이 같은 다른 고객이 있어도 섞이지 않음. (검증: 끝8 동일 2고객 → 각자 정확히 매칭). 참고: 010 휴대폰은 끝8이 곧 식별자라 실사용 충돌은 원래 거의 불가 — 방어적 강화.
- 🟢 **#11 티켓 첫폴러 바인딩 — 검토 후 수용(변경 안 함)**: QR 로그인 표준(티켓 60s TTL·1회용·QR은 로그인 PC 화면에만 표시). 폴러 바인딩 추가의 실익 없음·회귀위험만. 현행 유지.
...
## 2026-08-17 · cowork(server) → ✅ 기기세션 UX 개선(핸드오프 3cca79d)
- **authorize dedup 정렬(스펙대로)**: `web_login_status` 세션발급을 **같은 브라우저(owner+user_agent) 기존 세션 DELETE→새 sid INSERT**(대체) + 30일 미활동 정리로. 옛 sid 폐기라 남은 옛 쿠키도 무효(더 안전). (검증: 같은 브라우저 5회 로그인 → 세션 1행, 안 쌓임)
- **마이페이지 UX(사장님 핵심)**: 로그인&보안 = 평소 **배지 '이 기기 안전'** 고정(‘N대’로 상시 겁주지 않음). 기기 목록(일시·기기·최근활동)은 **[🖥️ 내 활동 기록 보기 →]** 접힘 섹션으로 이동(기본 닫힘, 읽기전용). '모든 기기 로그아웃'도 그 안으로. 네이버 '내 활동 기록' 느낌.
...
## 2026-08-17 · android → 🔴 cowork 사장님 결정/요청 2건 (금액 마스킹 + 24h 세션)
1. **금액 마스킹 (F-1 ❓ → 결정됨)**: 사장님 "금액 넘기지 말기 — 이제 회원들이 쓰니 각자 선택에 맡겨야". → **기본 = 예약금·잔금·개별단가 블로그 미노출**(프롬프트 금지 1줄 + 재료 세척). 이유=멀티회원이라 사장님 개인 정찰제 취향을 전체에 강요 X. (추후 owner별 '금액 공개' 토글 = 2차.) → F-1 계좌·링크·동호는 무조건 마스킹, 여기에 금액도 추가.
...
## 2026-08-17 · android → 🔴 cowork '내 스타일 학습' (블로그 톤 실구현·F-7 완성형) — 정본 확정 + 사장님 핵심
사장님 정본 확정: **아티팩트 https://claude.ai/code/artifact/0b889340-d455-475f-88bb-e2ba17426ffb** ("내 스타일 학습 — 따라할 톤을 AI가 분석"). §0 = 이 프로토 **1:1**. 마이페이지 '블로그 톤' 자리를 이걸로 키움. (claude.ai 로그인 필요 — 못 열면 알려줘, docs/ 에 HTML 떨궈줄게.)

**⭐ 사장님 핵심(관건): "톤(말투)도 톤이지만 그 사람이 글을 어떻게 풀어냈는지 = 글 전개 구조가 관건."** → 프로토 리포트의 **'· 글 흐름'** 이 정확히 그것: `[고객 고민→현장 진단→상담·결정→시공 과정→시공 전/후→정직한 마무리]` 식 **구조를 뽑아 저장**. 말투보다 이 흐름이 중심.

프로토 요지(1:1):
- **UI(마이페이지)**: 플랫폼 탭 블로그/인스타/스레드 **각각 따로** 학습. 블로그=글(포스트) 주소, 인스타=계정주소 or 캡션 붙여넣기, 스레드=글 붙여넣기. [✨분석하기]→로딩 5단계(글카테고리·글자수/어미·**글구조/흐름**·어투/리액션·리포트생성)→**수정가능(contenteditable) 리포트**→[이 스타일로 저장].
- **리포트=저장대상**: ①페르소나 한 줄 ②성격 문단 ③칩(존댓말·정직·이모지절제…) ④**글 구성·어미·어투 박스**: **·글 흐름**(위 구조=관건) ·종결어미 ·어투 ·리액션·이모지. 사장님이 줄단위 수정 후 저장.
- 🔴 **실학습(F-7 fix 핵심)**: tone 등록 시 **본문 실제 크롤/파싱** → owner Gemini 키로 리포트 생성. 생성 시 **저장된 '스타일 요약'을 프롬프트에 주입**(URL 문자열 X — flash 는 URL 못 엶).
- 🔒 **PII/타업체명 안전(사장님 우려+프로토 명시)**: "원문은 학습에만, **저장은 요약된 스타일만**" → 경쟁사 상호·내용 저장·유출 X. 구조·어미·어투만 추상화.
...
## 2026-08-17 · android → ✅ 서버 해결됨(인수 불필요) + 🔴 블로그 사진 flow 3건 핸드오프
✅ **generate 성공 확인** — 사장님 실사용에서 블로그 글 생성됨(통화요약 재료 반영 확인: 구축26평 욕실·아들집·현관추가). 서버 안정화 완료 → **android 서버 인수 불필요, cowork 서버 계속 담당 OK.** (재발 시 위 인수룰 유효.)

🔴 **사진 flow 3건 (사장님 확정) — 전부 웹/서버=cowork, 확장(android)은 최종 배치대로 네이버 넣기 이미 됨:**
1. **익명화=동호수만 제거 (사장님 정정)** — 아파트명·지역은 **남김**("DMC 한양 아파트"·"서대문구" OK), **"N동 N호"·번지만** 삭제. generate 프롬프트 익명화를 "동/호수·번지만"으로(아파트명 마스킹 X). ※방금 생성글에 "1동 1204호"는 없었는지 확인·보강.
2. **사진 선택→생성 연결(핵심)** — 현재 generate 는 업로드순 6장 자동(부위·전후·선택 무시=Fable F-6). → **생성 전 뷰어에서 고른 정보를 generate 가 받게**: 쓸 사진(체크)·순서(1,2,3)·부위(거실화장실·현관, 이미 뷰어에 있음)·전후. 프롬프트에 "1=거실화장실 시공전, 2=거실화장실 시공후, 3=현관…" 주입 → [1][2][3]을 해당 부위 문단·전후 위치에 매칭. naver-draft photos[] 도 이 선택·순서 반영.
3. **블로그 글 편집기 = 프로토 https://claude.ai/code/artifact/e049d59b-1b6e-491c-a456-a17d80edfc90 (드래그·부위매칭·인라인수정·서식)** 1:1. **드래그 보완(사장님 요청):** 프로토는 줄(row) 통째=그룹만 → **개별 드래그 추가**(각 사진 개별 손잡이로 1장씩 이동, 나란히 묶음은 그룹으로 같이; 개별로 빼면 나란히 해제, 나란히 자리에 놓으면 재결합).
...
## 2026-08-17 · android → ✅ 사진 '고르고 번호매기기' UX 프로토 확정(사장님 OK) → cowork 웹뷰어 구현
사장님 확정: 블로그 낼 사진을 **뷰어에서 고르고 순서까지** 정하는 화면(생성 전 단계). 여기서 나온 [고른 사진 + 순서]가 cowork 가 이미 만든 `generate-content` `photos:[{photo_id,part,ba}]`(#2, 위 블록)로 그대로 들어감. 사장님 헷갈림("사진 선택도 안 했는데 번호가 들어갔다"=F-6 자동6장)을 정면 해결.
- **SoT(1:1)**: `design-preview/photo-order-flow.html`(repo·이번커밋) + 아티팩트 https://claude.ai/code/artifact/edea0b04-4d88-4589-8075-749a37157a6c . 마크업·문구·색·애니 그대로(§0).
- **flow 3단계**: ①선택(체크박스 그리드, 부위 노랑칩·시공전 회색/후 티얼=뷰어와 동일, 전체선택/해제) → ②확인 오버레이("사진 N장을 선택하셨어요! 이 사진들로 갈까요?" + 썸네일 부채꼴) → ③번호(**누르는 순서대로 ①②③**, 다시 누르면 취소+뒤 번호 자동 당김, 마우스오버 다음번호 고스트, [순서 초기화], "실리는 순서" 트레이).
- **핵심 계약**: ③에서 매긴 **클릭 순서 = generate 로 보내는 photos[] 순서**(order[0]=①=글의 [1]). 최소 3장, 번호 다 안 매기면 [글 만들기] 막힘. 뒤로 가서 더 골라도 이미 매긴 순서 유지(뺀 것만 빠짐).
- **색 규칙(주의)**: 파랑=고르는 중(뷰어 체크박스색)·보라=순서매기는 중(스튜디오색). 같은 클릭이 단계마다 뜻 달라짐을 색으로 알림.
...
## 2026-09-12 13:45 · android(서버 직접수정)
블로그 글쓰기 규칙에 '전환 사슬' 반영 (창업→영업→자동포스팅→상담AI→계약→관리)
...
## 2026-09-12 14:05 · android(서버 직접수정)
...
## 2026-09-15 · android
통화요약 알림 묶음('✨ 통화요약 N건') + 앱 진입 시 자동 정리
- 변경: 앱 내부만(알림 그룹). 서버 인터페이스 무관.
- commit: (아래 push 참조)

## 2026-09-15 · android
상담함 스크롤 앱 종료 원인 확정 + 수정 (MMS addr 조회 폭주 → LRU 캐시)
- 변경: 앱 내부만. 서버 무관.
- 근거: 갤S9+ logcat 실측 — addr 조회 350회/초, Skipped 355 frames(5.9초 멈춤)
...
## 2026-09-16 10:05 · android
고객 메모가 화면 열기만 해도 빈 글자로 저장되던 버그 수정 (데이터 유실 경로)
- 변경: 앱 UI만. 서버 영향 없음. 단, 서버 백업/캘린더 동기화가 빈 메모를 퍼뜨릴 수 있었음
- commit: 3ca1ff32

## 2026-09-16 11:20 · android
간단 일정(번호 없는 메모형) + 구글식 제목-우선 등록 화면. DB v50→v51 (simple_events 표 추가)
- 변경: 앱 전용. 서버 API 영향 없음. 구글 캘린더에 '📌 제목' 이벤트가 새로 올라감
- commit: 2339ff4a
- 프로토: artifact/FRrHSLRUGDFBQCTms9cjXZ (사장님 승인)

## 2026-09-16 11:50 · android
가입/로그인에 '내 번호 자동 채우기' (구글 전화번호 힌트). 새 권한·새 라이브러리 없음
- 변경: 앱 전용. 서버 API 영향 없음
- commit: cd3f8d4a

## 2026-09-16 12:20 · android
온보딩에서 업종·업체명·활동지역 질문 제거 (문자함 먼저 쓰게). 상단 칩 fallback '줄눈'→'시공'
- 변경: 앱 전용. 서버에 ownerTrade 가 null 로 오는 비율이 늘어남 (서버는 이미 null 허용)
- commit: 33cded6f

## 2026-09-16 13:10 · android
업종 추측(TradeGuesser) + 추천답변 위 '어느 쪽 일 하세요?' 카드
- 변경: 앱 전용. 서버에 ownerTrade 가 채워지는 비율이 다시 올라감
- 실측: 업무폰 1,828통 → 줄눈 884 · 타일 270 · 실리콘 132 (정확)
- commit: 0a8f3abe, 8343b3c5

## 2026-09-16 13:40 · android
신규 고객 · 날짜별에 분류 태그 추가 + 대표번호(114·15xx 등) 제외
- 변경: 앱 전용
- commit: 6709603a

## 2026-09-17 00:10 · android
문구 넣기 시트 개편 — 저장은 ⊕ 메뉴로, 삭제·수정은 꾹 누르기로
- 변경: 앱 전용
- commit: b07e410a · 프로토 artifact/3AjVfigUgxugmXvtdychZE

## 2026-09-17 00:35 · android
'보냈어요' 스낵바 제거(성공은 조용히) + 남은 스낵바가 입력창 안 덮게
- 변경: 앱 전용
- commit: 2b2507a7

## 2026-09-17 00:55 · android
견적 만들기 시트 — 제목·탭 고정, 그 아래만 스크롤
- 변경: 앱 전용
- commit: 736fe107

## 2026-09-17 01:30 · android
...
## 2026-09-17 02:20 · android
✨다듬기 되돌리기 — 입력칸 위 [↩︎ 다듬기 전으로]
- 변경: 앱 전용
- commit: 1ee9454b

## 2026-09-17 03:10 · android
"입금했습니다" 판별기 + 채팅 확인 카드 (잔금 받음 원탭)
...
## 2026-09-17 06:00 · android (server/static 배포 완료)
에이닷 처리방침 대조 → 우리 문서가 실제와 어긋난 곳 3건 수정·배포
- "요약 텍스트만 저장"이 거짓이었음: summary_cache 에 transcript 전문이 남음 → 문구를 실제대로
- 앱만 지우면 서버 데이터 안 지워진다는 경고 추가(앱 시트 + 웹)
- 위치는 서버 전송 없음 명시(GeofenceManager 에 네트워크 호출 0)
- 안 베낀 것: 보유기간 숫자(180일)·1년 미이용 파기 — **파기 코드가 없어서**. 만들고 나서 적을 것
- commit: f7075b25 · 라이브 3개 페이지 200 + 문구 확인

## 2026-09-17 08:45 · android
폰 실기 검증 중 발견한 버그 2개 수정 (계정 삭제 시트 · 달력에서 사라진 시공)
- 변경: DB **v51 → v52** (치유 마이그레이션. 스키마 변경 없음, jobs backfill 재실행)
  - `SettingsScreen.kt` — 계정 삭제 시트를 Scaffold **뒤로** 이동. 앞에 있어서 본문이 덮었고,
    눌러도 아무 일도 안 일어나는 것처럼 보였다. NavHost `composable{}` 안은 Box → 나중 것이 위.
  - `AppDatabase.kt` — MIGRATION_51_52. 접수서로 잡힌 시공일이 customers 에만 있고 jobs 에
    없던 옛 건들을 되살린다. 9/15(0aeef7f6)에 구멍은 막았지만 그 전 데이터는 그대로였음.
    **일정·달력뿐 아니라 D-1 안내·잔금 알림도 jobs 를 돌기 때문에** 문자가 안 나가고 있었다.
- commit: ddf4adf1, a94cc033
- 실기 확인 완료(0.2.1779): 통화 요약 동의 화면 · 문구 넣기 개편 · 다듬기 되돌리기 ·
  간단 일정 등록/삭제 · 고객 정보 하단 여백 · 통화 요약 동의 전 차단(⚠️ 배지)
- 남은 확인: 재방문 건 탭 · 입금 감지 카드 · 업종 물어보기 카드 (해당 데이터 있는 고객 필요)

## 2026-09-17 09:45 · android
🚨 v52 backfill 이 앱을 못 켜게 만든 사고 + 수정 + 실기 검증 완료
- 사고: 0.2.1783 에서 `NOT NULL constraint failed: jobs.memo` → 런치 크래시 루프.
  새로 깐 폰의 jobs.memo 에는 DEFAULT 가 없다(v50 ALTER 를 거친 폰에만 있었다).
  같은 SQL 이 기기마다 다르게 동작 → 기존 폰 테스트로는 못 잡는 종류.
- 수정(4caaa684): NOT NULL 칸 전부 명시(COALESCE(c.memo,'')) + runCatching 으로 감쌈
  (되살리기는 '있으면 좋은' 일, 앱이 켜지는 것보다 우선일 수 없음). 복원 backfill 도 동일.
  배포 전 Room 스키마 그대로 로컬 sqlite 에 올려 5가지 경우 검증.
- 실기 검증(0.2.1784, SM-G965N):
  · 앱 정상 실행 · 달력에 시공 되살아남(9/19 초록, 지난 회색들)
  · [계정 삭제] 시트 정상 표시 → 삭제 페이지(앱 이름·개발자명 노출) 정상 열림
- 사장님: Play Console 데이터 보안에 삭제 URL 등록 + 검토 제출 완료(2026-09-17)
- 남음: 처리방침 배포 대기(계정 삭제 방법 추가 + "앱에서 삭제하면 서버 캐시도 파기" 2곳 정정
  — 실제로 그 기능이 없음을 코드로 확인)

## 2026-09-17 10:30 · android
사장님 실사용 보고 2건 수정 (잘못된 미수 알람 · MMS 본문 안 보임) + 처리방침 배포
- 변경: DB **v52 → v53** (기존 어긋남 1회 보정. 스키마 변경 없음)
  - `CustomerRepository.mutate` — 돈 5칸(총액·계약금·계약금받음·잔금·잔금받음)이 바뀌면
    대표 건(jobs)에 미러링. 돈 수정 경로 8곳이 전부 mutate 를 지나므로 **한 곳에서 전부 커버**.
    🔴 뿌리: Stage A 주석대로 돈은 customers 에만 뒀는데 Stage B 에서 알람만 jobs 로 옮겼다.
       → 고객 카드 '받음' / 건 '안 받음' = 잘못된 미수 알람. (아침 달력 버그와 같은 종류)
  - `AppDatabase` MIGRATION_52_53 — 이미 어긋난 건을 같은 날짜 기준으로 1회 보정. runCatching.
  - `SmsRepository.fillMmsBodies` — MMS placeholder 자리에 진짜 본문. **대화마다가 아니라
    mid IN (...) 한 방**(조회 +1, 대화 수 무관). 사진만이면 placeholder 유지.
    2026-09-15 5.9초 멈춤 사고의 재발 방지가 placeholder 였으므로, 그 제약을 지키는 방식으로.
- commit: 55462664, 7fff9f30
- 서버: `server/static/privacy.html` 배포 완료(백업 privacy.html.bak.20260917_094521)
  · 계정 삭제 방법 2가지(앱 경로 + https://si0in.kr/account/delete) 추가
  · "앱에서 삭제하면 서버 캐시도 함께 파기" **2곳 정정** — 실제로 그 기능이 없음을 코드로 확인
    (서버에 단건 요약 삭제 엔드포인트 없음. 계정 삭제 시에만 _DELETE_BY_CUSTOMER 로 파기)
- 사장님: Play Console 데이터 보안 — 계정 생성 방식 '사용자 이름 및 기타 인증',
  삭제 URL 등록, 부분 삭제 '아니요', 검토 제출까지 완료

## 2026-09-18 01:30 · android
건(件) 단위 전환 착수 — 연결부 전수 점검 + 돈 사고 3건 수정. DB v53 → v54.
- **점검 보고서 신설**: `docs/PLAN_job_centric_migration.md` (fable 작성, 파일:줄 근거 포함)
  · 위험 5개 / 연결부 표(데이터층 D1~D9 + 화면 ①~⑫) / PC 사진 서버 구조 / 6단계 작업 계획 / 함정
- **Step 1 — 돈 이중 합산 제거** (bb65043b): 정산 '이번 달 받은 돈'·현금흐름 달력·마감 브리핑·통계 매출이
  customers + jobs 를 그냥 더하고 있었다(v49 이후 같은 돈이 양쪽에). 계약금 20만 → 40만.
  규칙 = **건이 있는 고객은 jobs 만, 건 없는 고객만 customers**. 먼저 실패하는 시험 3개 작성 후 수정.
- **취소/추천** (01ca78d1): 예약 취소가 다른 건 돈을 지우던 것(cancelJob 으로 그 건만).
  팀 배정도 그 날짜만. 추천답변이 대화방 나가면 취소되던 것 → 앱 수명 스코프.
- **Step 2** (bf6cf3f1): JobRepository 에 건별 쓰기(updateMoney/setWorkCompleted/updateAddress/
  updateMemo/setDepositPaid/setBalancePaid/representativeJobId). 홈 [완료]가 건에도 찍히게.
  **MIGRATION_53_54** = 이미 완료한 옛 건을 전표에 1회 반영(같은 날짜만·이미 찍힌 건 보존·멱등).
- **Step 3-①** (32cd0952): 정산 목록을 건 단위로 — 1차 미수가 안 보이던 것.
- **Step 3-②** (209f9544): 홈 미수 카드를 건 단위로 — 같은 이유로 안 뜨던 것.
- 곁가지: 주소 '동탄산척로2다길' 잘림 수정 + 두 줄 주소 줄바꿈 펴기(+회귀 시험 2건).
...
## 2026-09-18 15:35 · android
🔴 실기 시험 중 발견 — 「＋ 새 시공」으로 2차를 **아예 못 만들었다**
- 증상: ＋ 를 누르면 날짜창이 **지금 건 날짜(10/6)가 이미 찍힌 채** 열린다. 다른 날(10/20)을 누르면
  그게 '끝날'이 되어 기간(10/6~10/20)이 잡히고, 저장하면 시작일이 여전히 10/6 →
  addJob 의 같은날 중복 가드에 걸려 "그 날짜엔 이미 시공이 있어요" 만 뜨고 건이 안 생김.
- 원인: `CustomerDetailScreen` 의 공용 날짜창이 `addingNewJob` 여부와 무관하게
  `customer.scheduledWorkDate` 로 rangeState 를 초기화.
- 고침: 새 건일 땐 **빈 달력**으로 열고(초기값 null), 제목도 "새 시공 — …" 으로.
- 9/17 의 addNewJob 수정은 **쓰기 경로만** 고쳤고 이 입구는 그대로였다.

## 2026-09-18 15:50 · android
🔴 실기 시험 중 발견 2 — 일정 등록으로 **같은 손님이 두 명**이 됨 (원래 있던 버그)
- 재현: 일정 탭 ＋ → 시공 일정 등록 → 기존 고객 번호 입력 → 저장 → 고객 수 225 → **226**.
  같은 번호가 목록에 두 줄, 새 고객엔 주소·지난 건이 없음(돈·미수가 갈림).
- 원인: `ScheduleAddScreen` 이 `phoneField.text`(=하이픈 포함 "010-3404-5247")를 그대로 넘기고
  `upsertByPhone` → `findByPhone` 이 **문자열 정확 비교**. 저장된 번호는 문자 수신 경로의 raw digits.
  ※ 추천 목록을 탭해도 같은 포맷이라 똑같이 갈린다. 건 단위 전환과 무관한 기존 결함.
- 고침: `CustomerDao.findByDigitsSuffix(last8)` 추가(스키마 변경 없음) + `upsertByPhone` 이
  정확 비교 실패 시 **끝 8자리**로 한 번 더 찾는다. 앱의 다른 곳도 끝 8자리를 식별 키로 쓴다.
- ⚠️ 테스트폰에 만들어진 중복 고객(010-3404-5247, 10/20 건)은 시험 끝나고 지울 것.

## 2026-09-18 16:05 · android
실기 시험(S23U·1814·구글 연결됨) — 통과 8 / 버그 2 더 발견해 고침
- ✅ CASE 6-1 업데이트 후 구글 시공 일정 **중복 없음**(달력 2개는 브랜드 2개라 정상)
- ✅ CASE 6-2 **건마다 일정 하나씩** — 10/27, 11/10 각각 생성(Step 5 실기 검증 완료)
- ✅ CASE 2-1 고친 「＋ 새 시공」 정상: 빈 달력 + 제목 "새 시공 —", 탭 2개 생성
- ✅ 일정 카드에 대표 건 금액이 새지 않음(11/10 카드에 100만 안 붙음)
- 🔴 **건 탭 차수·순서가 뒤집힘**: pastJobs 를 먼저, '지금 건'(대표=가장 가까운 날)을 **항상 맨 뒤**로
  그려서 2차를 더 뒤 날짜로 잡으면 [1차 11/10][2차 10/27]. → 날짜순으로 합쳐 그리도록 수정.
- 🔴 **취소한 건의 구글 일정이 안 지워짐**: syncAll 이 `scheduledWorkJobs()`(날짜 있는 건)만 훑어
  날짜가 비워진 건은 영영 안 들른다. → `unscheduledWorkJobsWithEvent()` 추가, 날짜 비운 복사본을
  syncOne 에 넘겨 기존 삭제 경로를 타게 함.
- ❓ CASE 6-3(날짜 바꾸면 그 건만 이동) **미확정** — 폰 달력에 6분 넘게 반영 안 됨.
  구글→폰 동기화 지연인지 앱이 patch 를 안 하는지 구분 못 함. 다음 빌드로 재확인 필요.
- 정리: 시험용 손님 010-0000-0077 = 두 건 예약취소 + '고객 아님'. ⚠️ 구글 일정 2개는 이번 수정이
  깔려야 지워진다. ⚠️ S9+ 에 만든 중복 고객(010-3404-5247, 10/20 건)도 그 폰에서 정리 필요.

## 2026-09-18 16:40 · android
🔁 개발 흐름 전환 — 테스트폰을 **직접 설치(adb)** 로, 플레이는 **통과한 것만**
- 왜: 테스트폰이 플레이 서명이라 고칠 때마다 사장님이 [업데이트]를 눌러야 다음이 진행됐다.
  사장님 지시(2026-09-18): "내가 스스로 업데이트하고 잘됐으면 내부로 올리는거".
- 한 것:
  1) 서버 백업(4.28MB, 01064610131) → **서버 DB에서 실재 확인** + 맥미니에 사본
     (~/ringgo-backups/s9plus_01064610131_20260918.bin)
  2) 플레이판 제거 → 로컬 release APK 설치 → `서버에서 복원` → **고객 226명·예약 4건 복구 확인**
  3) 권한 12개 `pm grant`, 기본 문자앱 역할도 `cmd role add-role-holder --user 0` 로 복구
     → **사장님 손 없이** 전환 완료. 이후 `adb install -r` = 22초.
  4) CI: Play 업로드 스텝에 `if: github.event_name == 'workflow_dispatch'` →
     push 는 si0in.kr APK 만, 플레이는 [Run workflow] 눌러야 올라감.
- 🔑 마스터 로그인(문자 없이): 서버 verify-code 에 **번호+코드 둘 다** 맞아야 통과하는 우회 추가.
  값은 **깃에 없음** — launchd plist 의 `AUTH_MASTER_PHONES` / `AUTH_MASTER_CODE`.
  ⚠️ 정식 출시 전 반드시 제거하거나 코드 교체.
- ⚠️ 테스트폰 versionCode 가 1815(로컬) 로 서버 1816 보다 낮아 '새 버전' 배너가 뜬다. 무시 가능.

## 2026-09-18 16:55 · android
배포는 **수동 실행 전용**으로 (사장님: "사이트에 굳이 apk 올릴 이유가 있나?")
- 확인한 사실: 베타 등록 **13명**, 최근 로그에 사장님 폰 외 번호도 활동
  (01021972496, 01082660840 …). 사이트 APK 를 끊으면 그분들이 옛 버전에 갇힌다 → **안 지운다.**
- 대신 뿌리를 맞췄다: `play-deploy.yml` 에서 **push 트리거 제거** → 이제 플레이도 si0in.kr 도
  [Run workflow] 를 눌러야만 올라간다. 전엔 push 마다 사이트로 검증 안 된 빌드가 나가
  베타 회원 앱에 '새 버전' 배너가 떴다(플레이만 막고 사이트는 뚫려 있었음).
- 흐름: 코드 수정 → `adb install -r` 로 테스트폰 → 경우의 수 통과 → [Run workflow] → 플레이+사이트.

## 2026-09-18 17:05 · android
si0in.kr APK 자동 업로드 **제거** — 이제 배포는 플레이 하나
- 사장님: "사이트에서 apk 받아 쓰던 분들께 플레이에서 받으라고 다 알렸다. apk 는 이제 우리 테스트용."
- `play-deploy.yml` 에서 si0in 업로드 스텝 삭제. 남은 스텝 = 체크아웃·JDK·Gradle·서명복원·AAB빌드·
  versionCode확인·Play업로드·아티팩트. 워크플로는 **수동 실행 전용**(push 트리거 없음).
- ⚠️ 서버의 마지막 APK(1816)와 `/api/download/version` 은 **그대로 뒀다.** 앱 '새 버전' 배너가
  이 값을 보는데, 플레이가 앞서 나가면 배너는 안 뜬다. **배너를 플레이로 돌리는 건 별도 작업**
  (안 하면 언젠가 옛 APK 를 가리키는 배너가 남을 수 있음).

## 2026-09-18 17:25 · android
🔴 실기 시험 3 — 일정 등록이 **다른 건의 금액을 덮었다**
- 재현(테스트폰 직접 설치 루프 첫 실전): 1차 10/20·50만이 있는 손님에게
  일정 탭으로 2차 11/17·**90만** 등록 → **1차가 90만으로 바뀜.**
- 원인: `ScheduleAddViewModel` 이 addJob 뒤에 `customerRepository.updateTotalAmount/...` 로
  고객 카드에 돈을 또 썼고, 그 값이 `mutate → mirrorMoneyToRepresentativeJob` 를 타고
  **대표 건(=오늘 이후 가장 가까운 건 = 1차)** 전표로 미러링됐다. (PLAN §2-⑧ 그대로)
- 고침: 그 3줄 제거 → `JobRepository.syncMoneyFromRepresentative(customerId)` 호출로 교체.
  방향을 **jobs → customers 한 방향**으로 고정. 회귀 테스트
  `JobMoneyPerJobTest '대표 건이 아닌 새 건을 등록해도 대표 건 돈은 그대로다'` 추가.
- 실기 재확인(1818): 3차 12/8·120만 등록 → 손님 카드 **10/20·90만 그대로**, 탭 1·2·3차 날짜순 ✅
- ✅ 같이 확인된 것: 고객 중복 안 생김(226명 유지) · **차수 순서 날짜순으로 고쳐짐**(1차 10/20·2차 11/17·3차 12/8)

## 2026-09-18 18:00 · android
📊 통계를 **건 단위**로 + 💸 제미나이 단가표 교정
- 통계(사장님 지시): `VisitedViewModel`·`StatsViewModel` 이 고객 표만 봐서 한 손님에게
  1·2·3차를 해도 '1곳'이었다 → 건으로 센다(건 없는 옛 고객만 고객 표).
  ⚠️ **전환율은 손님 기준 유지** — 문의 대비 계약 비율이라 건으로 세면 100%를 넘는다.
  실기 확인: 9월 손님에게 두 번째 건 추가 → 현장 7곳 → **8곳**, 다녀올 2→3곳, 전환율 22% 불변.
- 단가표: `gemini-3.5-flash` 가 dict 에 없어 `_resolve_pricing` 이 **Sonnet 단가**로 폴백 →
  다듬기 비용이 실제의 약 2배(15.4원 vs 8.1원/회)로 기록·표시되고 있었다. 실단가 추가
  (input $1.50 / output $9.00 / cached $0.15 per 1M, 출처 ai.google.dev 2026-09-18). 라이브 반영.
- 💰 실측 비용: 평균 in=2885 out=168 → **1회 8.1원, 1000회 약 8,100원.**
  비용의 **74%가 입력**(말투 샘플 50개 등) → 입력을 1500토큰으로 줄이면 1000회 5,200원.
- 정리: 시험용 9/29 건 취소 완료(010-9242-1558 은 9/15·50만 원상복구).

## 2026-09-18 18:20 · android
🎨 6단계(고객 정보 화면 재편) 1·2조각 — 확정 프로토 artifact/4ZvDfUfxDAQU8uNNvQQ1h1
- ① **건 줄은 시공이 둘 이상일 때만.** 1건 손님 화면에서 '1차' 라는 말과 탭 줄이 사라졌다
  (프로토: "100명 중 95명한테 쓸데없는 줄 하나를 얹는 것 = 지저분함의 정체").
  두 번째 시공 입구는 맨 아래 **＋ 시공 하나 더 잡기**(회색 14sp 밑줄, 프로토 `.more-job`).
- ② **메모 두 곳으로 분리.** 👤 이 손님 메모(customers.memo · 머리 카드 바로 아래, 주소보다 위)
  / 📍 이 현장 메모(jobs.memo · 건마다, 2건 이상이면 "N차 현장 메모").
  ⚠️ 건을 바꿀 때 빈 값이 저장되던 사고를 막으려 **shouldSaveMemo dirty 가드를 그대로 재사용**.
  `CustomerDetailViewModel.updateJobMemo(jobId)` 추가. jobs.memo 는 v50 이후 처음으로 화면에 배선됨.
- 헬퍼 `jobNthOf(all, job)` — 차수는 **날짜순**. 탭 줄과 메모/사진 제목의 숫자가 어긋나지 않게.
- 실기 확인(S9+): 1건 손님 → 건 줄 없음 · 순서 = 머리카드 → 👤손님 메모 → 주소 → 탭 ✅
- 남은 조각: ③ 잔금→완료 자동 + 마무리 건 '지난 건' 접기 ④ 고른 건 편집 ⑤ 마무리 뒤 주소 묻기

## 2026-09-18 18:40 · android
🎨 6단계 3조각 — ③-a 완료 / ③-b 는 **대표 건 규칙 결정 필요**
- ③-a ✅ **잔금 받으면 그 건은 자동 완료.** (사장님 확정)
  · `JobRepository.setBalancePaid` — at != null 이면 workCompletedAt 도 채움(이미 있으면 존중,
    되돌릴 땐 완료를 안 건드림 — 시공 완료 되돌리기는 따로 있음)
  · `CustomerRepository.updateBalancePaidAt` — 고객 카드에도 같은 규칙
  · `CustomerDetailViewModel.setBalancePaid` — 대표 **건 전표**에도 반영(미러는 돈만 옮기므로 직접)
  실기: 고객 정보 [전액 확인] → "전액 완납 ✓" ✅
- ③-b ⛔ **막힘.** 프로토는 "마무리된 건은 탭에서 빠져 지난 건으로". 탭 UI·`지난 건 N` 칩·펼침
  목록·`jobClosed()` 는 다 넣었는데, **'지금 건' 탭은 고객 카드 미러라 뺄 수가 없다.**
  빼려면 `recomputeMirror` 의 대표 건 규칙을 "완료(잔금 받음)된 건은 미래여도 대표 아님"으로
  바꿔야 한다 — `docs/PLAN_job_centric_migration.md` §8-2 가 **사장님 결정 필요**로 남겨둔 항목.
  파급: 홈 카드·전화 미리보기·정산·자동분류 등 customers 미러를 읽는 40여 곳이 같이 바뀐다.
  → 사장님 확인 후 진행.

## 2026-09-18 19:00 · android
🧹 같은 메모가 두 곳에 보이던 것 — DB v56 (사장님 결정: "옛 메모는 현장 것으로")
- 원인: v50(9/14)에 건 메모를 만들며 **손님 메모를 건으로 복사**해 뒀는데, 9/18 화면 재편으로
  👤손님 메모 / 📍현장 메모 를 나누자 **같은 글이 양쪽에** 떴다.
- MIGRATION_55_56: `UPDATE customers SET memo='' WHERE memo<>'' AND EXISTS(같은 글을 가진 jobs)`
  · 스키마 변경 없음(UPDATE만) · `runCatching` · **복사본이 실제로 있는 손님만**
  · 건 없는 손님 / 글이 다른 손님은 손대지 않음 · jobs.memo 는 그대로 보존
- 배포 전 로컬 sqlite 4경우 검증(복사본O→비움 / 건없음→유지 / 글다름→유지 / 원래빈칸→유지) ✅
- 실기(S9+): 앱 정상 실행 · 손님 메모 빈칸 · 현장 메모에 글 그대로 ✅
- 대표 건 관련 정정: 사장님 지적("1차 돈 받아야 2차 간다")대로 **현실에선 마무리 건이 과거 날짜**라
  "오늘 이후 가장 가까운 건" 규칙에서 자동으로 대표에서 빠진다. 규칙 변경 불필요 —
  앞서 '막혔다'고 한 건 시험용 비현실 상황(미래 건 선완납)이었음.

## 2026-09-18 19:30 · android
🎨 6단계 ③·④ 완료 — 확정 프로토 artifact/4ZvDfUfxDAQU8uNNvQQ1h1
- ④ **고른 건도 편집 가능.** 읽기 전용 `PastJobPanel` 제거. 정산 카드가 고른 건 값을 보여주고
  (`cShown` = 그 건 값을 채운 고객 복사본) 버튼은 **그 건에 직접** 쓴다(`editJobId`).
  ViewModel 추가: `setJobTotalAmount / setJobDepositAmount / setJobDepositPaid /
  setJobBalancePaid / setJobAddress`. 금액 수정 창도 어느 건인지 기억(`amountEditJobId`).
- ③-b **마무리된 건은 탭에서 빠져 '지난 건'으로.** 실기로 끝까지 확인:
  · 2차(9/10·40만) 탭 선택 → 카드가 그 건 값으로 바뀜 ✅
  · [전액 확인] → 그 건만 "전액 완납 ✓ / 2차 · 완료", 대표(11/17·90만) 그대로 ✅
  · 다른 탭으로 이동 → 2차가 탭에서 빠지고 **[지난 건 1건]** 칩 생성 ✅
  · 칩 탭 → "마무리된 건 · 고르면 열려요 / 2차 · 9/10(목) / 주소 없음 · 400,000원 · 잔금 받음" ✅
- 남은 조각: ⑤ 마무리 뒤 문자에서 주소가 잡히면 "다음 현장 주소인가요?"

## 2026-09-18 19:55 · android
🎨 6단계 ⑤ — 마무리 뒤 문자 주소를 **다음 현장 것으로** 묻기 (확정 프로토 `.catch`)
- 규칙(사장님 9/17): "1차가 잔금 확인되면 마무리. 그 이후 대화에서 주소가 나오면 2차 주소로 캐치 묻기"
  · 주소가 **빈 진행 중 건**이 있으면 → "진행 중인 N차 현장 주소인가요?" → [N차 주소로 등록]
  · 없으면 → "새 현장 주소인가요?" → [새 시공으로 잡기] → **날짜 미정 새 건**이 생기며 주소가 들어감
  · **마무리된 건이 하나라도 있을 때만** 뜬다(그 전엔 기존 주소 제안이 담당)
  · 이미 어느 건이 그 주소를 가지고 있으면 안 묻는다(중복 질문 방지)
  · [아니에요] 는 기존과 같은 기억 장치(dismissAddressSuggest) 사용
- 추가: `JobRepository.addDraftJob(customerId, address)` — 날짜 없이 주소만 든 건.
  `CustomerDetailViewModel.addJobWithAddress(address)`.
- 빌드·테스트 통과, 폰 설치 후 정상 실행 확인. ⚠️ 카드가 **실제로 뜨는 화면**은 문자에서 주소가
  새로 잡혀야 해서 아직 못 봄 — 업무폰 USB 연결 때 7-x 와 같이 확인 필요.

## 2026-09-18 20:20 · android
🚀 배포 트리거 수정 — 사장님 승인 즉시 Claude 가 올릴 수 있게
- 문제: 오후에 push 자동배포를 없애면서 **사장님이 직접 [Run workflow] 를 눌러야만** 올라가게 됐다.
  사장님: "내가 없애자고 했던 건 테스트 안 된 걸 올리지 말자였지. 내부테스트는 내 승인하에
  바로 업로드가 가능해야지." → 승인 절차는 맞지만 **즉시성**이 사라진 게 문제였다.
- 고침: `play-deploy.yml` 에 `push: tags: ["upload-*"]` 추가.
  평소 push 로는 여전히 아무 데도 안 올라가고, **사장님이 "올려" 하면 Claude 가 `upload-*` 태그를
  붙여 배포**한다. PC 에 깃허브 토큰이 없어도 되고(태그 push 는 기존 git 자격으로 됨),
  검증 안 된 빌드가 자동으로 새어 나가지도 않는다.

## 2026-09-18 20:50 · android
✅ CASE 6-4 **실기 통과** — 취소한 건의 구글 일정이 지워진다 (업무폰 1827)
- 절차: 가짜 번호(010-0000-0099)로 12/22 시공 등록 → **구글 웹에 생성 확인**(시공막내 달력,
  오전 9~11시, 고객님 연락처 포함) → 앱에서 예약 취소 → **구글 웹에서 그 줄만 사라짐**(사장님 확인).
- ⚠️ 측정 교훈: **폰 안 달력 DB(content://com.android.calendar)는 캐시라 못 믿는다.**
  이 업무폰은 구글에서 새 일정을 안 당겨와 20분 넘게 56개 그대로였고, 나는 그걸 보고
  "안 올라간다"고 **잘못 결론냈다**. 실제로는 구글 웹엔 정상 생성돼 있었다.
  → 앞으로 캘린더 검증은 **구글 웹**으로. 폰 provider 는 참고만.
- 🔎 그 과정에서 사장님이 찾은 논리 구멍: **취소한 건이 '날짜 미정'으로 탭에 남아 차수를 차지**
  → 날짜도 없는 1차·2차가 줄줄이 생기고 그 위에 ＋새 시공도 가능. 규칙 확정 대기:
  ① 취소한 건은 탭에서 빼기(되살리기 가능하게) ② 날짜 없는 건이 있으면 ＋새 시공 막기.

## 2026-09-18 22:40 · android
☁️ 자동 백업 + 취소한 건 규칙 + 일정 등록 UX 6건 — **테스트폰(S9+) 실기 확인 완료**, 플레이 업로드 대기

### 🚨 사고와 복구 (먼저 읽을 것)
- **업무폰 앱을 실수로 지웠다.** USB로 꽂힌 폰이 테스트폰인 줄 알고(둘 다 SM-G965N)
  uninstall → 앱 안의 자료가 통째로 날아감. 폰 자체·문자·통화기록·사진첩은 무사.
- **복구 성공** — 서버 `app_backups` 에 **당일 16:15 전체 백업**이 있었다(고객 226명·jobs 54·
  통화기록 893·통화요약 548·문자이력 578·녹음첨부 649, prefs·첨부파일 포함). 사장님이 복원.
- 왜 안 덮어썼나: 미러 스냅샷/서버 백업 **둘 다 수동/옵트인**이라 빈 앱이 올릴 수 없었다.
- 🔒 **앞으로 폰 작업 전 반드시 확인**: 일련번호 + `installerPackageName`(플레이=업무폰) + 서명.
  · 업무폰 `1cba6ed4…` / com.android.vending / sig `6cdfa378`
  · 테스트폰 `23514638…` / 사이드로드 / sig `76a8dfb5`  (업무폰 자료를 심어 쓰는 복사폰)

### 변경
- **☁️ 서버 백업 자동화** (ReminderWorker, 20시간 간격). 눌러야만 되던 걸 앱이 알아서.
  · 봉투를 zip 으로 — 실측 4.08MB → **0.65MB(6분의 1)**. 복원은 원래 zip 을 읽어 무변경.
  · **빈 장부는 안 올린다** (막 깐 앱이 멀쩡한 백업을 덮는 것 방지)
  · 🛡️ **같은 번호 복사폰 가드** — 서버 것이 내 것보다 새것이면 자동은 쉰다(3일 조용하면 이어받음).
    서버 코드의 경고와 같은 문제("테스트용 복사폰을 같은 번호로 붙이면서 실제 위험이 됐다").
    손버튼은 그대로 — 누르면 주인이 바뀐다.
- **취소한 건 규칙** (DB **v56→57**, `jobs.cancelledAt`)
  · ① 취소한 건은 앞줄에서 빼고 '지난 건'으로 접음(되살리기 가능) · 목록엔 "예약 취소함"
  · ② 날짜 안 정한 자리가 있으면 ＋새 시공 막고 그 자리로. 1차에 날짜 있으면 2차 미리 잡기는 허용
  · ③ **취소한 건은 차수를 차지하지 않는다** (1차 취소 후 빈 자리가 "2차"로 뜨던 것)
  · MIGRATION_56_57 = ALTER + 날짜·돈·주소가 전부 빈 옛 건만 취소로 표시
  · ⚠️ 마이그레이션은 **v56 실제 DB에 덮어씌워 실기 검증**(테스트폰): 안 죽고 user_version=57
- **일정 등록 화면 UX** (프로토 schedule_form_ux)
  01 번호 칸 위 두 줄 설명 → 칸 아래 한 줄 / 02 저장 버튼 위 "무슨 날로 저장되는지" 한 줄(**A안**)
  03 **"탭" → "누르기"** 앱 전체 25곳 (하단 탭·정산 탭 등 진짜 탭은 그대로)
  05 시간 칩 9개 → `오전 9시 ›` 한 줄 + 창에 자주 쓰는 셋 / 06 토글에 "이 일은"
  07 챗 입력줄 여러 줄일 때 바닥 정렬
- commit: 984d2b79 · 16674577 · 603c207c · bc95b5df · d766d21f · 6a05c45e · 71195370 · 65342068

### 다음 액션
- 플레이 내부테스트 업로드 (`upload-*` 태그) — 사장님 승인 후
- 🆕 **전화 카드에 내 일정** 기획 — 프로토 artifact/63gZ21x4Q48qHFAWg9rPaB.
  신규=가장 빠른 빈 날+일주일 줄 / 기존=요약·문자 위, [📅 내 일정 보기] 버튼.
  ❓사장님 답 대기: 그리는 방법(가/나/다) · "비어 있다" 규칙(두 탕/일요일/A-S)
- ❓ 고객 정보 화면의 **시공 시간 칩 13개**도 일정 등록처럼 한 줄로 접을지 (05번은 일정 등록만 했음)
- 🔎 미해결: 복원 뒤 "이 사람 고객인가요?"를 다시 묻는다는 사장님 보고.
  백업엔 `customer_asked_suffixes` 43개가 들어 있고 restorePrefs 도 호출된다 → 원인 미확인
- 청소 대기: 테스트폰 시험 손님 010-0000-0077, 업무폰 010-0000-0099

## 2026-09-19 00:10 · android
🔎 **9/14 재설치 뒤 나흘간 통화·문자·요약이 하나도 안 쌓이고 있었다** (사장님 신고로 발견) + 사장님 지적 4건 수정

### 발견 — 백업을 뜯어서 확인
- `call_records` / `call_summaries` / `recording_attachments` / `message_histories` 가 **전부 2026-09-14 에서 멈춤.**
  업무폰 `firstInstallTime` = 2026-09-14 21:12 → **재설치 시점부터 캡처가 죽어 있었다.** (내 삭제 사고와 무관)
- 원인 절반 = **코드**: 깔고 한 번 도는 '최근 7일치 통화기록 가져오기' 의 완료 표시
  (`initial_call_log_imported`)가 **백업에 실려 복원**돼, 새로 깐 폰이 "이미 했다"고 건너뛰었다.
  → `skipPrefKey` 에 추가해 복원 제외. 재설치·기변 후 다시 채운다. (8ad31636)
- 원인 절반 = **권한**: 재설치하면 READ_CALL_LOG·에이닷 녹음 폴더(SAF) 접근이 풀린다.
  사장님께 '더보기 > 시작 체크' 재설정 안내함. **눈에 띄게 알리는 건 다음 작업.**

### 5397 금액 120→140 미저장 조사 (사장님 "금액 수정까지 했는데 날아갔네")
- 결론: **백업(9/18 16:15)에도 120만원.** 고객 카드는 9/14 14:41 이후 손댄 흔적 없음
  (금액 수정은 `mutate` 가 `updatedAt` 을 반드시 올리고 `timeline_events("amount")` 도 남기는데 둘 다 없음).
  앱 전체 마지막 금액 변경 이력은 9/9. → **저장 자체가 안 됐다.**
- 재현 시도: 테스트폰에서 120→140 저장 = **정상 동작**(잔금 130만 자동, 이력 창 뜸).
- 가장 유력한 경로: 숫자를 치면 키보드가 올라오며 **[저장] 버튼이 위로 밀린다.**
  원래 자리를 누르면 키보드가 눌리고, 바깥/뒤로 = **입력값이 안내 없이 버려짐.**
  → 값을 바꿔놓고 닫으려 하면 "적어둔 금액을 버릴까요?" 로 되묻게 수정. (4acd5531)

### 사장님 지적 4건 (전부 테스트폰 실기 확인)
- 홈 **'다음 시공' = 주소 우선**, 누르면 그 손님 문자로 ("번호를 봐도 뭐 어쩌라고..?")
- **간단 일정 고치기** 창(제목·날짜·시간·메모) — 전엔 누르면 삭제만 물었다
- **빈 건은 차수를 안 차지** — "1차가 날짜도 금액도 없는데 2차 3차가 있다는 게 말이 안 돼"
  (cancelledAt 없는 옛 껍데기도 같이 접음. 탭 이름 "취소한 건" / "빈 건")
- 금액 입력 창 **조용히 버리지 않기** (위 참조)
- commit: 4acd5531 · 8ad31636

### 다음 액션
- ⚠️ **재설치 후 권한 복구를 눈에 띄게** — 캡처가 죽었는데 사장님이 나흘간 몰랐다. 홈에 경고 배너 필요.
- ❓사장님 답 대기: 전화 카드 일정(가/나/다 · "비어 있다" 규칙) — 프로토 artifact/63gZ21x4Q48qHFAWg9rPaB
- 청소: 테스트폰 010-0000-0077 / JAJAE(10/2), 업무폰 010-0000-0099

## 2026-09-19 13:10 · android
🧭 **"설정 없이 문자하다 되는 앱"** 방향 합의 (프로토) + 오전 지적 4건 수정·업로드

### 방향 (사장님 발의 — "좋은 것 같다", 본인이 더 디벨롭 예정)
- 계기: "바이브코딩 플랫폼들 다 필요한 건 맞는데 읽을 게 많고 배워야 하니 나조차 설치를 망설이더라.
  우리 앱도 그런 느낌 아닐까" → "문자하는데 **별도 설정 없이 클릭만 하면** 다 됐으면"
- 실측: 온보딩 안내 화면 **16개** / 시작 체크 **5가지** / 더보기 **22항목**
- 원칙: **미리 채우게 하지 말고, 처음 쓰는 그 순간에 채워지게 한다.**
- 시나리오 5 + 첫날 흐름 → 프로토 **artifact/FCngD4YnNkkgQtMyqWrkK9**
  ① 통화 녹음 = 통화 끝난 카드에서 묻기(연결 풀리면 다시 나타남 ← 나흘 무지 사고 재발 방지)
  ② 가격표 = **이미 됨**(견적 중 직접 추가 → 가격표 저장). 시작 체크에서 빼기만
  ③ 답장 문구 = 저장 길은 이미 있음 + "같은 말 3번이면 먼저 묻기" 추가
  ④ 사업자 정보 = 첫 견적 때 상호만(내 번호는 폰에서 선채움), 계좌는 잔금 요청 때
  ⑤ 구글 달력 = 첫 일정 잡은 뒤 "내 달력에도 올릴까요?"
  ⑥ 시작 체크 5 → 2 (문자앱·알림만)
- ❓사장님 답 대기: 5→2 승인 / ① 방식 / ③ 기준(3번? 2번?) / ④ 상호만 맞나
- 추천 순서: ① → ⑥ → ②③ → ④ → ⑤

### 오늘 오전~낮 수정 (전부 테스트폰 실기 확인 후 내부테스트 업로드)
- **차수가 하나씩 밀리던 것** — "1차를 고르면 2차 현장메모가 나오네"
  탭과 메모가 차수를 **따로 세고** 있었다(탭은 취소·빈 건 제외, 메모는 포함).
  → 셈을 `jobNthOf` 하나로 모으고 꼬리표(`jobNthPrefix`)도 한 곳에서 만든다.
- **사진 제목에 차수** — "1차 현장 사진 3장 / 20". 사진은 원래 건별로 나뉘어 있었는데 제목이 안 알려줬다.
- **입력줄 정렬 3연타** (모두 어제 바닥정렬 변경의 뒷탈)
  · ⊕ · 보내기가 📷 보다 처짐 → 알약 안쪽 여백(5dp)만큼 같이 띄움
  · "메시지 입력…" 글자가 처짐 → 글칸에 **최소 높이 44dp**(📷와 동일) + 가운데 정렬
    dp 로 밀지 않은 이유: 글자 크기 키운 폰에서도 안 어긋난다
  · 픽셀 측정: 📷 대비 ⊕ 0.5px · 글자 0.5px · 보내기 1.5px (전엔 14 / 22.5 / 19px)
- commit: 4acd5531 · 8ad31636 · cae1275a · (입력줄 글자) · (차수)
- 업로드: upload-20260919-0815 / -1131 / -1229 / -1300 (전부 internal 성공)

### 다음 액션
- ⚠️ **팀원·협업 사장님 사진은 아직 건 구분이 없다** — 서버가 jobId 를 안 들고 있어
  모든 차수 탭에 똑같이 보인다(내 사진은 건별로 나뉨). **코워크: site_photos 에 jobId 싣기.**
- 정식(production) 통로: 지금은 태그=internal 고정. `release-*` 태그로 production 나가게
  열어둘지 **사장님 답 대기** (열어만 두고 올리는 건 지시 있을 때만).
- 업무폰 검증 대기: 앱 켜짐(v57) · 권한 재설정(통화 녹음) · 백업 왕복(zip) · 5397 금액 140만

## 2026-09-19 17:40 · android
🔍 **검색 고도화 01~04 전부 완료** (프로토 artifact/NMcwAqZwJBfZe1Qkcej1kD · 사장님 "제안 준 거 다 마음에 든다")

전제: 우리는 **이미 통화 전문(transcriptText)까지 뒤지고 있었다**(2026-09-02).
  문제는 범위가 아니라 **보여주기** — 결과가 한 손님당 한 줄이라 "안까지 다 봤다"가 안 보였다.

- **01 문장 펼치기 + 색칠** — 걸린 문장 최대 3개(MAX_SENTENCES) 말풍선 + "이 통화에서 N곳 더" + 파란 하이라이트.
  통화 전문은 받아쓰기라 마침표가 거의 없어 문장부호 + "요/죠/다 " 로 쪼갠다. 덩어리면 매칭 둘레만.
- **02 묶음** — 👤손님 → 📞통화 → 💬문자 → 📝메모, 개수 배지 + 접기(화면 떠날 때까지 기억).
- **03 최근 검색** — 최대 8개, **결과를 눌러 들어갈 때만** 저장(오타가 안 쌓이게) + 오늘 통화한 손님 5명.
  저장칸 `AppPreferences.recentSearches` — 구분자는 제어문자 하나(순서 보존. Set 은 순서가 안 지켜짐).
- **04 📍 주소·아파트로 현장 찾기** (사장님 1순위) — 통화·문자보다 **위**.
  한 줄 = 주소(색칠) + 손님 + **시공일 · 돈 상태**(완납/잔금 N만/N만원/완료). 건마다 한 줄, 최근 시공일 순.
  → 에이닷은 '말'만 찾는다. 우리는 '장부'를 갖고 있고 여기가 우리만 되는 자리.

🔴 만들다 발견해 같이 고침 — **두세 자리 숫자를 번호로 찾던 것**
  "35" 를 치니 번호에 35 가 든 손님이 **15명** 올라와 정작 "35만원" 통화를 아래로 밀어냈다.
  → 번호 매칭은 **4자리 이상**일 때만. 두세 자리는 번호가 아니라 금액이다.

실기 확인(테스트폰 S9+): "35" → 📞통화 45 가 맨 위, 문장 3개+색칠.
  주소 속 "475" → 📍현장 1 "…동탄대로24길 199 **475**동 901호 · 9/19(토) 시공 · 35만원"

⚠️ **업로드 한 번 실패**(upload-20260919-1617, "Play 업로드" 단계). 같은 빌드로 재시도하니 성공 →
  플레이 쪽 일시 오류로 보이나 **원인 미확인**(Actions 로그는 저장소 관리자만 열람 가능).
  또 나면 사장님이 그 단계 로그를 봐주셔야 한다.

업로드: upload-20260919-1601 / -1617(실패) / -1624-retry / -1734 (마지막 성공)

### 다음 액션
- 검색 응용 남은 것: **미수 · 기간**으로 찾기 (04와 같은 틀 재사용)
- ❓"설정 없이 문자하다 되는 앱"(artifact/FCngD4YnNkkgQtMyqWrkK9) — 사장님 디벨롭 후 착수
- 업무폰 검증 대기: v57 앱 켜짐 · 권한 재설정(통화 녹음) · 백업 왕복 · 5397 금액 140만

## 2026-09-19 21:30 · android
🪝 **"가르치지 말고 눌러보게 한다"** — 미끼 칩 + 상담함 검색창 (프로토 artifact/Wb1zoMZT9bWxjX2cKoAszu)

계기: 통화 전문 검색을 **9/2 에 넣었는데 사장님이 9/19 에 아셨다.** 기능은 있는데 아무도 모른다.
  사장님: "사용자들이 어떻게 쉽게 알게 할까" → "에이닷은 검색창이 위에 있던데 우리는 자리가 없지?"

- **🪝 미끼 칩** — 검색창 열면 "이렇게도 찾아요". 설명이 아니라 **내 숫자**라야 누른다.
  "미수로 찾기"(안 누름) vs **"💰 미수 8건"**(누름). 0건이면 칩을 안 만든다(빈 약속 금지). 최대 3개.
  ① 💰 미수 N건 ② 📅 N월 시공 N곳(이번 달) ③ 📍 제일 많이 일한 동네 N곳(2곳 이상일 때만)
- **검색창을 상담함 위로** — 돋보기 아이콘을 **창으로 승격**(아이콘 4→3개, 위가 오히려 가벼워짐).
  창 안 글자가 곧 안내문 — 두 문장 4초 교차. 목록 내리면 **접힘**(listState 를 Scaffold 밖으로).
  검색 화면 안 문구도 같은 말로 통일(홈만 바꿔서 두 말이 달랐다).

### 🔴 경우의 수 검산에서 찾은 버그 (사장님 "변수가 있는지 경우의 수 테스트 좀 봐줘")
- **지역 칩이 앱 켤 때마다 바뀌었다.** 사장님 자료에 **4곳짜리 동네가 여섯 군데 동점** →
  `maxByOrNull` 이 HashMap 순서대로 아무거나 집었다. 미끼는 "지금 머릿속에 있는 현장" 이라야 누른다.
  → 동점이면 **최근에 일한 동네**(그마저 같으면 이름 순). 앱 두 번 껐다 켜서 **픽셀 비교 동일** 확인.
  (양천구 → 강서구로 고정)

### 검산해서 문제 없던 것
- 💰 미수 8건 — SettlementCalc 규칙을 그대로 복제해 백업으로 계산 = **8건, 화면과 일치**.
  ※ 내 첫 계산은 37 이었는데 계약금 처리 규칙을 대충 해서 틀린 것이었다. **정산 코드를 그대로 옮겨야 맞다.**
- listState 를 Scaffold 밖으로 올린 것 — 상담함 목록만 쓰고 문자함은 MessageBoxSection 이라 영향 없음.
- 주소 동네 뽑기 — "475동", "102동1603호" 같은 숫자 앞 패턴은 안 걸린다. 오탐 없었음.

### ❓ 아직 못 밝힌 것 / 못 해본 것
- **9월 시공: 백업 7곳 vs 화면 9곳.** 테스트폰엔 내가 만든 시험 자료가 있어 차이가 나는 게 정상이나
  **2건 중 1건만 설명된다.** 업무폰에서 "9월" 을 쳐 실제 건수와 맞는지 확인 필요.
- **10건 넘는 묶음이 접힌 채 시작하는지** · **미수 칩 + 문자 묶음 동시 표시** —
  둘 다 한글 입력이 필요한데 **adb 로 한글을 못 친다.** 사장님이 업무폰에서 쳐보셔야 한다.

업로드: upload-20260919-1928 / -2123 (둘 다 성공)

### 다음 액션
- ❓업무폰 확인: "9월" 건수 · 10건 넘는 묶음 접힘 · 미수 칩
- ⚠️ 업무폰 **권한 재설정(통화 녹음 연결)** 아직 미확인 — 안 켜면 통화가 계속 안 쌓인다
- ❓"설정 없이 문자하다 되는 앱"(artifact/FCngD4YnNkkgQtMyqWrkK9) — 사장님 디벨롭 후 착수

## 2026-09-19 23:10 · android
📞 **전화 카드에 내 일정** — 프로토 확정 (artifact/63gZ21x4Q48qHFAWg9rPaB)

계기: 사장님 "전화 와서 **언제 스케줄되냐** 가장 많이 물어보거든? 내 스케줄이 바로 보였으면 좋겠더라고."

### 확정된 규칙 (사장님 "ㄱㄱ 딱좋아")
- **2주 미니 달력** — 카드를 넓게 잡고 7칸 × 2줄.
- **칸 색은 둘뿐** — 🟩 비었음(시공·A/S·일정 전부 없음) / 그 외는 회색.
  ⚠️ **일 있는 날을 빨갛게 칠하지 않는다.** "받지 마라"는 말이 되기 때문.
- ❌ **"한 자리 남음 / 꽉 참" 은 만들지 않는다.** 사장님:
  *"어떤 사람은 하루에 한 집만, 어떤 사람은 5~6집도 할 수 있을걸? 경우의 수가 다양해."*
  → **앱이 여유를 판단하지 않는다.** 곳수·동네만 보여주고 판단은 회원이.
- ❌ **"같은 동네 가는 날" 표시도 만들지 않는다.** 사장님:
  *"통화상에서 나온 지역이라 체크가 어려울 듯."* → 손님 동네를 앱은 모른다. **모르는 걸 아는 척하면 틀린다.**
- **칸에 적는 글 — 최대 두 줄**, 우선순위 **시공 → A/S → 내 일정**, 넘치면 둘째 줄 뒤에 `외1`
  - 시공: `동탄` / `동탄·수원` / `동탄 외 2` — **구·시·동을 뗀 동네 이름**
  - A/S: **`AS 동탄`** (주황 #FFC24D). `[A/S]동탄` 은 그것만 다섯 칸이라 지역이 안 들어감
  - 내 일정: `장모님댁` (보라 #C4AFFF) — **첫 낱말만**
  - 아무것도 없으면 `비었음`
- **긴 제목 = 첫 낱말만** (사장님 "첫낱말 나도 그거 괜찮은듯")
  칸 하나 47px ÷ 8px 글씨 = **다섯 글자**가 한계.
  "장모님댁 생신 모임"→`장모님댁` · "병원 물리치료"→`병원` · "부가세 신고 마감"→`부가세`
  이유: **"부가세 신…"은 읽다 걸리는데 "부가세"는 안 걸린다.** 정확한 건 눌러서 본다.
  띄어쓰기 없는 제목("어머니모시고병원가기")은 어쩔 수 없이 자른다.
- **칸을 누르면 그날이 카드 안에서 펴진다** — 몇 시·어디·누구. 통화 중이라 **딴 화면으로 안 나간다.**
- **일요일도 똑같이 보여준다** (사장님 "일요일까지 다 보여야함") — 요일로 미리 빼지 않는다.
- **시공막내 간단 일정만 읽는다. 구글 캘린더는 안 읽는다.**
  사장님: *"그래야 시공막내를 더 열심히 사용하지 ㅎ"* (사생활이 통화 카드에 안 뜨는 것도 덤)
- **"빈 날" = 시공도 A/S도 일정도 없는 날.**
  ⚠️ 장모님댁 가는 날을 "비어요"라고 하면 **앱이 거짓말을 한 것.**

### 카드가 세 종류로 나뉜다
| 누구 | 무엇을 | 일정표 |
|---|---|---|
| **처음 거는 사람** | "처음 걸려온 번호예요" 한 줄 | **바로 펼침** |
| 🔁 **또 거는 사람 (시공 전)** | `🔁 3번째 통화` + 지난 통화 + 마지막 문자 | **바로 펼침** |
| **시공했던 사람** | `2번 시공` + **지난 시공 날짜·받은 돈** + 미수 | 버튼으로 접음 |

- 가운데(또 거는 사람)는 **사장님이 짚어준 빠져 있던 종류**:
  *"계약은 안했지만 두번째 세번째 전화하는사람도 있을거란말이지. 이사람은 계약의도가 있는사람이니까 같이 일정표도 보여주면 좋을듯"*
- 시공했던 사람 이름 옆: "기존 손님" → **"2번 시공"**
- 기존 카드 문구 중 **"주고받은 문자도, 지난 통화도 없어요"는 뺀다** — 이미 아는 얘기라 자리만 먹는다.

### 만드는 순서
처음 거는 사람 → 또 거는 사람 → 시공했던 사람

### 열어둔 것
- **A/S 있는 날은 지금 "빈 날"에서 빠져 있다.** A/S가 짧으니 그날 시공도 받을 수 있으면 빈 날에 넣는다 (사장님 확인 대기)

## 2026-09-20 00:05 · android
📅 **전화 카드 2주 달력 — 1단계 완성** (프로토 artifact/63gZ21x4Q48qHFAWg9rPaB)

- 새 파일 `util/RegionName.kt` — 주소 → 동네 이름. **구 → 시·군 → 동·읍·면** 순, 붙는 말을 뗀다.
  도·특별시·광역시는 안 쓴다("서울"은 동네가 아니다). 떼면 한 글자만 남는 곳("중구")은 붙은 채로.
  띄어쓰기 없는 주소는 붙은 글자 속에서 정규식으로 찾는다.
- 새 파일 `util/TwoWeekSchedule.kt` — 순수 계산. 칸 글자·빈 날 판정·첫 낱말 자르기가 전부 여기.
- `service/IncomingCallOverlay.kt` — `loadTwoWeeks()` + `CallerState.schedule` + 카드에 달력 View.
  ⚠️ 이 카드는 **Compose 가 아니라 옛날 View** 다(오버레이 창에서 Compose 가 안 그려진다).
- 신규 = 달력 펼친 채로 / 그 외 = `[📅 내 일정 보기 · 2주 안에 빈 날 N]` 버튼으로 접음.
- 신규 상자에서 "주고받은 문자도, 지난 통화도 없어요" 를 뺐다 — **이미 아는 얘기**라 자리만 먹는다.

### 🔎 실제 자료로 검산 (사장님 주소 62개)
- **61/62 정상** (못 뽑은 1건은 내가 넣은 시험 자료). 띄어쓰기 없는 1건은 보완해서 해결.
- 사장님 자료엔 **"화성시 동탄구"**, "병점구", "효행구" 로 적혀 있어 원하시던 **`동탄` 이 그대로** 나온다.
  ※ 파이썬으로 다시 짜지 않고 **진짜 Kotlin 코드**로 돌렸다 — 미수 37 vs 8 때 다시 짰다가 틀린 적이 있다.
- 단위 테스트 46개: 하루 두 탕 · A/S+시공 같은 날 · 세 개 겹칠 때 · 이틀짜리 ·
  **어제 시작해 오늘 걸치는 시공**(안 잡으면 오늘이 빈 날로 둔갑) · 일요일 · 긴 제목.

### 폰에서 눈으로 잡은 것 2개
- 누른 날 상세가 **네 면 다 둘린 상자**로 나왔다 → `setStroke` 는 네 면을 그린다. 색 막대 View 를 왼쪽에 세웠다.
- 시간 안 적은 시공이 **"하루 종일"** 로 나왔다 → 온종일 묶인 것처럼 보인다. **"시간 미정"** 으로.
  (내 일정은 원래 종일이 많아 "하루 종일" 유지)

### 테스트폰 확인 (23514638…, Play 아님, 9/18 16:20 설치)
`019 동탄`(오늘) · `020~024 비었음` · `025 시공`(주소 없는 건 대체 글자) ·
`028 동대문` · `002 JAJAE`(보라) · **일요일 20·27 도 초록** · 칸 누르면 "10월 2일 (금) / 오전 9시 · JAJAE"

### ⚙️ 테스트폰 설정 변경 (돌려놓을 것 있으면 알려주세요)
- `appops SYSTEM_ALERT_WINDOW allow` — 카드 미리보기를 보려고
- "전화 오는 사람 미리보기" 토글 ON

### 다음
- 2단계 **또 거는 사람** (`🔁 3번째 통화` · 계약 전인데 또 거는 사람은 일정표를 펼친다)
- 3단계 **시공했던 사람** (`2번 시공` · 지난 시공 날짜·받은 돈)
- ❓A/S 있는 날을 "빈 날"에 넣을지 (지금은 뺌)

## 2026-09-20 01:20 · android
🔁🧾 **전화 카드 2·3단계 — 또 거는 사람 / 시공했던 사람** (프로토 artifact/63gZ21x4Q48qHFAWg9rPaB)

### 2단계 — 또 거는 사람 (CallerStatus.REPEAT, 보라 #8B5CF6)
사장님: *"계약은 안했지만 두번째 세번쨰 전화하는사람도 있을거란말이지. … 이사람은 계약의도가
있는사람이니까 같이 일정표도 보여주면 좋을듯"*
- 시공을 한 적도 **잡은 적도 없는데 또 건다** → 칩 `🔁 N번째 통화` (지난 call_records + 1)
- 부제 `아직 시공 전 · 9월 8일부터 문의 중` (첫 통화 기록)
- **일정표를 접지 않고 펼친다** (신규와 같이) — 사려는 사람이라서

### 3단계 — 시공했던 사람
사장님: *"기존고객은 언제 시공했었는지.. 얼마를 받았었는지.."*
- 칩 `기존 손님` → **`N번 시공`** (끝낸 건 수)
- 🧾 `지난 시공` 상자 — 최근 2건, 요약·문자보다 **위**에
- 돈은 **SettlementCalc.rowOf 를 그대로** 쓴다 (규칙을 다시 짰다가 미수 37 로 틀린 적 있음)

### 🔴 실제 자료가 잡아준 것 3개 (내 가정이 틀렸다)
1. **건 메모는 '부위'가 아니다.** 사장님 메모는 견적 내역("모서리 2줄 4만원")이라
   `6월 11일 · 모서리 2줄 4만원 · 54만원 받음` 처럼 **금액이 두 번** 나왔다 → 메모를 뺐다.
   사장님이 원한 건 "언제 · 얼마 받았나" 둘뿐이었다(내가 프로토에 부위를 넣은 것).
2. **날짜가 남아 있다고 '다음 시공'이 아니다.** 끝낸 건도 예약일이 그대로 남아
   `upcoming` 이 true → 칩이 "1번 시공" 으로 안 바뀌었다.
   → `workCompletedAt == null && scheduledWorkDate >= 오늘` 로 정정.
3. **해가 다르면 연도를 말해야 한다.** 작년 11월 건이 `11월 8일부터 문의 중` 으로 나와
   **미래처럼 읽혔다** → `작년 11월 8일` / `2024년 11월 8일`.

### 테스트폰 확인 (23514638…, Play 아님)
- 또 거는 사람: 보라 띠 · `🔁 15번째 통화` · `아직 시공 전 · 작년 11월 8일부터 문의 중` · 일정표 펼침
- 시공했던 사람: `1번 시공` · `지난 시공 / 6월 11일 · 54만원 받음`
- 예약 있는 손님: `🔨 9월 25일 시공 · D-5` (예약이 제일 급한 말이라 칩을 양보하지 않는다)
- 달력: `25 시공`(주소 없는 건) · `28 동대문` · `2 JAJAE`(보라) · 나머지 초록

### 다음
- ❓A/S 있는 날을 "빈 날"에 넣을지 (지금은 뺌)
- ❓테스트폰에 켜둔 것 되돌릴지 — `SYSTEM_ALERT_WINDOW allow` · "전화 오는 사람 미리보기" ON

## 2026-09-20 01:05 · android
🛑 **달력 안을 누르면 카드가 닫히던 것** — 사장님: *"일정을 누르고 다른데 누르니까 꺼져버려서"*

카드 전체에 "누르면 이 손님 대화로" 클릭이 걸려 있는데, **달력 칸 사이 빈 곳**을 누르면
그게 카드 클릭으로 새어나가 대화가 열리고 **카드가 닫혔다**.
달력이 들어오면서 잘못 누를 자리가 훨씬 넓어진 것 — 내가 만든 빈틈이다.

- `schedBox` · `weekHeadRow/weekRow1/weekRow2` · `dayDetailBox` 에 빈 클릭 리스너를 달아
  **달력 안의 탭은 거기서 삼킨다.** 대화로 넘어가는 건 달력 **바깥**(이름·주소·요약)을 눌렀을 때만.
- 테스트폰 검증: 달력 안쪽 빈 곳 3군데 눌러도 카드 유지 ✓ / 칸 누르면 상세 그대로 열림 ✓

### ❓아직 남은 것 — "꺼진 걸 다시 켜는 법"
**일부러** 카드를 눌러 대화로 넘어가면 통화 중에 카드를 되살릴 방법이 없다.
제안: 화면 맨 위에 얇은 띠(`📌 김현수 님 · 통화 중 · 01:12  [일정 보기 ▾]`)를 남기고 누르면 다시 펼침.
→ 프로토 먼저 그리기로 함.

## 2026-09-20 02:40 · android
🔍 **신규 고객 줄 — 어색한 것 고침** (사장님 "uiux전문가 관점에서 어색한것 찾아봐줘" → "응 한번 고쳐봐")
리포트: artifact/1KsaswWcjmfD4sWzZPfoN2

`NewLeadsScreen.NewLeadRow` 세 가지:
- **배지가 버튼인지 상태인지 몰랐다** — 계약완료·답장함·재연락이 **셋 다 같은 알약**인데
  실제로 눌리는 건 재연락 하나뿐. 처음 쓰는 사람은 "답장함"을 누른다.
  → 채운 버튼은 [재연락]만, 나머지는 **배경 없는 글자**로.
- **시각·배지가 둘 다 떠다녀 세로줄이 안 맞았다** — 시각이 이름 줄 안에 있어 분류 태그가 많으면 밀리고,
  배지 폭이 제각각이라 x 가 어긋났다. → **오른쪽 고정 칸**(min 62dp, 시각 위 / 상태·버튼 아래).
- **점이 색으로만 말했다** — 초록·회색·빨강 8dp 점. 햇빛 아래선 안 갈린다.
  앱 다른 데(전화 카드)는 색+칩+글자까지 쓰는데 여기만 색 혼자였다.
  → **안 답한 것만 빨강 점 하나**, 나머지는 자리만 비움(줄 맞춤 유지).

### ⚠️ 내가 틀렸던 것 — 실제 코드 읽고 철회
- **"✨가 모든 줄에 있다"** → 틀렸다. 코드는 **✨AI / 📞통화 / 💬문자**를 갈라 쓰고 있다.
  사장님 자료가 대부분 AI 요약이라 다 ✨로 보였을 뿐. **안 건드렸다.**
- **"시각이 둘째 줄로 밀린다"** → 그건 내 HTML 목업(flex-wrap) 얘기. Row 는 줄바꿈이 없다.
  실제 증상은 **이름이 잘리는 것**이었다. 고친 방향은 그대로 맞다.

### 안 건드린 것 — 물어봐야 함
- **한 줄에 색 넷**(✨파랑 · 시공대기 보라 · 잔금미수 주황 · 계약완료 초록)이 남았다.
  분류 태그는 **공용 `CustomerTags`** 라 채팅 상단·고객 목록까지 같이 바뀐다.
  사장님이 "여기 태그는?" 하고 일부러 맞춘 것(2026-09-16)이라 **혼자 안 바꿨다.**

⚠️ **Play 업로드 안 함** — 사장님: "play에는 업데이트하지말고. apk 로 테스트 폰에만".
테스트폰(23514638…) 설치·확인 완료.

## 2026-09-20 03:10 · android
🔨 **홈 맨 위 — 오늘 시공 카드를 띠 한 줄로** (프로토 artifact/CitTfr44fUSaY6W8avpQrF)

- `TodayBand` + `BandShell` 새로 만들고 `item("today-hero")` 가 이걸 쓴다.
  **옛 `TodayHeroCard` 는 안 지웠다** — 되돌릴 땐 그 호출 한 곳만 바꾸면 된다(테스트폰 실험이라).
- 규칙: 오늘 것 중 **아직 완료 안 된 첫 번째** · 시각 지나면 **"(지났어요)" + [완료]** ·
  두 곳 이상이면 이름 뒤 **(1/N)** · 없는 날은 **색을 안 쓴다**.
- 카드 160dp → **띠 52dp**.

⚠️ **Play 업로드 안 함.** 테스트폰(23514638…)만. 확인: "오늘은 시공이 없어요 / 다음 · 9월 25일 (금) 오전 9시" + [일정 추가]

### ❓칩으로 가기 전에 정해야 할 것
칩이 **목록을 거르는 것**인지 **다른 화면으로 가는 것**인지 정해야 한다.
지금 "오늘 신규"와 "미수"는 **다른 화면에 산다**(신규 고객 화면 / 정산).
- 거르기면: 그 화면들의 내용을 상담함 목록으로 **가져와야** 한다(큰 작업)
- 가기면: 칩을 눌렀는데 **화면이 바뀐다** — 칩 같지 않다
→ 사장님께 물음.

## 2026-09-20 03:35 · android
🏷️ **상담함 탭 두 개 → 칩 한 줄** (사장님 "거르기로 가자" · 프로토 artifact/6qoXfXjd4uxrL4ZNpN3rHU '다' 안)

🔑 **알고 보니 끌어올 게 없었다.** `HomeItem` 에 `isNewToday`·`isUnconfirmed`·`customer` 가 이미 있어서
  `dedupItems` 를 칩으로 한 번 거르면 **기존 '지금 답장 기다려요'/'최근 대화' 렌더가 그대로** 돈다.
  다른 화면(신규 고객·정산)에서 데이터를 가져올 필요가 없었다.

- `InboxChips` + `ChipPill` 새로. `InboxFolderTabs` 는 **안 지웠다**(되돌릴 땐 호출 한 줄).
- 칩: `전체 · 오늘 신규 N · 안 챙긴 N · 미수 N ┃ 새 번호 · 시공 끝남 · 문자함 N`
  가운데 얇은 금 = **왼쪽은 오늘 할 일, 오른쪽은 사람 찾기.** 빨간 숫자는 왼쪽에만.
- **카드 더미가 칩 안으로 들어갔다** — 전체 화면에서 사라짐:
  · 오늘 신규 카드 → [오늘 신규] 칩 · 잔금 카드들 → [미수] 칩 · 오늘 시공 띠 → [전체] 에서만
  · [미수] 칩은 목록을 접는다 — 잔금 카드와 **같은 사람이 두 번** 나오기 때문
- 결과: 카드 넉 장이 밀어내던 자리가 **띠 한 줄 + 칩 한 줄**로. 목록이 화면 위에서 바로 시작.

### 🐞 실기에서 잡은 것
- **안 고른 칩이 안 보였다** — `TossGrayBg` 로 했더니 **화면 배경과 같은 회색**이라 그냥 글자로 보였다.
  → 흰 바탕 + 얇은 테두리. 누를 수 있는 것으로 읽힌다.

⚠️ **Play 업로드 안 함** — 사장님: "apk 로 테스트 폰에만. 한번 내가 실제로 보는게 좋을것같아."

## 2026-09-20 08:40 · android
✨ **AI 추천 답변 — 누르는 자리를 하나로 + 모순 메시지 고침**

### 🐞 "다른 대답을 하는 느낌" (사장님 실기 신고, 업무폰)
한 화면에 **두 말이 동시에** 떴다:
  "AI 답변 준비를 꺼두셨어요 — 더보기 → 설정에서 켤 수 있어요"  /  "✨ 답변 만드는 중…"
원인: `onGenerateReply` 가 `regenerateSuggestions()` **결과를 안 보고 무조건** "만드는 중" 토스트.
  ViewModel 은 꺼져 있으면 바로 돌아서 제 토스트를 띄운다 → 시작도 안 했는데 "만드는 중".
  게다가 `awaitingManualReply=true` 라 **화면은 오지 않을 답을 계속 기다렸다.**
→ `regenerateSuggestions` 가 **시작했는지 Boolean 으로** 돌려준다. 시작했을 때만 말한다.
  ("됐다 하고 결과 확인 없이 넘어가지 마라" 가 토스트에도 그대로 적용된다)

### ❌ ⊕ 메뉴의 "다음 답변 AI 추천" 제거 (사장님 "+ 쪽은 버리자")
AI 추천을 누르는 자리가 **두 군데**라 역할이 안 갈렸다 — 사장님: *"그 둘의 역할이 무엇이 다른지 명확하지않아"*
- 실제 차이: **띠 = 3개를 펼쳐 고르기** / **⊕ = 첫 하나를 입력칸에 바로 꽂기**
- 🔴 그런데 **⊕ 도 속으로는 똑같이 3개를 만들고 2개를 버린다.** 비용은 같은데 쓰는 건 하나.
  자동 추천을 비용 때문에 없앤 마당에 앞뒤가 안 맞는다.
- 띠가 입력창 바로 위라 **손가락에 더 가깝다**(⊕ 는 한 번 더 눌러야 한다).
→ 띠 하나로. `onGenerateReply`·`awaitingManualReply` 통째로 제거.

### 📌 확인된 것
**자동 추천 답변은 이제 안 만드는 게 맞다** — 사장님: *"클릭율이 저조하기 때문에 굳이 비용 들여서
준비해줄 필요 없겠다. 필요한 사람만 클릭해서 사용하게끔."* 업무폰 토글 꺼짐 = **정상.**

⚠️ Play 업로드 대기 — 업무폰에 필요한 수정이라 사장님 말씀 듣고 올린다.

## 2026-09-20 12:10 · android
🏷️💰 **칩 4개 확정 + 미수 규칙 바로잡기**

### 칩(할 일) = `오늘 신규 · 안 챙긴 · 시공 대기 · 미수` (사장님 확정)
- **시공 대기** 새로 — 예약 잡히고 아직 안 끝난 손님. 오른쪽 `시공 끝남` 과 **짝**이 된다.
  🔑 이게 **"예약일은 지났는데 완료 표시가 없는 건"** 을 담는다. 따로 칩 만들 필요가 없어졌다.
- **0 이면 흐리게, 자리는 지킨다.** 사장님: *"생겼다 없어졌다하면 버그인가? 생각할수도있으니까.
  회색으로 안눌리는 버튼처럼"* → 숨기면 옆 칩이 밀려 손이 기억하는 위치가 흔들린다.
  다만 **누르는 건 살려뒀다** — 못 누르면 그것도 고장으로 보인다. 눌러보면 빈 화면이 확인해준다.

### 🔴 빨간 숫자의 뜻을 하나로 — **"내가 손댈 게 몇 개인가"**
사장님: *"빨강 통일이 좋을 것 같아. 빨간 숫자는 아무 연락이 안오면 안뜨는거야.
  시공대기 목록 인원한테 문자가 오면 숫자가 뜨는 것인데... 잡혀있어서 뜨는게 아니라."*
- 전엔 **"몇 명인가"** 를 셌다 → 시공이 열 건 잡히면 **잡혀 있다는 이유로 빨간 10** 이 늘 떠 있었다.
  그건 할 일이 아니라 오히려 **좋은 상태**다.
- → 오늘 신규·시공 대기도 **답 안 한 것만** 센다. 목록은 전부 보여주되 안 챙긴 게 위로 온다.
- 미수만 예외 — 연락이 와서가 아니라 **받을 돈이 남아서** 할 일이라 건수 그대로.

### 💰 미수 = **시공이 끝났는데 못 받은 돈** (SettlementCalc.overdueDays)
사장님: *"시공이 끝났는데 돈을 못받은게 미수여야하지않을까?"*
전엔 완료일이 없으면 **예약일**로 세서 **시공을 안 했어도** 예약일만 지나면 미수가 됐다.
🔎 **업무폰 백업(9/19)으로 검산** — 받을 돈 남은 건이 3개인데 전부 앞으로 할 시공이었다:
    850,000원 예약 9/28 · 800,000원 예약 10/6 · 500,000원 예약 10/20 (셋 다 완료 표시 없음)
  옛 규칙이면 **9/29 부터 850,000원이 "미수"** 로 떠 **아직 안 한 일의 돈을 독촉**하게 된다.
→ `workCompletedAt` 이 있어야 미수. 같이 바뀌는 곳: **홈 미수 카드·칩 · 본폰 미러링 · 리포트**.
  (검산은 Python 재구현이 아니라 **SettlementCalc 를 그대로 쓰는 임시 단위테스트**로. 확인 후 삭제)

### ❓ 남은 것
- **정산 화면**은 규칙이 달라(받을 돈>0 전부) **지금도 시공 전 잔금을 "미수"에 넣는다.** 사장님 판단 대기.
- 흐린 칩을 **진짜 안 눌리게** 할지
⚠️ Play 업로드 안 함 — 테스트폰만.

## 2026-09-20 13:20 · android
📷 **받은 사진이 깨졌으면 다시 받는다** (사장님 실기 신고 — 손님 현장 사진 3장이 전부 지지직)

사장님: *"삼성 메세지 사용할때는 이런적 단한번도없었어. 왜 우리 어플만그래?"*
  맞는 지적이었다. **삼성은 자기 MMS 엔진이 통째로 처리하고 실패하면 다시 받는다.**
  우리는 PDU 를 받아 구글 라이브러리로 뜯고 우리가 저장하는데 **깨졌는지 확인조차 안 했다.**

### 🔎 업무폰에서 실제로 뜯어본 것
- 깨진 것(mid=860, 받은 것): **240×320** · JPEG 머리 FFD8 · 꼬리 FFD9 **정상** · 내용만 노이즈
- 같은 폰의 보낸 것(859·861)은 정상. **받은 것만** 깨졌다.
- 사장님 폰→폰 시험: **1장 정상 · 3장 한 번에도 전부 정상** →
  ⚠️ **내 시험은 재현을 못 했다**(같은 통신사·같은 폰이라 통신사 재압축을 안 거침).
  그걸로 "우리 잘못 아니다" 라고 한 건 성급했다. 사장님 지적이 맞았다.

### 판정 기준 — 실측으로 잡았다
`util/ImageNoiseCheck.kt` — 가운데 200×200 을 **원본 해상도 그대로** 떼서 옆 점 밝기 차이.
  깨진 것 **54~55** / 정상 **3.9~14.1** → 기준 **40**.
- ⚠️ **작게 줄여서 재면 안 된다.** 64×64 로 줄여 쟀더니 깨진 게 9.8, 정상이 12.8 로 **뒤집혔다**
  (노이즈는 점 단위라 줄이면 평균으로 뭉개진다). 이걸 모르고 처음에 잘못된 숫자를 보고했다.
- ⚠️ **가로·세로 중 작은 쪽**을 쓴다 — 줄눈 사진은 **격자무늬**라 한 방향만 튈 수 있다. 단위테스트로 못 박음.

### 흐름 (MmsDownloadedReceiver)
1. 저장 전 이미지 조각을 검사 → 2. 깨졌으면 **PDU 원본을 남긴다**(최근 3개, 다음에 뜯어보려고)
3. **한 번만** 다시 받는다 → 4. 다시 받은 게 멀쩡하면 **옛 문자를 지우고** 새 걸 저장
5. 두 번 다 깨졌으면 **새 걸 버리고 옛 것 유지** + 알림 "📷 사진이 깨져서 왔어요"
- **먼저 저장하고 나서** 다시 받는다 — 다시 받기가 실패해도 문자를 안 잃는다.
- 처음 깨졌을 때도 **알림은 울린다** — 안 그러면 문자가 온 줄도 모른다.
- `EXTRA_RETRY` 가 무한 반복을 막는 유일한 장치.

### ❓아직 모르는 것
**통신사가 준 PDU 가 이미 깨졌는지, 우리가 뜯다 깨뜨렸는지는 아직 모른다.**
남겨두는 PDU(`filesDir/mms_bad/`)가 다음에 그걸 알려준다.

## 2026-09-20 18:50 · android
상담함 리디자인 + 디자인 시스템 1·2단계. 사장님과 폰을 보며 하루 종일.

- **MMS 사진 깨짐**: 받은 사진이 깨졌으면 한 번 다시 받는다(Play 배포 완료).
  폰 위 시험으로 검산 — 실제로 깨져서 온 3장 전부 걸림 / 멀쩡한 2장은 안 건드림.
  ⚠️ 원인 미확정. 깨진 것만 240x320(통신사가 줄이다 망가뜨린 것으로 보임).
- **상담함**: 칩 13가지 고침 → 이름 바꾸기(답장 대기·잔금 대기·종료 고객)
  → [새 번호]·[답장 대기] 제거 → 택배+광고를 [문자함] 하나로 → 내리면 칩 줄이 접힘.
  칩 9개 → 6개. 답장 대기 숫자는 **하단 탭 배지**로 (카톡·문자앱이 쓰는 자리).
- **띠**: 옆으로 넘기면 다음 시공(2쪽). 1쪽 탭 → 일정 화면(전엔 엉뚱한 손님 대화로 갔다).
- **미수 → 잔금 대기**: 알림 띠를 **문자 줄**로 되돌림 — 상담함은 문자함이다.
- **카테고리**: 자동 둘(🔨시공 대기·✅시공 완료) 제거. 칩·상태딱지와 3중으로 겹쳤고
  화면 네 곳에서 숨기고 있었다. ⚠️ 서버 복원으로 되살아나므로 **켤 때마다** 지운다.
- **디자인 시스템**: Claude Design 이 만든 것을 코드에 깔았다(DesignSystem.kt).
  색 30 · 글자 8 · 여백 8 · 모서리 5 · 크기 6. 현황 조사 = 코드 325가지 색(손으로 303).
  1단계(이름 깔기)·2단계(상담함) 완료 — 나머지 화면은 아직.

- 변경: 서버 인터페이스 없음. 앱 UI·테마만.
- commit: 4e62fd27
- 다음 액션 (사장님): 업무폰은 Play 그대로 — 리디자인은 테스트폰에만 있다.
  결정 대기 = 딱지 모양(알약→둥근네모) · 아바타 원 한 색 · 미답장 점 빨강 · 버튼 모양.

## 2026-09-20 21:20 · android
대화창(ChatScreen) 디자인 정리 + 서버 요약 프롬프트에서 이모지 제거
- 앱: 통화 카드 한 줄(118→68dp, 태그는 연한 딱지로 유지), 부재중 [문자하기] 제거,
  이력/발행 카드 이모지→아이콘 칩·흰 카드·값 검정, 상단 ⓘ 선 아이콘, 시트 이모지 정리
- 서버(⚠️ android 가 직접 수정 — 사장님 지시 "그냥 너가 다해"):
  CALL_SUMMARY bullets / CONVERSATION_SUMMARY 에서 **이모지 금지** 로 프롬프트 변경.
  이유 = 📅 가 갤럭시에서 "JUL 17" 이라고 적힌 미국 달력으로 나와 엉뚱한 날짜로 읽힘.
  라이브(~/ringgo-server/main.py) 직접 반영 + 백업 main.py.bak_emoji_* + LaunchAgent reload,
  /health 200 확인. repo server/main.py 에도 같은 내용 반영(이 커밋).
  ⚠️ CARD_SUMMARY(상담함 목록 ✨ 한 줄)는 **그대로 둠** — 거기선 이모지 1개가 훑기에 도움.
- 이미 저장된 옛 요약은 그대로(소급 X). 새로 요약하는 통화부터 적용.
- commit: (아래)

## 2026-09-20 22:05 · android
일정 화면 색 정리 + 상담함 한 줄 요약 이모지 제거(서버 포함)
- 앱: 일정 카드 점 빨강→초록(달력과 동일), 딱지 초록=완료·파랑=D-N·회색=지남(전엔 거꾸로),
  범례 A/S 주황→파랑(요청과 같은 주황이었음)·모양을 달력 그대로(막대/점)·이모지 제거,
  제목/안내문 이모지 제거, [팀원·일당 배정] 알약→둥근네모, "전액 완납 ✓"→"전액 완납"
- 앱: ✨ 글자 → AutoAwesome 아이콘 (갤럭시에서 ✨가 남색 타일로 그려져 목록이 어두워짐)
- 앱: util/SummaryText.stripLeadingEmoji — DB에 남은 옛 요약의 앞 이모지 제거(표시 시점)
- 서버(android 직접): CARD_SUMMARY 프롬프트 "이모지 사용 OK" → **이모지 금지**.
  라이브 반영 + 백업 main.py.bak_cardemoji_* + reload + /health 200.
- commit: (아래)

## 2026-09-21 09:15 · android
✨·📅 앱 전체 아이콘화 + 정산·더보기 정리 + 설정 안쪽 감사
- 공용 부품 presentation/theme/AiMark.kt (AutoAwesome + 간격) 추가
- ✨ 화면 라벨 20여 곳 → 아이콘 (상담함·대화창·통화상세·고객정보·가격표·문자템플릿·
  통화요약·설정·말투학습·온보딩·박람회). 알림·고객 발신 문자는 제외
- 📅 라벨은 글자만, 아이콘 칸은 CalendarMonth. 자동 문자 카드 4개(📞📅📍🤖) 아이콘화
- 값 색: 가격표 금액·알림 소리 값 파랑 → 검정 (색은 상태에만)
- 정산: 목표 소식 한 줄·단위 만원·순이익 한 줄·미수 문구 통일·전월대비 회색
- 더보기: SHOW_TIER_BADGES=false (베타 동안 배지 숨김), 계정 삭제 맨 아래로
- 설정 안쪽 15개 화면 감사 리포트 = artifact/VbWPDq72UY4pgHpK9paSLH
- 남음: MakneTips·KpiCard·채팅 퀵액션·일정 추가 스위치의 이모지 슬롯,
  박람회·협업·통계 화면, 빌드에서 하드코딩 막기(checkHardcodedStyle)
- 전부 테스트폰(S9+)에서 확인. 플레이 미배포.

## 2026-09-21 15:07 · android
내부테스트 업로드 — upload-20260921-1500 (Play internal, 빌드 성공)
- 어제~오늘 디자인 정리 전부 포함: 상담함·대화창·통화상세·고객정보·일정·정산·더보기·통계·고객관리
- ✨·📅 앱 전체 아이콘화 · 탭 다섯 모양 통일(AppTabs) · 죽은 코드(퀵액션·KPI) 제거
- 빌드 감시 checkHardcodedStyle 도입 (tools/style_guard.gradle.kts + style_baseline.txt)
- 서버: 통화/대화/한 줄 요약 프롬프트에서 이모지 제거 (라이브 반영 완료)
- 박람회(screen/expo)는 사장님 지시로 손대지 않음 · 감시에서도 제외
- 다음: 협업 현장 안쪽(데이터 생기면), 남은 손으로 적은 값 2,903곳 줄이기

## 2026-09-21 18:10 · android
안 보던 화면까지 디자인 정리 완료 + 재업로드
- 검색·마감브리핑·새문의·통화요약·사업자정보·거래처·후속문자·수첩·팀관리·로그인·가입·AI문자
- 아이콘 칸 이모지 22곳 → Material 아이콘 (더보기·고객정보·상담함·협업현장)
- 토스트/스낵바 이모지 29곳 제거 (9개 파일)
- 앞서: 배지 원형, QR 세로 고정(PortraitCaptureActivity), 날짜창 버튼 weight, 적는 창 버튼 통일,
  흰 창 tonalElevation=0 22곳
...
## 2026-09-21 22:30 · android (서버 직접 수정)
가입 인증 문자 = 막내가 직접 인사하는 문구로 (사장님이 직접 쓰심)
...
## 2026-09-21 23:55 · android
사장님 실사용 피드백 6건 반영 (홈 띠·상담함 배지·캘린더·필터 뱃지·날짜창)
- 홈 띠 '다음' 에 협업 포함 (내일 협업 가는 날 "다음 · 9/28" 이라 하던 것)
- 하단 빨간 숫자 누르면 '안 챙긴 것만' 거르기 (필터 키 "pending" — "unhandled" 는 옛 칩 값이라 리셋됨)
- 캘린더: 고른 날이 비면 '앞으로의 일정' 5건 (시공+협업)
- 칩별 뱃지: 종료 고객="9/19 끝", 잔금 대기="잔금 45만"
- 날짜 고르는 창이 내비바에 가리던 것 (bottomBarClearance)

## 2026-09-22 00:30 · android
사장님 피드백 추가 반영 — 달력 지역명 · ⊕ 메뉴 색 나눔
...
## 2026-09-22 16:05 · android
그 날 카드 다이어트 + 달력 레퍼런스식 개편 + 현장 사진 카드 개선
- 일정 탭 그 날 카드: 주소가 제목, [길찾기]·[전화] 추가. 번호줄·문자요약·돈 제거, 배정 줄 유지.
- 달력: 날짜 작게 왼쪽 위, 띠 3줄, 연한 바탕+진한 글자, A/S 를 점→띠. 칸 66→62dp.
- 고객 정보 사진 카드: 사진마다 있던 ✕ 제거 → [고르기] 모드에서 여러 장 한 번에.
  6장까지만 보이고 "+N장", [올리기] 를 제목 줄로, 안내문은 0장일 때만.
- 변경: `ScheduleViewModel.cardSummariesByPhoneSuffix` 삭제(사용처 없어짐).
  `NavAppPickerDialog` 홈 private → `presentation/component/NavAppPicker.kt` 공용.
- commit: e8b5a19c, 460ed8d4, (사진카드)
- 내부테스트: upload-20260922-1546 성공 (달력까지 포함, 사진 카드는 미포함)
- ⚠️ 폰 검증 못 함 — 테스트폰 미연결.

## 2026-09-22 17:55 · android
전화 팝업 흰글씨 버그 + 주소 자동 보정 + 배정 시트 개선
- 전화 팝업: bindSchedule 안 `schedFreeTv.setTextColor(0xFFFFFFFF)` 를 못 보고 놔둬서
  흰 바탕에 흰 글씨가 됐다(사장님 신고 → 해결 확인). bind 를 runCatching 으로 감싸고 로그 추가.
  내용 없으면 카드를 아예 안 띄움.
- 주소: `CustomerRepository.repairAddressMirror()` — 앱 켤 때 한 번, 고객 주소와 대표 건 주소가
  어긋나면 고객 쪽으로 맞춤. 앞선 미러 수정은 '앞으로 고치는 것'만이라 이미 어긋난 건 안 고쳐졌다.
- 배정 시트: 이름 셋(카드 [팀원·일당 배정] / 제목 "전문가 배정" / 섹션 "일당사장") → **[같이 할 사장님]** 하나로.
  주소는 회색 칸(없으면 숨김), 설명 한 줄, 빈 상태 한 덩어리, 못 누르는 버튼 숨김, 🤝 제거.
- commit: 8209c03b, ad378b8d, 115f96cc, (배정시트)
- 폰 검증: 테스트폰 SM-G965N 에서 카드 버튼·시트·주소 보정 로그 확인.

## 2026-09-22 19:55 · android
설정 화면 한 바퀴 — 문구 15곳 + 순서 재배치 + 백업 카드
- 문구: 개발자 말 5곳(Mac mini/pip install/endpoint/Tone RAG/임베딩) · 느낌표 4곳 ·
  정중체 3곳 · 조사 앞 공백 · 긴 글 2곳.
  ⚠️ 개발자 말은 **개발자 전용 플래그가 없어** 회원도 보는 상태였다.
- 순서: 성격별 6묶음 → **언제 여는지** 4묶음
  (자주 쓰는 것 / 일이 생기면 / 한 번 해두면 끝 / 막히거나 이상하면). 항목은 그대로, 자리만.
  백업 3번째 → 아래로. 자동 문자·문자 템플릿·가격표 6번째 → 맨 위.
- 백업 카드: 띠+카드가 같은 말 두 번 → 한 덩어리. 청록(#1E6E6A) → 초록 토큰. 큰 버튼 → 작은 버튼 2개.
- 시작 준비 5/5면 **숨김**(전엔 한 줄이 카드 하나를 먹었다).
- 폰 검증: 테스트폰 SM-G965N 에서 직접 확인.

## 2026-09-22 20:25 · android
설정 줄 모양 — 묶음 하나 = 카드 하나
- 전엔 LockRow 가 **줄마다 자기 카드**(그림자+흰 바탕)라 17개가 각자 떠 있었다.
  순서를 묶어도 '나열된 느낌'이 남던 진짜 이유. → SettingsGroup 이 카드 한 장을 그리고
  LockRow 는 그 안에 사는 줄이 됐다. 첫 줄 빼고 위에 구분선(first=true 로 알려준다).
- 아이콘 42→36, 여백 16→13, 모서리 18→16. 글자·색·누르는 느낌은 그대로.
- commit: (줄 모양)
- 폰 검증: 테스트폰 SM-G965N.

## 2026-09-22 21:30 · android
남은 설정 하위화면 디자인 — 이름 통일 + 0원 색 + 협업 빈 화면 + 캡처줄
- 변경: UI만. 화면 제목 "비즈니스 리포트"→"상세 리포트", "시공막내 웹 (PC)"→"(PC 사진)".
...
## 2026-09-23 10:50 · android
대시보드 감사 ⑥⑦⑧⑨ + 지난 비용 재계산 — 배포 완료
- 지난 비용: api_usage.cost_usd 가 전부 Sonnet 단가였다. llm_usage_log(모델별 정확 단가)에서
  같은 endpoint ±3초로 찾아 그 값을 옮김. 6,926행 갱신, 94.9% 매칭, 못 찾은 372행은 그대로.
  누적 175,601원 → 95,776원. 7일 멤버비용 12,215 → 3,477원.
  ⚠️ 토큰으로 재계산하면 안 된다 — api_usage 엔 cache_write 가 없어 오히려 덜 잡힌다(49,330원).
  되돌리기: cache.db.bak-recost-20260923-1022 또는 테이블 api_usage_cost_backup(7,301행).
- ⑥ 기능발견율에 화이트리스트 필터. **접수서는 화면이 없다**(채팅에서 링크 발급) →
  intake_forms 발급 기록으로 센다. 0명 → 3명·18건. 협업현장이 7명(70%)으로 1위.
  진짜 안 쓰는 건 팀원 1명·노트 1명.
- ⑦⑧ 비용 카드에 "회사 전체 AI 비용" 안내 + 평균의 분모 명시.
- ⑨ DASHBOARD_EXCLUDE_PHONES 신설 — 봇·합성번호만. **사장님 본인 폰은 뺀다**
  (STATS_EXCLUDE_PHONES 를 그대로 쓰면 회원 14명이 12명이 된다. 사장님은 진짜 회원).
- 백업: ~/ringgo-server/main.py.bak-20260923-1049

## 2026-09-23 13:50 · android (server/ 직접 — 사장님 승인)
시공접수서 리뉴얼 — 문서 모양 + 동/호수 두 칸 + 마케팅 질문 바로 알약
- 변경: `INTAKE_FORM_HTML_TEMPLATE` (=/intake/{token} 과 /q/{token} 양쪽)
  - ①시공일+②견적 카드 → **문서 한 장**(레터헤드·표·합계·계약금/잔금·비고·서명란).
    앱 견적서 QuoteDocScreen 구조를 그대로 옮김. 도장은 안 넣음(사장님 A안).
  - `_INTAKE_SELECT_COLS` 에 `biz_owner, biz_no, biz_phone` 추가 — **컬럼은 원래 있었고
    _persist_quote_issue_to_db 가 저장까지 하고 있었는데 SELECT 에서만 빠져 있었다.**
    마이그레이션 불필요.
  - `_biz_display_name()` — 상호 없으면 사장님 전화번호, 그것도 없으면 "시공 담당자".
    **fallback "RING-GO 시공" 전부 제거**(없어진 옛 이름 + 개인정보 동의문의 수집·이용 주체로 박혔음).
  - 빈 줄은 통째로 뺀다 — 사업자번호 없는 회원(간이·미등록) 대비.
  - 금액 표기 45만원 → 450,000. `_vat_label` 에서 "(공급가의 10% 별도)" 제거 → "부가세 별도".
    **증빙(세금계산서·현금영수증) 문구는 문서 어디에도 안 넣는다** — 적으면 "증빙 주는 대가로
    돈을 더 받는다"로 읽혀 고객이 캡처해 들고 갈 수 있다. (사장님 지적)
  - 잔금 = **합계 − 계약금** 한 식. (앱 견적서는 부가세 포함 총액에서 빼고 있어 같은 건인데
    115,000원이 달랐다 → 앱 쪽도 맞출 예정)
  - 동/호수: 칸 하나 → `[동][호]` 두 칸(`q-dong`/`q-ho`) + `dongHoText()` 로 "103동 1103호" 고정 저장.
  - 주소: 회색칸 → 파란 [주소 찾기] 버튼, 고르면 초록 + "기사님이 받는 주소" 미리보기.
  - 마케팅: `[지금은 바빠요][네 좋아요!]` 게이트 제거 → 바로 알약 5개(네이버·인스타·**당근**·구글·기타).
    네이버는 **카페를 먼저** 묻는다(전엔 키워드를 먼저 물어 카페 방문자에게 없는 검색어를 물었음).
    뒷질문(카페명·키워드)은 빈칸 [다 됐어요] / [그냥 넘어갈래요] 로 넘어감.
- 검증: 템플릿 4경우(사업자 유/무·상호 유/무) 렌더 OK, 설문 6경로 JS 에러 0, 가로넘침 0(400px).
- ⚠️ **아직 라이브 미배포** — 사장님 배포 승인 대기.
- 다음 액션 (앱): 견적서 잔금식 통일 + 주소 입력 3곳 통일 + 발행 전 업체정보 시트.

## 2026-09-23 14:30 · android
주소 받는 칸 네 군데 통일 + 견적서 합계/잔금을 접수서와 맞춤 + 발행 전 업체정보 한 번 묻기
- 새 파일: `presentation/component/SiteAddressField.kt` — 앱 안 모든 주소 입력의 공용 부품.
  파란 [주소 찾기] → 고르면 초록, `[동][호]` 두 칸, (옵션) "기사님이 받는 주소" 미리보기.
- 새 파일: `presentation/component/BizIdentitySheet.kt` — 상호가 비었을 때 **발행 직전 한 번**만.
  상호만 필수, 대표·사업자번호는 선택("없으면 비워두세요"). [나중에 할래요]로 건너뛰면 번호로 나감.
- `util/AddressUtils.kt` — `splitDongHo` / `joinDongHo` 추가. 못 알아본 글자는 **버리지 않는다**.
  단위 테스트 `DongHoTest` 6개 통과.
- 붙인 곳 3: ScheduleAddScreen / ScheduleScreen 협업 배정 시트 / CustomerDetailScreen.
  ⚠️ **협업 시트는 주소 검색이 아예 없었다** — 손타이핑 오타면 상대 사장님이 길찾기로 못 감.
  여기에 AddressSearchDialog 를 새로 넣었다. 이번 작업에서 제일 큰 수확.
  고객상세의 '도로명 직접 고치기'(2026-06-14 사장님 요청)는 링크 뒤로 **접어서 보존**.
- `QuoteDocScreen` — 공급가/부가세/합계 3줄 → **합계 한 줄 + 작게 "부가세 별도"**.
  잔금 = 합계 − 계약금. 이걸로 접수서와 **같은 숫자**가 된다(전엔 115,000원 차이).
  ⚠️ 표시만 바뀐다 — 정산·미수·매출은 `totalAmount` 를 쓰고 부가세와 무관함을 확인했고,
  발행 스냅샷도 원래 공급가로 저장되고 있었다.
- 빌드 OK (손으로 적은 값 2934→2822). **폰 확인 아직 안 함.**

## 2026-09-23 15:30 · android
더보기에서 사라졌던 [내 업종] 되살림
- 화면 흐름도를 만들며 **들어갈 길이 없는 화면**을 찾다가 발견. 2026-08-31 '더보기 정리' 때
  같이 빠진 것으로 보인다. 온보딩 코드 주석엔 "업종은 **더보기에서 설정**"이라 적혀 있는데
  그 줄이 없어서, **한 번 놓치면 다시 고칠 방법이 없었다.**
  그동안 AI 답변이 계속 '일반 시공 사장님' 톤으로 나갔다. (TradeGuesser 가 이 목록을 참조)
- 위치: 더보기 › 한 번 해두면 끝 › [내 업종] (내 말투 학습 바로 위 — 둘 다 AI 답변을 정한다)
- 폰(SM-G965N)에서 눈으로 확인.
- ⚠️ 같이 조사한 결과 **나머지는 지우면 안 된다**:
  · 팀 관리 — `FeatureFlags.SHOW_TEAM_MEMBERS=false` (2026-07-18 사장님이 끈 것, true 면 부활)
...
## 2026-09-23 17:00 · android
화면 점검(71장) 후속 수정 — ④⑤⑥
- ④ 통계 문의 추이: 양쪽 다 0이면 **빈 140dp 그래프**가 화면 한가운데를 먹었다 → 한 줄로 접음.
  ('시장 비교'를 2026-09-21 에 접어 둔 것과 같은 방식)
- ⑤ 채팅 링크: `https://api.si0in.kr/mirror/...` 가 **세 줄**로 말풍선을 먹었다 → 보이는 글자만 줄임
  (`https://` 제거, 36자 넘으면 가운데 생략). **복사(msg.body)·탭해서 열기(firstUrlIn(body))는
  원문을 쓰므로 안 깨진다** — 확인하고 넣었다. 폰에서 세 줄 → 한 줄 확인.
- ⑥ 접수서: 항목을 안 고르면 "계약금 0원 (합계의 30%)" 가 떠서 뭘 잘못한 줄 알았다
  → "항목을 고르면 계약금이 계산돼요".
- ❌ ③ 가격표·문자 템플릿은 **손대지 않았다** — 2026-09-22 에 사장님이 이미 정리하신 상태였다
  (가격표=묶음 카드로 "일곱 줄→열두 줄", 템플릿=한 줄씩 접고 꺼진 건 흐리게). 내 점검이 틀렸다.
- ⏸ ⑦ 이름 대신 번호(상담함) · ⑧ 말투 무한 로딩 — 사장님 지시로 **보류**(자세히 본 뒤 다음에).

## 2026-09-23 17:40 · android
🔴 접수서 한 건이 조용히 사라지던 버그 — 원인 확정 + 수정 (사장님 신고: 4027)
- **서버엔 멀쩡히 있었다.** token DjfcGMNv · 09-16 09:24 제출 · 시공일 09-29 · 80만원.
- 앱 intake_events 32건 중 **이 건만 없음**. 앞뒤(09-13, 09-22)는 정상.
- 다른 회원 계정 3개는 **빠진 것 0건** — 이 폰에서만 일어났다.
- 경위 (서버 백업 이력 app_backups_history 로 확인):
  09-16 09:24 제출 → **09-18 업무폰 앱 삭제 사고** → [서버에서 복원](백업은 09-17 이전이라 이 건 없음)
  → 복원 뒤 첫 폴링에서 `since==0` → **기존 제출을 전부 '이미 본 것'으로 찍는 기준선 로직**이
  이 건까지 삼켰다 → 앱에도 없고 복원본에도 없는 상태가 됨.
- 옛 주석의 가정이 틀렸다: "옛 접수서는 [서버에서 복원]으로 들어온다" —
  복원은 **하루 한 번 뜨는 백업**이라 백업 이후 제출은 복원에도 없다.
- 수정: 첫 실행을 두 경우로 가른다.
  · 앱에 접수서 기록이 0건 = 진짜 새 설치 → 예전처럼 기준선만(2026-09-14 '쏟아짐' 방지 유지)
...
## 2026-09-23 18:25 · android + server
🔴 **내가 오늘 낸 버그** — 접수서 폴링이 약 2시간 깨져 있었다 (16:30 배포 ~ 18:20 수정)
- 접수서 리뉴얼 때 `_INTAKE_SELECT_COLS` 를 20 → 23칸으로 늘렸는데,
  **그 SELECT 를 잘라 쓰는 두 곳**(`/api/quote/submissions`, `/q/{token}/doc`)이 20 그대로였다.
  `ValueError: not enough values to unpack (expected 23, got 20)` → 500.
- 하필 `/api/quote/submissions` 는 **앱이 새 접수서를 받아오는 바로 그 API** 라,
  그 동안 고객이 접수서를 내도 어느 앱에도 안 들어갔다. 오늘 사장님이 겪은 것과 같은 증상.
- 수정: 컬럼 수를 `INTAKE_BASE_COLS = 23` 상수로 묶고 세 곳이 다 그걸 쓰게. 숫자를 손으로 안 적는다.
- 검증: 세 계정 폴링 200, /intake 200, /q/doc 200.

함께 배포 — `/api/quote/submissions` 의 owner_phone 비교를 **숫자만**으로
- owner_phone 이 두 형식으로 쌓여 있다: '010-6461-0131'(~08-13) / '01064610131'(07-27~).
  정확히 일치로만 찾아서 **하이픈 시절 접수서 28건이 앱에서 안 보였다.**
- 이제 44건(28+16) 전부 잡힌다. 제출 33건, 사라졌던 DjfcGMNv 포함 확인.

앱: [접수서 다시 가져오기] 버튼 — 더보기 › 막히거나 이상하면
- 서버는 접수서를 하나도 안 지운다(2026-06-01~ 79건 보관). 앱에만 없을 때 눌러서 되찾는다.
- 앱에 없는 token 만 다시 태운다. 주소·시공일·금액은 기존 가드대로 **적어둔 값을 안 덮는다.**
- 결과를 숫자로: "3건 가져왔어요" / "빠진 접수서가 없어요".

## 2026-09-23 19:05 · server
재발 방지 2종 — "이런 사고 절대 안 돼" (사장님)
- **서버가 500 을 내면 스스로 슬랙에 신고**한다 (`_alert_500`, `_record_system_error` 에 연결).
  오늘 폴링이 2시간 깨져 있었는데 알게 된 경로가 **사장님 전화**였다. 버그는 또 난다 —
  2시간이냐 2분이냐는 누가 먼저 아느냐로 갈린다.
  · 같은 길 10분에 1번 / 전체 시간당 12번 상한(알림 폭탄 방지), 백그라운드 전송(요청 안 죽임).
  · 웹훅 주소는 **코드에 없다** — 서버의 `~/.ringgo_alert_webhook` 또는 `ALERT_WEBHOOK`. 없으면 꺼짐.
- **`server/smoke.sh`** — 배포 후 손님이 쓰는 길 9곳을 실제로 눌러본다. 하나라도 깨지면 exit 1.
  오늘 내 배포 검증은 `/health` 200 만 봤는데, /health 는 무슨 일이 나도 200 이다.
  · 200 만 보지 않고 **내용까지** 본다(예: 접수서 폴링 응답에 `"items"` 가 있는지).
- 🔜 라이브 반영 대기: 알림 코드 배포 + `~/.ringgo_alert_webhook` 설치 (사장님 승인 필요).

## 2026-09-23 19:05 · android (조사만, 코드 변경 없음)
10/20 유령 일정 원인 확정 — **같은 손님이 둘로 갈라져 있었다**
- 고객 208 `01034045247` = 진짜(접수서 EhF9hFJT, 10/6, 80만, 주소·구글캘린더 있음)
- 고객 226 `010-3404-5247` = 09-18 15:27 생성, 10/20, 50만, 주소·메모·캘린더 전부 없음
- 원인 = 하이픈 때문에 같은 번호를 못 알아보던 버그. 고친 커밋이 `b53e9bcf` (09-18 **15:31**)
  → 이 건은 **고치기 4분 전에 만들어진 마지막 중복**이다.
- ⚠️ 고침은 새로 생기는 것만 막았고 **이미 갈라진 건 그대로 남아 있다**: 0131 5쌍 / 6674 1쌍.
- 다음: (a) 10/20 일정은 사장님이 지우면 끝. (b) 갈라진 고객 합치기 = 되돌릴 수 없는 작업이라 승인 후.

## 2026-09-23 19:40 · server — 재발 방지 2차 (페이블 자문 → 검증 후 반영)
오늘 사고(접수서 폴링 2시간 500)의 세 겹 구멍을 각각 채운다.
**뿌리 = ①코드를 한 번도 실행 안 해보고 배포 ②배포 후 손님 길을 안 눌러봄 ③깨져도 아무도 안 알려줌.**

- `INTAKE_BASE_COLS` 를 **손으로 안 적는다** — `_INTAKE_SELECT_COLS` 에서 세어 만든다(23 확인, 괄호 안 콤마 없음).
- `/healthz/deep` — DB 쓰기·디스크 2GB·**실제 접수서 행으로 파싱**·**진짜 폴링 핸들러 호출**·최근 10분 손님 길 500·죽은 백그라운드 일꾼. 아프면 503 + `problems`.
- `_selfcheck_loop()` 5분마다 자가검진, **상태 바뀔 때만** 슬랙(아픔/회복).
- 알림 열쇠를 **경로 틀**(`/q/{token}/doc`)로 — 토큰마다 다른 길로 세어 상한을 몇 분 만에 소진하던 것.
- 손님 길은 시간당 상한 **예외** — 봇 잡음이 진짜 사고를 삼키던 것.
- 기동 시 "서버 켜짐 · 커밋" 한 줄 → **이 줄이 안 오면 알림 통로가 죽은 것**.
- `X-Ringgo-Smoke` 헤더면 열람 기록 안 남김 — 점검이 손님 접수서에 "열어봤음 +1" 찍던 것.
- `smoke.sh` — `/healthz/deep` 추가, 토큰을 목록에서 **그때그때** 뽑음(고정 토큰 만료 오탐 제거), `"items"` 대신 `"token"`(빈 배열 통과 막음).
- `deploy_phase1.sh` — 0.5단계 **라이브 3.9 로 먼저 파싱**(실패면 손도 안 댐), `main.py.prev` 사본, healthz 실패 또는 **smoke 실패 → 자동 되돌림 → 재기동 → 재검사 → 슬랙 보고 → exit 1**.

검증(내가 직접 돌림): TestClient 21개 전부 통과. **오늘 사고 재현(상수 20) → deep 503 + 정확한 이유** 확인.
Python 3.9 파싱 통과 · PEP 604 없음 · `_dt` 별칭 지킴 · 비밀값 repo 유출 없음.
...
## 2026-09-23 20:05 · server — 운영 기본기 점검 (사장님 "또 이런거 있어?")
- ✅ **업타임로봇** 가동 — `https://api.si0in.kr/healthz/deep` 5분 간격, 메일 알림. 밖에서 보는 유일한 눈.
- ✅ **백업 예약은 돌고 있었다** — launchd `com.detailline.ringgo-backup`, 매일 04:30. 로컬 11벌 보관.
- ✅ **복원 연습 통과** (처음) — 최신 백업(129M→168M) 무결성 `ok`,
  접수서 81/라이브 82 · 블로그 103/103 · app_backups 4/5 (백업 이후 늘어난 만큼 차이, 정상).
  사라졌던 `DjfcGMNv` 도 백업 안에서 읽힘. **"백업이 있다" 가 아니라 "복원된다" 를 확인했다.**
- 🔴 **외장하드 복사가 9일째 조용히 실패** — `Operation not permitted` (2026-09-14 이후 외장 0벌).
  launchd 가 띄운 `/bin/bash` 에 맥OS 전체디스크 접근 권한이 없어서. 로그에 WARN 만 남고 아무도 안 봤다.
  → 사장님 액션: 시스템 설정 › 개인정보 보호 및 보안 › 전체 디스크 접근 권한 › `/bin/bash` 추가.
- 🔴 **클라우드 백업 자리 없음** — 맥미니에 구글드라이브/드롭박스/iCloud 폴더가 하나도 없다.
  지금 백업은 **맥미니 1대 + 바로 옆 외장하드(그나마 9일째 실패)** 뿐. 불·도난이면 전부 같이 간다.
- ✅ **`server/requirements.lock.txt`** 추가 — 라이브에 실제로 깔린 124개 버전 그대로 기록.
  평소 배포 흐름은 **안 바꿨다** (오늘 이미 한 번 과거로 되돌아가서, 배포 경로는 더 안 건드림).

## 2026-09-23 20:20 · 정정 — 외장하드 전체디스크 권한은 **주지 않는다**
앞 블록에서 "`/bin/bash` 에 전체 디스크 접근 권한을 주자"고 적었는데 **철회한다.**
그 외장하드는 백업 전용이 아니라 **사장님 작업 자산이 든 드라이브**다:
`housepick-photos` · `영상 파일` · `줄눈 관련 자료` (263GB 사용). ringgo-backups 는 195M 뿐.
`/bin/bash` 에 전체 디스크 권한을 주면 **launchd 가 띄우는 모든 스크립트**가 그 사진·영상까지
읽고 쓸 수 있게 된다. DB 백업 195M 하나 때문에 치를 대가가 아니다.
→ 대신 **클라우드로 보낸다**(홈 폴더 안이라 추가 권한 필요 없음). 외장하드 복사는 끈다.
⚠️ 별건: 그 드라이브의 사진·영상 263GB 가 **유일본인지 확인 필요** — 서버 DB 보다 큰 위험일 수 있다.

## 2026-09-23 20:12 · server — 백업 스크립트 개정 (조용한 실패 제거)
`backup_cache.sh` 를 고쳐 라이브(`~/ringgo-server/`)에 반영. 이전 판은 `.bak.*` 로 남김.
- **외장하드 제거** — 거긴 사장님 사진·영상 작업 드라이브(263GB)다. 백업 195M 때문에
  `/bin/bash` 에 전체 디스크 권한을 줄 이유가 없다.
- **바깥 보관 = 클라우드 폴더 자동 탐지** (`~/Library/CloudStorage/GoogleDrive-*/My Drive` → 그 밖 → iCloud).
  홈 폴더 안이라 맥OS 추가 권한이 필요 없다. **폴더가 생기는 순간 자동으로 쓰기 시작**한다.
- **조용한 실패 제거** — 스냅샷/무결성/압축 실패는 즉시 슬랙.
  바깥 사본은 매일 떠들면 무뎌지니 **3일 넘게 못 만들었을 때만** 운다(`~/ringgo-backups/.last_offsite`).
- 시험 실행 ✅ 로컬 12벌 · "바깥 보관 자리 없음" 정확히 감지 + 슬랙 경고 발사(첫 회, LAST=0).
- 🔜 사장님: 맥미니에 구글드라이브 데스크탑 설치 → 로그인. 그 뒤 새벽 4:30 부터 자동으로 바깥 사본 생김.

## 2026-09-23 20:12 · server — 바깥 백업 ✅ 가동 확인
구글드라이브(`hugman2080@gmail.com`) 설치됨 → 폴더 자동 탐지 성공
(`~/Library/CloudStorage/GoogleDrive-hugman2080@gmail.com/내 드라이브/ringgo-backups`, 한국어 폴더명도 잡음).
- 시험 실행: 로컬 13벌 · **바깥 1벌**. 로컬↔드라이브 **125,609,775 바이트 동일 · SHA-256 동일**.
- `.last_offsite` 기록됨 → 3일 넘게 실패하면 슬랙이 운다.
- 이제 매일 04:30 자동. 맥미니가 통째로 죽어도 구글드라이브에 30벌 남는다.

## 2026-09-23 20:20 · server — 바깥 백업 보관 수 30 → 14
사장님 구글드라이브가 **83% 참** (83.57GB / 100GB, 여유 16GB).
구글은 드라이브가 꽉 차면 **Gmail 까지 멈춘다** — 백업 때문에 메일이 멈추면 본말전도다.
30벌(3.6GB) → **14벌(약 1.7GB)**. 2주치면 복구엔 충분하다. 로컬도 14벌 그대로.

## 2026-09-23 20:45 · server — 업타임로봇이 빨간불이던 이유 = 우리가 HEAD 를 거부
업타임로봇 첫 점검부터 계속 Down, Root Cause **405 Method Not Allowed**. 서버는 멀쩡했다.
바깥 감시는 몸통이 필요 없어 **HEAD** 로 두드리는데 `/healthz`·`/health`·`/healthz/deep` 이
`@app.get` 이라 405 를 돌려줬다. → `@app.api_route(..., methods=["GET","HEAD"])` 로 변경.
검증: HEAD/GET × `/healthz`·`/healthz/deep` **4가지 전부 200**, smoke 10/10.
⚠️ 교훈: **거짓으로 우는 감시는 안 우는 감시만큼 나쁘다.** 양치기 소년이 되면 진짜 사고를 놓친다.

## 2026-09-23 21:20 · server — 카카오맵 API 가 막혀 있었다 (주소 찾기)
카카오 개발자 알림으로 발견. `/api/address-resolve` 가 부르는 카카오 로컬 검색이 계속 거절당함.
- 처음 `403` = **링고 서비스(ID 1469156) 앱에서 카카오맵 기능이 꺼져 있었음** → 사장님이 ON.
- 켜니까 `400 "API limit has been exceeded."` = **그 앱엔 무료 쿼터가 0**.
  카카오 정책상 무료 쿼터는 **처음 카카오맵을 켠 앱(디테일라인 · ID 1373563)** 하나에만 준다.
- 고객 화면은 안 깨졌다 — 카카오가 실패하면 조용히 **AI(하이쿠)** 로 넘어간다.
  그래서 아무도 몰랐다. 대신 **공짜로 될 일을 AI 값 주고** 하고 있었고 주소 정확도도 떨어진다.
- 🔜 고침: `KAKAO_REST_API_KEY` 를 **디테일라인(1373563) 앱의 REST 키**로 교체 (사장님이 키 주셔야 함).
- ✅ 같이 고침: 거절 이유를 로그에 남긴다. 전엔 `status=403` 숫자만 남아서 원인을 찾으려고
  카카오에 직접 물어봐야 했다. **남이 거절하면 그쪽이 준 이유를 같이 적는다.**
- 🧪 오늘 만든 배포 관문이 실제로 일했다 — 패치가 문법 오류였는데 **복사 전에 잡아서 서버를 안 건드렸다.**

## 2026-09-23 23:10 · server — 카카오 키 교체 ✅ 주소 찾기 되살아남
`KAKAO_REST_API_KEY` 를 **디테일라인(ID 1373563)** 앱 REST 키로 교체(그 앱이 '카카오맵 무료 쿼터' 보유).
plist 는 PlistBuddy 로 **그 한 칸만** Set — 바꾸기 전 백업(`~/plist-backups/`) + 전후 칸 목록 대조 완료(16개 동일, 유실 0).
실측 비교 — 같은 주소, 같은 요청:
  · 전: source=llm · confidence 0.6 · 도로명 없음 · 좌표 0,0
  · 후: source=kakao · confidence 0.9 · "경기 화성시 효행구 봉담읍 와우로34번길 63" · 신명아파트 · 37.2129,126.9730
좌표가 생겨서 **길찾기가 정확한 지점으로 꽂힌다.** AI(하이쿠) 호출도 그만큼 줄어든다.
⚠️ 링고 서비스(1469156) 앱의 카카오맵은 **다시 끄는 게 좋다** — 무료 쿼터가 없어 결제수단이 붙으면 유료로 나간다.

## 2026-09-23 23:50 · android — [갈라진 손님 합치기]
더보기 › 막히거나 이상하면 › **[갈라진 손님 합치기]**. 번호를 하이픈 있게/없게 적어 한 사람이
손님 둘로 갈라진 자국(고침 `b53e9bcf` 이전 것)을 합친다. 사장님 폰 5쌍 · 하우스픽 1쌍.

**되돌릴 수 없는 일이라 세 겹으로 막았다**
1. 누르면 먼저 **미리보기** — 무엇이 어떻게 될지만 보여준다(이 단계에선 아무것도 안 바뀐다).
2. 합치기 **직전에 서버 백업**을 뜬다. **백업이 실패하면 합치지 않는다.**
3. **아무것도 안 버린다** — 일정·문자·통화요약·녹음·사진·발행문서·팀배정 10개 표를 전부 옮기고,
   빈 칸만 채우고, **덮이는 값(이름·주소·금액·계약금)은 메모에 적어 남긴다.**
   같은 시공일이 겹치면 **지우지 않고 세어서** "일정 탭에서 확인해주세요" 라고 알린다.

새 파일: `CustomerMergeDao.kt` · `CustomerMergeManager.kt` · `CustomerMergeScoreTest.kt`
- 남길 쪽 고르는 잣대는 순수 계산(`CustomerMergeScore`)으로 빼서 **테스트 6개**로 못 박았다.
  테스트가 모순을 잡아냈다 — 주석엔 "일정이 제일 무겁다"인데 점수는 이름+주소가 이기고 있었다(50→120).
- 실제 백업에 규칙을 그대로 돌려 검증. 그 과정에서 **금액 45만이 조용히 사라질 뻔한 것**을 발견
  (남는 쪽 15만에 가림) → '덮이는 값을 메모에 남긴다'를 추가했다.
  `적어둔 값을 안 덮는다`는 지켰어도 **덮인 쪽을 안 남기면 그것도 버리는 것**이다.
검증: assembleRelease 통과(style 2824/2934) · 단위 테스트 전부 통과.

## 2026-09-24 00:10 · android — 정산은 '번호'가 아니라 '어느 현장'
사장님: "돈을 받았으면 번호가 아니라 어떤 현장인지. 번호는 작게 표현해도 되는데."
- `util/SiteLabel.kt` 신규 — 제목은 **이름 → 주소(시·도 떼고 앞 세 토막) → 번호** 순, 번호는 **곁줄**.
  `SiteLabelTest` 6개. "경기 시흥시 가마길 4 ㅓㅁ머머" → **"시흥시 가마길 4"**.
- 🔴 **지난 시공(jobs)에서 온 돈 줄은 단서가 아예 없었다** — 번호만 덩그러니. 이제 주소·시공일이 곁줄에.
- 정산 미수 카드/완납 줄도 같은 규칙. `CashFlowCalcTest` 2개를 새 규칙으로 고침.
- ⚠️ 사장님 지적: "봉담 18평 빌라 탄성 카드는 옛 UI 같다" → **같은 컴포넌트다.**
  이름이 있으면 글자 아바타 + '이 시공 일정 보기' 줄이 붙고, 없으면 사람 아이콘만. 데이터 차이지 옛 UI 아님.

## 2026-09-24 00:10 · android — 되찾기가 조용해졌다 + 무엇이 들어왔는지 보여준다
사장님: "1건이라는데 무슨 1건이 들어왔는지 보이질 않으니까 확인할수가없네" / "푸쉬알람으로 접수서가 한번에 다 들어왔는데"
- `resyncMissing` 이 **숫자 대신 들어온 접수서 목록**을 돌려준다 → 창으로 번호·시공일·금액·주소를 보여준다.
- `sync(notify=false)` — 되찾기는 **알림을 안 울린다.** 사장님이 직접 누른 것이고 결과는 창으로 본다.
  여기서 울리면 **진짜 새 접수서 알림이 묻힌다.**
- 실제로 되찾은 1건 = `DjfcGMNv` (010-9928-4027 · 9/16 제출 · 9/29 시공 · 80만원 · 인천 부평). 사라졌던 그 건 맞음.
- 합치기 실패를 "백업이 안 돼서"로 **거짓 보고**하던 것도 고침(한 runCatching 으로 묶여 있었음).

## 2026-09-24 01:20 · server — 후속 문자를 줄글로 · 상호 지어내던 것
사장님: "좀 자연스럽게 못하나 통화 내용정리를. 지금은 딱딱 끊어지는데" → (줄글 안) "너무 자연스럽고 좋다"
- `CALL_SUMMARY_SYSTEM` 의 `suggested_followup_sms` 규칙: **`- ` 불릿 금지 → 이어지는 줄글**.
  빈 줄로 나눈 2~3 문단, [왜 그렇게 정했는지 → 얼마·언제 → 다음에 하실 것] 흐름.
  줄마다 "안내드렸어요" 반복도 금지(딱딱함의 절반은 그것이었다).
- 🔴 **상호를 지어내고 있었다** — 실측 "방금 통화드렸던 **줄눈/타일 시공 사장**입니다."
  시스템 프롬프트 페르소나에서 끌어다 쓴 말. **고객에게 나가는 글**이라 없는 이름이 박히면 안 된다.
  → 프롬프트에 `__BIZ_NAME__` 칸을 두고 `_web_owner_biz_suggested(owner)` 로 채운다.
  모르면 빈 칸이 아니라 **"(모름 — 문자에 이름을 쓰지 말 것)"** 으로 채운다. 빈 칸이면 또 지어낸다.
- 검증(실제 서버 호출): 전 "줄눈/타일 시공 사장" → 후 **"방금 통화드렸던 하우스픽입니다."**
- 프로토: artifact/C9u4cizRHUy7psniKFLkUV (지금/B/B′/후속문자 + 말투 3안)
🔜 남음: 요약 불릿을 **시간 구간**으로 (`0:00–0:35`). 재료(`start_ms`)는 이미 있으나
   요약 프롬프트엔 **시각이 안 들어간다** — 타임스탬프가 붙은 전문을 넣어줘야 한다.

## 2026-09-24 02:10 · server + android — 통화 요약을 시간 구간으로
사장님: "몇 초부터 몇 초까지 요약이랄까 — 이거 되게 좋은 방법이다 싶네"
- 서버: 받아쓰기를 `[m:ss]` 붙여 넣고, 불릿 형식을 **`시작-끝|화자|한 문장`** 으로.
  응답은 **둘 다** — `bullet_rows`(구조) + `bullets`(옛 문자열). **이미 배포된 앱은 지금과 똑같이 보인다.**
- 앱: `util/CallSummaryLines` 한 군데서만 읽는다(새 모양·옛 모양 둘 다).
  요약 화면 = 왼쪽 시각 · 점(손님/나) · 문장. 통화 카드·상담함·채팅 카드도 같은 파서로.
  ⚠️ 사장님이 먼저 짚었다 — "전화오면 뜨는 그 화면… 그건 문제없겠지~?" **거기가 깨질 자리였다.**
  `firstLineOf` 가 직접 쪼개고 있어서 `0:00-0:35|손님|…` 이 그대로 보일 뻔했다.
- 점(손님/나)은 **AI 짐작**이다(녹음 소리가 아니라 글을 읽고 고름) → 화면에 그렇게 적어뒀다.
- 실서버 검증: `0:00-0:35 / 0:35-1:20 / 1:20-2:05 / 2:05-2:47`, 마지막이 통화 길이로 정확히 닫힘.
- 검증: `CallSummaryLinesTest` 7개 신규 · assembleRelease + 전체 단위테스트 통과(style 2824/2934).

## 2026-09-24 03:10 · android — 통화 후 문자 카드
사장님 지시 그대로. **배치·순서·문구는 안 바꿨다.**
- 🔴 **파란 막대가 "다 차면 보내진다"로 읽혔다** — 사장님: "내가 처음에 그랬거든."
  파랑은 우리 앱에서 '누르는 색'이다. → **회색**으로. 막대 밑 "그냥 두면 닫혀요",
  보내기 버튼 밑 "눌러야 나갑니다 · 그냥 두면 안 보내고 닫혀요".
  **오해가 나는 자리는 버튼 옆**이라 거기서 못을 박는다.
- **[고치기] 버튼** + 문자 직접 누르기. 고친 글은 **이번 발송에만**(`sendEdited`) — 템플릿 불변.
  한 손님한테 깎아준 값이 다음 손님한테 나가면 안 된다. 고치는 동안 **창이 안 닫힌다**(`pauseSafety`).
- **안 자른다** — `replace("\n"," ")` + 4줄 컷 제거. 줄바꿈 살리고 넘치면 그 칸에서 스크롤.
  사장님: "그냥 스크롤로 다 보여줄수있는거아닌가" — 자르고 "…더 있어요"는 **문제를 안내로 덮는 것**이었다.
- 디자인 시스템: 이 파일이 혼자 들고 있던 색·크기·둥글기를 `AppTheme.colors`/`AppType`/`AppShape` 으로.
  파랑은 원래 같은 값. 26·18·13·10 → xl·lg·md·sm. 이제 `DesignSystem.kt` 한 곳만 고치면 따라온다.
- "← 옆으로 넘겨 다른 문자 보기" 제거 — 점이 이미 그 말을 한다.
- ⚠️ 사장님 폰엔 **템플릿 1번만** 채워져 있다(백업 확인). 2·3을 채워야 넘기기가 보인다.
프로토: artifact/7UJv2f1go6W5xEAg9Mfy3i (만져지는 것)

## 2026-09-24 03:40 · android — 접수서 보내기 창 정리
사장님 프로토 확인 후. **있는 것을 빼거나 옮기지 않았다** — 라벨 줄 모양과 여백만.
- 라벨 줄 = **왼쪽 이름 / 오른쪽 늘 지금 고른 값** (`EstLabelRow`).
  전엔 같은 자리에 시공일은 **값**, 시공 기간은 **설명**이 와서 눈이 매번 판단해야 했다.
  이제 스크롤 중에도 뭘 골랐는지 보인다. 설명은 뺐다 — 칩이 이미 그 말을 한다.
- 같은 라벨 코드 **6곳 복사** → `EstLabelRow` 하나.
- 여백 3·4·6·8·9·10·11·12·14·17dp → **`AppSpace.s24`(섹션) / `s12`(머리-내용)**.
- `별도 (+10%)` → **`별도`** (2026-09-23 사장님: 부가세는 네 글자만).
- 손으로 적은 값 2821 → **2810**.
프로토: artifact/CxoUTNsCkJ93k8bQGDFNp4

## 2026-09-24 02:40 · android
앱 꺼짐(업데이트 중) 원인 확정·수정 + 접수서 창 재배치 + 달력 통일 + 일정 카드 금액
- 변경: 서버 인터페이스 변화 없음. 앱 UI/크래시 안전망만.
- commit: ba4697fa
- 다음 액션: 없음 (서버측 무관)

## 2026-09-24 12:55 · android
통계 탭 → 「내 기록」 전면 개편 (지도 포함) + 앱 꺼짐 뿌리 수정 + 빌드 가드
- 변경: 서버 인터페이스 변화 없음. DB v57 → **v58**(jobs.recordNo = 현장 번호).
...
## 2026-09-25 12:20 · android (사장님 지시로 server 직접 수정)
통화 요약 사고 수정 — 18분21초 통화가 "부재중 콜백" 한 줄로 요약되던 것.
- 사고: 9/24 19:38 통화(1101초)가 `title="부재중 콜백"`, 줄 1개, 시각 `0:00-1:101` 로 저장됨.
- 원인 3겹:
  ① Gemini 가 503(붐빔) 으로 실패 → **첫 실패에 바로** Haiku 로 넘어감 → Haiku 가 긴 통화에서
     무너져 **프롬프트 예시에 적힌 "부재중 콜백" 을 그대로 베낌**.
  ② 프롬프트가 "길이(초)를 m:ss 로 바꿔 쓴다" 며 **AI 에게 나눗셈을 시킴** → 1101 → "1:101".
  ③ 받아쓰기 8000자 컷이 18분 통화의 뒤 1/3 을 잘라먹음.
- 변경 (server/main.py, 라이브 ~/ringgo-server 에도 동일본 배포·재기동 완료):
  · `_call_gemini_json_for_summary` — 500/502/503/504 면 3초·6초 쉬었다 최대 3회 재시도
  · `_fmt_mmss()` 신설 + `[통화 메타]` 를 `길이: 18:21 (1101초)` 로 (두 경로 모두)
  · CALL_SUMMARY_SYSTEM — 1분 넘는 통화에 "부재중/콜백 필요" 금지 + 예시 베끼기 금지 +
    사적 통화는 억지로 업무로 만들지 말 것
  · `_summary_looks_broken()` 신설 — (1분↑인데 부재중) / (5분↑인데 2줄 이하) 이면 **한 번 더 시킴**
    (녹음 경로. gemini → 실패시 haiku 순)
  · `_coerce_call_summary(parsed, duration_sec)` — `0:00-1:101` 같은 엉터리 시각은 버림
    (앱 CallSummaryLine.kt 와 같은 잣대)
  · 녹음 경로 받아쓰기 컷 8000 → 16000자
...
## 2026-09-25 13:45 · android
「내 기록」 탭 + 인증샷 창 — 프로토/시안 반영 (사장님 승인 2건)
- 내 기록: 지도가 주인공. 달 고르기 맨 위 한 줄 / 지도 260dp 첫 카드 / 현장번호 제목 옆 배지 /
  세 칸 숫자 지도 밑 / 할 일 한 줄(각 칸이 제 목록으로) / [인증샷][영상 만들기] 나란히.
- 인증샷: 「이미지 스타일」(사진 위에·숫자 중심·지도 포함) 과 「이미지 비율」(4:5·1:1·9:16) 을 분리.
  업체명·연락처 켜고 끄기(끄면 그만큼 지도가 커짐). "무엇을 자랑할까요?" 문구. 저장 크기 표시.
  창이 길어져 버튼이 화면 밖으로 밀리던 것 → 창 높이 가두고 속을 스크롤.
- 서버 변경 없음.

## 2026-09-25 14:05 · android (+server 한 줄)
🔴 「다시 요약」 두 번째 원인 — **앱이 먼저 끊었다.** 서버는 다 만들어놓고 못 전달.
- 증상: 2분20초 통화 다시 요약 → 폰엔 옛 "부재중 (콜백 필요)" 그대로.
- 서버 기록: force_refresh → STT·보정·화자분리 → `→ ready haiku` 까지 찍힘.
  **그런데 POST 응답 access 로그가 없다** = 받아갈 사람이 없었다.
  캐시엔 제대로 들어가 있었다("개인 통화 (업무 무관)").
- 원인: 이 통화는 **한 번에 받아오는 길(동기)** 을 탔다. 기준이 3분/3MB 인데 2분20초·2.2MB.
  그 길은 Cloudflare 가 **100초**에서 자른다. STT+보정+화자분리에 9/25 넣은 구글 재시도(3+6초)까지
  얹혀 넘겼다.
- 고침:
  · app `ASYNC_DURATION_SEC` 180→**45초**, `ASYNC_BYTES` 3MB→**700KB**
    (맡겨두고 물어보는 길은 8초 폴링이라 오래 걸려도 안 끊긴다 — 뿌리 수정)
  · app 동기 경로가 IOException 이면 `tryFetchResult` 로 **/result 를 6번 더 물어본다**
    (서버엔 이미 답이 있다 — 100초를 날리고 실패로 끝내지 않는다)
  · server 구글 재시도 쉬는 시간 3·6초 → **2·4초** (배포·smoke 완료)

## 2026-09-25 14:30 · android
🔴 채팅 통화카드가 **시간 구간을 버리고 있었다** — 사장님: "원래 시간대가 나와서 그 시간대 요약이 있었는데"
- 서버는 `0:00-0:46|나|…` 를 제대로 주고 있었고(실측 확인), 통화요약 화면은 그대로 그렸다.
  **채팅 통화카드만** `.map { it.text }` 로 시각·화자를 버리고 문장만 그렸다.
- 뿌리: 읽는 코드(CallSummaryLines)는 한 군데였는데 **그리는 코드가 두 벌**이었다.
  2026-09-24 에 시간 구간을 넣을 때 한 벌만 고쳤고, 나머지가 사장님이 매일 보는 쪽이었다.
...
## 2026-09-25 15:55 · android
지도 — **끝까지 확대하면 옆으로 한 번 꺾이던 것** (사장님: "줌 인을 쭉 하면 마지막에 좌측으로 한번 꺾인다")
- 그리는 식에서 역산: 보는 창 반폭=1/(2z), 중심 이동=panX/z → 틀 안에 있으려면 **|panX| ≤ (z-1)/2**.
  코드는 ±3 **고정**. 8배에선 3.5 가 필요한데 3 에서 잘려 집은 자리가 어긋났다.
- 고침: `lim = ((nz-1)/2).coerceAtLeast(0.05f)` — 배율을 따라간다.
  덤으로 1배에서 지도를 화면 밖으로 밀어내던 것도 같이 막힌다.
- 단위 테스트 MapPanLimitTest 5개(창이 틀 밖으로 안 나가는지 숫자로).
- ⚠️ adb 로는 두 손가락 확대가 재현이 안 돼 **손으로는 확인 못 함** — 계산과 테스트로만 확인.

## 2026-09-25 16:30 · android
현장 사진을 인증샷에 — **B(지도+사진) · C(사진이 배경) 둘 다 고를 수 있게.** (프로토 EZkZj76N 승인)
- 대표 사진 = 그 현장에 **제일 먼저 올린** 사진(사장님 확정). 어느 현장 = 그 달 **제일 최근** 현장부터.
  파일이 사라졌으면 없는 셈 친다.
- SitePhotoDao.observeAllOldestFirst 신설 → StatsViewModel 이 4번째 재료로 받아 MyRecordState.photoPath.
- RecordShot: Shape.MAP_PHOTO / Shape.PHOTO 추가. loadPhoto(inSampleSize) + drawCover(가운데 잘라 꽉).
  사진 없으면 지도 갈래로 자동 하향 — 빈 네모를 내놓지 않는다.
- 인증샷 창: 「이미지 스타일」이 다섯이라 줄바꿈 칩으로. 사진 없는 달이면 두 갈래를 **안 보여주고** 안내 한 줄.
- 고객상세: 첫 사진에 파란 「대표」 딱지 + "「대표」 사진이 인증샷에 들어가요 (맨 처음 올린 사진)".
- 폰 확인: 대표 딱지 · 안내문 · 갈래 숨김/표시 · B · C 전부 눈으로 봄.
  B 사진 자리는 오른쪽 아래→**오른쪽 위**(영통·동탄 점을 덮었다), C 동네줄 여백 118→142.

## 2026-09-25 17:05 · android
사장님 지적 3건.
1) 🗺 **동네 줄엔 「대구」가 있는데 지도엔 없던 것** — 좌표표엔 대구의 *구*(중구·수성…)만 있고
   "대구" 라는 이름 자체가 없다. 주소가 "대구시 …" 면 이름이 "대구" 로 뽑히는데 자리를 못 찾아
   점을 안 찍었다. 글과 지도가 다른 말을 하면 둘 다 못 믿는다.
   → `RegionCoords.sidoCenter()` — 시·도 이름으로 떨어지면 **그 시·도의 첫 자리**(보통 중구)에 찍는다.
   단위 테스트 RegionSidoCenterTest 6개. (⚠ "광주시"=경기 광주시가 맞다 — 그대로 둠)
2) 🎬 **영상에 현장 사진이 없던 것** — 영상은 제 틀로 그려서 사진을 몰랐다.
   → RecordReel.Data.photoPath + 지도 오른쪽 위에 그림과 **같은 자리**. 사진은 한 번만 읽는다(240컷).
   RecordShot.loadPhoto/drawCover 를 internal 로 열어 **한 벌만** 쓴다.
3) 📊 **진행 표시를 차오르는 막대로** — "영상 만드는 중 34%" 글자만 있던 것.
   drawBehind 로 글자 뒤가 차오른다(자리 안 흔들림) + animateFloatAsState 로 부드럽게.
- ⚠️ 테스트폰이 중간에 빠져 **진행 막대는 눈으로 못 봄**(빌드·단위 통과).

## 2026-09-25 17:25 · android
🚛 **영상에서 현장에 도착할 때마다 그 현장 사진으로 바뀌게.** (사장님 "그 현장에 도착할때마다 바껴야하는데 왜안바껴")
- 한 장을 10초 내내 띄우면 장식이지만, 도착마다 바뀌면 "여기 가서 이렇게 해놨다"가 다섯 번 이어진다.
- 도착 시점은 **지도가 이미 계산하던 값**(arrivedUpTo)을 `onArrived` 로 꺼내 쓴다.
  ⚠️ 밖에서 따로 세면 트럭과 사진이 어긋난다 — 같은 셈을 두 번 하지 않는다.
- RegionDot.photoPath 신설 → StatsViewModel 이 동네마다 그 현장 첫 사진을 채운다.
- RecordReel: 동네별 사진을 **미리 다 읽어둔다**(320px, 최대 12곳). 그 동네에 사진이 없으면
  **직전 사진을 그대로** 둔다(깜빡임 방지). 끝나면 전부 recycle.
- 진행 막대 색: primaryBg(#EEF4FF)가 바탕 회색과 거의 같아 **안 보였다** → TossBlue 32%.

## 2026-09-25 17:40 · android
🗺 **미리보기(그림)와 다운받은 영상의 지도가 달랐다.** (사장님 "다운받기 전엔 이렇게, 다운하면 이렇게")
- `RecordShot.renderMapHero`(「지도 포함」·「지도+사진」) 만 `drawRegionMap` 에 **geo·trip·zoom 을 안 넘겼다.**
  → 진짜 지도(Natural Earth 바다·땅·큰길) 대신 **옛 손그림**으로 그리고, 길찾기 없이 **직선**으로 이었다.
  영상(RecordReel)은 넘기고 있어서 둘이 달라 보였다.
- **보이는 것과 나오는 게 다르면 미리보기가 쓸모가 없다.** geo·trip·zoom/pan 을 똑같이 넘긴다.
- ⚠️ 테스트폰이 빠져 눈으로는 못 봄(빌드 통과). 업무폰은 읽기 전용이라 설치 안 함.

## 2026-09-25 17:50 · android
🎬 **영상 미리보기** — 만들기 전에 움직이는 걸 본다. (사장님 "어떻게 영상이 저장될까 프리뷰처럼
   볼수있으면 좋은데 그냥 멈춰있어")
- 인증샷 창 미리보기 밑에 [그림 미리보기] / [영상 미리보기] 두 갈래.
- **영상을 그리는 그 코드(RecordReel.drawFrame)를 그대로** Compose Canvas 에 돌린다.
  ⚠️ 따로 흉내내면 미리보기와 결과가 갈린다 — 오늘 지도에서 이미 한 번 밟았다(17:40 건).
- 10초에 한 바퀴 무한 반복(영상과 같은 길이). 동네별 사진도 영상과 같은 방식으로 미리 읽고,
  창 닫을 때 recycle.
- ⚠️ 테스트폰이 빠져 눈으로는 못 봄. 업무폰은 읽기 전용이라 설치 안 함.

## 2026-09-25 18:05 · android
🔀 **순서가 거꾸로였던 것 + 취소가 안 되던 것.** (사장님 "순서가 안맞는데???" / "취소도안되")
- [영상 만들기] 를 누르면 **바로 만들기 시작**해놓고, 창엔 **영상에 안 먹히는 그림 설정**
  (이미지 스타일·비율)을 고르라고 띄웠다. 고르라고 해놓고 안 먹히는 게 제일 나쁘다.
  → 이제 **영상 미리보기로 열기만** 한다. 보고 나서 [이 영상으로 저장].
  → 영상일 땐 그림 전용 설정을 **아예 숨긴다.** 대신 업체명·연락처 토글은 **영상에도 먹히게**
    (RecordReel.Data.sign).
  → 그림 저장 버튼도 그림일 때만. 「영상 만들기」 버튼은 「영상으로 보기」로.
- ⏹ **취소가 무시되던 진짜 이유**: VideoMaker 인코딩 루프에 취소를 보는 자리가 없었다
  (`while(!done)` 안에 ensureActive 없음). 눌러도 끝까지 만들고 진행률이 계속 올라갔다.
...
## 2026-09-25 18:45 · android
🐞 **「완료를 안 누른 곳」인데 줄마다 초록 「완료」 딱지가 붙어 있었다.**
   (사장님 업무폰 "완료 인데 왜 이 페이지에 나와?")
- 거르기는 맞았다(`!it.done`). **딱지를 손으로 박아 둔 게 문제** —
  `VisitedRowItem(v, TossSuccess, "완료", …)` 처럼 글자를 고정해 실제 상태를 안 봤다.
  원래 "다녀온 = 지난 것 = 완료" 전제로 만든 화면인데, 9/25 에 「완료 안 누른 곳」이 생기며
  전제가 깨졌고 딱지만 옛 전제에 남았다. ← **또 잣대가 두 벌.**
- 고침(같은 화면 끝까지):
  · 딱지를 `v.done` 으로 — 완료면 초록 「완료」, 아니면 주황 「완료 안 누름」
  · 머리글 "(완료)" 제거 + 거르는 중엔 점 색도 주황
  · **매출 합계도 걸러진 줄만** 더한다(`VisitedRow.amountManwon` 신설).
    전엔 "2곳"은 걸러지고 "475만원"은 그 달 전체라 "이 2곳이 475만원"으로 읽혔다.
  · 빈 화면 문구를 상황대로 — 걸러서 없으면 "다 챙기셨네요", 아니면 "{그 달} 현장이 아직 없어요"
    (달 고르기가 생겨 "이번 달"이 거짓이 될 수 있다)
  · 띠 문구가 길어 [전체 보기]와 붙던 것 → 띠는 짧게, 하는 법은 아래 한 줄로
- ✅ **테스트폰에서 눈으로 확인** (딱지·매출·머리글·띠 전부).

## 2026-09-25 18:55 · android
🖐 **지도 위에서 화면이 안 내려가던 것.** (UX 점검 P2)
- `detectTransformGestures` 는 손가락 **하나**로 끌어도 다 먹는다. 지도가 「내 기록」 맨 위
  260dp 카드라 **화면 위 1/3 이 스크롤 불가 구간**이었다. 게다가 1배에선 지도가 거의 안 움직여서
  (한도 ±0.05) **먹어놓고 아무 일도 안 했다.**
- 사진 보는 앱 규칙으로 직접 짬(`awaitEachGesture`):
  · 기본(1배) — 한 손가락은 **안 먹는다** → 화면이 스크롤된다
  · 두 손가락이거나 **이미 확대된 뒤** — 지도가 먹는다(그때는 그게 자연스럽다)
  · 두 번 톡 = 1배 복귀 → 확대 상태에서 **나올 길**
- ✅ 폰 확인: 지도 위 한 손가락 끌기로 **화면이 내려감**. 한 번 톡 = 다시 달림. 크래시 없음.
- ⚠️ **두 손가락 확대는 adb 로 재현이 안 돼 손으로 확인 못 함** — 사장님 확인 필요.

## 2026-09-25 19:05 · android
UX 점검 **남은 것 넷 마무리.**
- 📊 **진행 막대가 40%→0% 로 되감기던 것** — 세 번 시도(하드웨어→소프트웨어→작게)가 각자 0부터
  진행률을 불렀다. → 막대는 **뒤로 안 간다**(지금까지 간 데 기억) + "다른 방법으로 다시 만드는 중…" 문구.
  덤으로 `RecordReel.lastMadeSize` — ③으로 떨어지면 540×960 인데 창엔 720×1280 이라 적혀 있었다.
- 🔵🟢 **점 범례가 통화요약 화면에만** 있었다(채팅 통화카드엔 없음) → `CallSummaryBody` 안으로.
  그리는 곳이 하나라 **두 화면이 자동으로 같이** 갖는다. 옛 안내는 제거(두 벌 방지).
- 👆 **손가락 자리** — 달 화살표 34→46dp · 할 일 띠 세 링크에 각자 여백(옆 것이 눌렸다) ·
  인증샷 닫기 6→14/10dp · 요약 시각 알약 세로 2→6dp + 줄 간격 4→8dp ·
  「다시 요약」(돈 드는 쪽)과 「요약 수정」 사이 14dp.
- 🟢 **「▲ 0%」가 초록**이던 것 → 0은 "변화 없음" 회색. (`>= 0` 이 원인)
- ✅ 폰 확인: 화살표·할 일 띠 여백 눈으로 봄, 크래시 없음. 단위 549개 · 실패 0.

## 2026-09-25 19:35 · android
🎬 **"5집인데 사진은 4개만 나온다"** — 마지막 현장 사진이 **0.7초**만 스치고 끝났다.
- ⏱ **끝에서 한 박자 쉰다** (`RecordReel.END_HOLD = 0.12`). 마지막 도착이 한 바퀴의 93% 지점이라
  남은 시간이 0.7초뿐이었다. 이제 88% 에서 다 달리고 남은 1.2초는 다 그려진 채로 머무른다.
  → 폰에서 실측: 마지막 사진이 **한 바퀴의 18%(≈1.8초)** 보인다(전 7%).
- 🔒 **미리보기도 같은 값**으로 그린다. 영상 만들 때만 넣었더니 미리보기와 저장본이 또 달라졌다
  (사장님이 이미 한 번 짚은 부류). `END_HOLD` 를 열어 `StatsScreen` 미리보기에도 넘긴다.
- 🔎 **사진 찾는 범위를 넓혔다** — `SitePhotoRepository.representativeOf` 마지막 수단으로
  **그 고객의 아무 사진**. 1차 탭에서 올린 사진이 2차 건에 안 붙어 조용히 빠지던 경우를 건진다.
- 🗣 **몇 곳에 사진이 있는지 말해준다** — 인증샷 창에 "현장 사진 4/5 · 단원은 아직 없어요".
  조용히 빠지는 대신 **어디를 채우면 되는지** 알려준다. (테스트폰 실측 = 1/5)
- 변경: 앱만. 서버·API 안 건드림.
- ✅ 빌드 EXIT=0 · 단위 549개 실패 0 · 테스트폰(23514638000c7ece) 설치·미리보기 실측.

## 2026-09-25 20:05 · android
🎥 **카메라가 트럭을 따라간다.** (사장님 "영상이 트럭을 중심으로 움직이면 어떨까")
- 전엔 수도권 전체를 멀리서 보여줘서 트럭이 깨알만 했다 — 안 움직이는 것처럼 보인다.
  이제 **2.6배로 따라붙고**, 마지막 현장에 닿으면 **쭉 빠지면서** 한 달 전체 길을 보여준다.
- 빠진 뒤 자리는 **사장님이 지도에서 손가락으로 맞춰둔 그 화면**(d.zoom/panX/panY).
  따라가기가 그 틀을 잡아먹지 않게 했다.
- 🔒 **트럭 자리 셈을 `util/MapRide.kt` 한 벌로 뺐다.** 전엔 `drawRegionMap` 안에만 있어
  카메라가 밖에서 또 계산해야 했다 — 그러면 트럭과 카메라가 어긋난다. 지도도 이걸 부른다.
  덤으로 **보여줄 범위(bounds) 도 한 벌로** — 카메라와 지도가 같은 틀을 봐야 한다.
- 당겨져 있을 땐 **동네 이름을 다 보여준다**(평소엔 많은 곳 4개만) — 안 그러면 지금 어디인지 모른다.
- 변경: 앱만. `MapRide` 는 화면·좌표 모르는 순수 셈이라 단위 테스트가 본다(8개 추가).
- ✅ 빌드 EXIT=0 · 단위 **557개** 실패 0 · 테스트폰에서 5컷 실측(트럭 가운데·끝에 빠짐 확인).

## 2026-09-25 20:30 · android
🎯 **따라가는 동안 트럭은 늘 정가운데.** (사장님 "확대를 해도 그 가운데가 유지되는거 맞지?")
- 카메라가 **가장자리에서 잡히던 것**을 풀었다. 손가락으로 끌 땐 '지도를 잃지 말자'고 잡아두는데,
  지도는 전국이 다 그려져 있어 더 밀어도 빈 데가 안 나온다 → 강서·동탄에서도 트럭이 가운데다.
- 🔎 **단위 테스트가 진짜 문제를 하나 잡았다** — 2.6배에서는 먼 구간에 트럭이 **1초에 화면 두 칸 반**을
  지나가 휙 스쳤고, 그 배율에선 지도에 그려진 게 없어 벌판처럼 보였다. → **2.0배**로.
  이제 해안선·강·둘레 도시 이름이 같이 보이면서도 가깝다.
- 테스트도 눈금을 물리 단위로: "한 컷(24분의 1초)에 화면의 6분의 1 넘게 움직이면 실패".
- ✅ 빌드 EXIT=0 · 단위 557개 실패 0 · 테스트폰 5컷 실측(다섯 컷 모두 트럭 정가운데).

## 2026-09-25 21:10 · android
🔢 **큰 숫자가 달린 만큼 올라간다.** (사장님 "키로수도 후르륵 올라가는 느낌")
- 숫자가 가만히 있으면 지도만 움직이고 위쪽이 죽어 있었다. 이제 **트럭이 달린 만큼** 오른다
  — 길에서는 후르륵, 집에 서면 숫자도 같이 멈춘다(반 초). 끝엔 반드시 진짜 값.
- 주행 상태(`MapRide.at`)를 `drawFrame` **맨 위에서 한 번만** 구해 숫자와 카메라가 같이 본다.
  따로 구하면 숫자는 다 올랐는데 트럭은 아직 가는 중인 일이 생긴다.
- 글자 **크기**는 최종 숫자로 고정(자릿수 늘 때 커졌다 작아졌다 안 하게), **단위는 지금 숫자에 붙어**
  따라온다 — 자리를 박아놨더니 "약 52     km" 처럼 멀찍이 떨어져 보였다(폰에서 확인하고 고침).
- 거리뿐 아니라 집·동네·번 돈 등 **고른 숫자 전부** 같이 오른다.
- ✅ 빌드 EXIT=0 · 단위 **563개** 실패 0(countUp 6개 추가 — "약"·콤마 지킴, 되감김 없음, 끝엔 진짜 값)
  · 테스트폰 실측 70 → 138 → 183km.

## 2026-09-25 22:05 · android
↔️ **km 이 숫자에 닿던 것.** (사장님 "글자가 겹쳤어 km 있는곳 간격")
- 어제 단위를 **지금 숫자에 붙여** 따라오게 하면서 띄우기를 그대로 뒀다. 전엔 **최종 자릿수**에
  박아놔서 우연히 떨어져 있었는데 그 우연이 사라졌다 — 두 자리일 때 `km` 이 숫자에 닿았다.
- 🔒 **띄우기가 네 군데에 각자** 있었다(영상 1 · 그림 3, 값도 12f·10f·0.014w 제각각).
  → `RecordShot.drawBigNumber` 한 곳으로. 띄우기는 **글자 크기의 24%** — 글자가 줄면 같이 준다.
  그림 쪽 세 군데는 더 좁아서(1%) 같은 결함이 있었다. 사장님이 짚기 전에 같이 고쳤다.
- ⚠️ **폰 확인 못 함** — 테스트폰 연결이 끊겼다. 대신 앱에 든 **그 폰트로 직접 그려** 대조했다
  (전: 닿음 / 지금: 여유). 폰 붙으면 눈으로 다시 본다.
- ✅ 빌드 EXIT=0 · 단위 563개 실패 0.

## 2026-09-25 22:40 · android
🧭 **지도 밑 동네 목록이 트럭과 거꾸로 적혀 있었다.** (사장님 "왜 3을 안 거치는 느낌이지?")
- 선을 따라가 보면 다섯 곳을 **다 들른다**(단원→관악→강서→분당→권선). 빠뜨리는 건 없다.
  그런데 지도 밑 목록은 **권선·분당·강서·관악·단원** — 정확히 **반대 방향**이었다.
  순서로 읽고 영상을 보면 트럭이 엉뚱하게 도는 것처럼 보인다.
- 원인은 또 **같은 걸 두 벌**: 지도 점의 순서(`orderOf`)는 **처음 간 날 오름차순**인데,
  동네 목록은 `month`(**최근 것부터**)를 그대로 훑어 만들었다. 같은 자료, 두 가지 순서.
  → 목록도 **처음 간 날 순**으로 훑는다.
- ⚠️ **폰 확인 못 함** — 테스트폰 연결이 아직 끊겨 있다. 붙는 대로 눈으로 본다.
- ✅ 빌드 EXIT=0 · 단위 563개 실패 0.
- 📌 사장님께 물어볼 것: 목록을 `·` 대신 **`→`** 로 두면 "다닌 순서"가 더 분명해진다. 디자인 변경이라 확인 필요.

## 2026-09-25 22:55 · android
→ **동네 목록을 「다닌 순서」로 읽히게.** (사장님 "ㄱㄱ")
- `권선 · 분당 · 강서` → **`단원 → 관악 → 강서`**. 점으로 나열하면 순서인지 목록인지 알 수 없다.
- 🔒 그 글줄 만드는 코드가 **네 군데에 똑같이 복사**돼 있었다(영상 1 · 그림 3).
  한 군데만 고쳤으면 나머지 셋은 점으로 남았다 — 오늘 이 부류를 다섯 번째로 밟는 셈이라
  **먼저 grep 으로 세고** `RecordShot.townLine` 한 곳으로 모은 뒤 바꿨다. 단위 테스트 5개.
- 폭 확인: 6곳 최악이 505 / 616px — 글자가 줄지 않는다(폰트로 직접 재봄).
- ⚠️ **폰 확인 못 함** — 테스트폰 연결이 아직 끊겨 있다.
- ✅ 빌드 EXIT=0 · 단위 **568개** 실패 0.

## 2026-09-25 23:15 · android
📍 **「내 기록」 줄에 다녀온 풀주소.** (사장님 "우리가 갔던 풀주소가 남아야하지 않을까?" → "목록에서 바로 보였으면")
- 주소는 이미 저장돼 있었지만 **줄을 눌러 고객 상세로 들어가야** 보였다.
  같은 성격의 「다녀온 현장」 목록엔 **이미 나오고 있었다** — 또 한쪽만 되어 있던 것.
  그 화면과 같은 모양(12.5sp · TossTextInfo · 한 줄, 넘치면 …)으로 맞췄다.
- 주소가 없으면 조용히 빼지 않고 **「주소 없음」**(주황)이라 적는다 — 어느 현장을 채우면 되는지 보인다.
- ⚠️ **아직 안 고친 것(사장님께 보고만)**: 건에 주소가 박힌 건과, **고객의 지금 주소를 그때그때 가져오는 건**이
  섞여 있다. 후자는 고객 주소를 고치면 **지난 기록의 주소까지 바뀐다.** 기록물로선 문제 — 다음에 다룰 것.
- ⚠️ **폰 확인 못 함** — 테스트폰 연결이 계속 끊겨 있다. 같은 글씨체로 그려서 대조만 했다.
- ✅ 빌드 EXIT=0 · 단위 568개 실패 0. (첫 시도 packageRelease 실패 → `--stop` 후 재빌드 성공)

## 2026-09-26 08:20 · android
💰 **돈은 집마다 다르게 오른다.** (사장님 "각 지역마다 금액이 다른데 그렇게 올라야지")
- 어제 넣은 오름세가 **달린 거리**에 맞춰 모든 숫자를 고르게 올렸다.
  거리(km)는 그게 맞지만, **돈·집 수는 집에 도착해야** 들어온다.
  200만원짜리를 지나든 100만원짜리를 지나든 똑같이 올랐다 — 거짓말에 가깝다.
- 이제 도착할 때마다 **그 동네 몫만큼** 오르고, **반 초 동안 굴러간다**
  (트럭이 서 있는 시간과 같아 떠날 때 딱 멈춘다 · 지폐가 올라오는 그 순간).
- 몫 정하는 법: 돈=그 동네에서 번 돈 · 집=그 동네 현장 수 · 동네=한 곳에 한 칸 ·
  나간 날=현장 수(날짜별 자료 없음) · **거리=몫 없음**(달린 만큼 이어서).
  고르는 곳(`BigPick`)이 자기 몫을 들고 다닌다 — 영상이 단위 글자로 짐작하지 않게.
- 💡 돈을 안 적은 집에선 **안 오른다**(권선 0원) — 없는 돈이 오르면 거짓말이 된다.
- ⚠️ **폰 확인 못 함** — 테스트폰 연결이 어젯밤부터 계속 끊겨 있다.
- ✅ 빌드 EXIT=0 · 단위 **575개** 실패 0 (stepUp 7개 추가 — 두 배 뛰는지 · 끝에 딱 차는지 ·
  0원이면 안 오르는지 · 되감김 · 거리는 그대로).

## 2026-09-26 09:20 · android
✅ **업무폰(1cba6ed49f0c7ece · Play 2070)에서 밀린 것 다섯 개 전부 눈으로 확인.** (읽기만 · 설치 없음)
- 📍 풀주소: "경기 화성시 동탄구 동탄대로24길 199 475동 901호" — 긴 건 …로 잘림, 「다음 현장」엔 안 붙음.
- → 화살표·순서: `강서 → 영통 → 덕양 → 강동 → 동탄` (날짜순).
- 💰 돈: **40 → 150 → 250 → 300 → 475만원** — 실제 금액(강서40·영통110·덕양100·강동50·동탄175)과 일치.
- ↔️ km 간격: 두 자리(51·59)에서도 `km` 이 안 닿는다.
- 🎥 카메라: 트럭이 늘 화면 가운데.

🧭 **여섯 번째 복사본 발견 — 「내 기록」 화면의 동네 줄.**
- 이 줄만 혼자 **「많이 간 순」**으로 정렬하고 있었다. 바로 밑에 "🚛 가 간 순서대로 달려요" 라고 적어두고.
  → `RecordShot.townLine(rec.towns)` 로 — 영상·그림과 **같은 곳**에서 만든다. (take 8 → 6 · 화살표)
- ⚠️ 업무폰엔 설치 못 하므로(Play 설치본) 다음 판으로 나간다.
- 💡 보고: 업무폰 9월 현장은 **대표 사진이 0/5** — 영상에 현장 사진이 하나도 안 들어간다.
- ✅ 빌드 EXIT=0 · 단위 575개 실패 0.

## 2026-09-26 10:05 · android
📸 **사진이 주인공인 영상.** (사장님 "지도는 옆에 작은 스타일로 움직이고 **시공사례를 더 부각**하고 싶은 사람들은 이 메뉴를 선택할수있게")
- 그림(인증샷)엔 「사진이 배경」이 있는데 **영상은 모양이 한 가지뿐**(지도가 주인공)이었다 — 또 한쪽만.
- 이제 「사진이 배경」을 고르면 **영상도 그 모양**: 현장 사진이 화면을 꽉 채우고,
  지도는 오른쪽 위 한 뼘에서 트럭이 달린다. 글자는 전부 흰색(위아래 어둡게).
- 💡 **배경 사진이 도착할 때마다 바뀐다** — 한 달 시공 사례가 차례로 지나간다.
  작은 지도는 **따라가지 않는다**(한 뼘 창에서 카메라까지 움직이면 멀미한다).
- 사진이 하나도 없으면 조용히 지도 갈래로 내려간다 — 빈 검은 화면을 안 내놓는다.
- 라벨 「이미지 스타일」 → 「스타일」(이제 영상도 따른다). `remember` 키에 `shape` 추가
  (안 넣으면 스타일을 바꿔도 영상이 안 바뀐다).
- ⚠️ **아무 폰에서도 못 봤다** — 업무폰은 Play 설치본이라 손대지 않고, 테스트폰은 안 꽂혀 있다.
  게다가 업무폰 9월 현장은 대표 사진이 0/5 라 설치해도 이 갈래가 안 뜬다. **눈 확인 필요.**
- ✅ 빌드 EXIT=0 · 단위 575개 실패 0.

## 2026-09-26 11:30 · android
📐 **「2026년 9월」과 큰 숫자가 겹치던 것.** (사장님 "글자가 겹친느낌임")
- 띄우기를 **숫자 높이 기준**으로만 잡아놨는데 「약」 같은 한글은 숫자보다 위로 더 솟는다
  (104px 기준 숫자 75 · 「약」 84). 실측 **7.6px 겹침** — 숫자만일 때도 1.4px 밖에 안 남았다.
- 이 블록이 **다섯 군데**에 각자(104f·96f·96f·h*0.062) → `RecordShot.bigTop` 한 곳으로.
  글자 크기에 비례하니 `fit` 으로 줄어도 같이 준다. ✅ 테스트폰 실측 확인.

📸 **사진이 주인공인 영상 — 고를 자리가 없었다.**
- 스타일 칩이 **영상 모드에선 통째로 숨어 있었다**("영상엔 안 먹히니"라는 어제까지의 사실 때문).
  → 영상 모드에 **「지도가 주인공」/「현장 사진이 주인공」** 두 칸을 따로 뒀다.
  다섯 칩을 그대로 쓰면 넷이 같은 영상이라 "고르라고 해놓고 안 먹히는" 꼴이 된다.
🌘 **사진 위 흰 글씨가 묻히던 것** — 밝은 사진에서 「16 집」이 거의 안 보였다.
  → 글자에 연한 그림자(`RecordShot.onPhoto`). 그림·영상 둘 다.
- ✅ 빌드 EXIT=0 · 단위 575개 실패 0 · 테스트폰 실측(사진 갈래 렌더·가독성·겹침).

## 2026-09-26 13:40 · android
🏷️ **업종을 골라도 저장이 안 되던 것.** (사장님 "시공ai 눌러서 업종고르는데 저장도안되는느낌임")
- **재현함** — 칩을 누르면 ★ 가 붙는데 뒤로 나갔다 오면 사라진다.
  「저장」 버튼이 **업종 40여 개를 다 지난 맨 아래**에 있어서, 고르고 그냥 나가면 날아갔다.
- 하나만 고르는 화면(라디오)에 저장 버튼이 있을 이유가 없다 → **고르는 순간 저장 + 토스트**.
  자동완성·직접입력·지우기도 같은 길로. 아래 버튼은 「저장」→「완료」(이미 저장돼 있다).
- ✅ 테스트폰 확인: 줄눈 누름 → 뒤로 → **상단 알약이 「시공 AI」→「줄눈 AI」** 로 바뀜 →
  앱 완전 종료 후 다시 켜도 ★ 줄눈 유지.
- ✅ 빌드 EXIT=0.

## 2026-09-26 13:20 · android (⚠️ server 영역 — 사장님 직접 지시)
🚫 **사이트에서 APK 직접 내려받기 제거 — 무조건 Play 스토어.** (사장님 "사이트에 apk 다운 안되게해야지")
- 라이브 `/install` 이 **APK 가 주인공인 옛 3단계 페이지**였다(레포는 이미 Play 우선이었지만
  「직접 설치(APK)」 접이식이 남아 있었다) → 레포·라이브 **둘 다** APK 갈래 통째 제거.
- `/download/shigongmagne.apk` 는 **죽이지 않고 Play 로 302** — 예전 안내 문자에 실려 나간 링크다.
- 앱 자체 업데이트는 이미 `market://` → play.google.com 으로 보내므로 **영향 없음**
  (버전 확인만 `/api/download/version` 사용 — 그대로 둠).
- 라이브 수술은 **최소**로: `static/install.html` 만 교체 + `main.py` 의 그 함수 앞에 리다이렉트 삽입
  (라이브는 origin 보다 한참 뒤라 통짜 배포 금물). 백업 `main.py.bak-20260926-131523`.
- ✅ 확인: `/install` 200 · APK 흔적 0 · Play 버튼 1개 · `/download/...` 302→Play ·
  `smoke.sh` **10곳 전부 정상**.
- 📌 코워크에게: 라이브 main.py 에 `_download_apk_legacy` 로 옛 함수가 남아 있다(호출자 없음).
  origin 의 같은 수정과 내용은 같으나 라이브가 뒤처져 있으니 다음 배포 때 충돌 주의.

## 2026-09-26 14:10 · android
⌨️ **키보드 위에 빈 칸이 남던 것.** (사장님 "키패드 위에 공백으로 남는현상 체크좀")
- 우리 창은 manifest 의 `adjustResize` 라 **시스템이 먼저 창을 줄여준다**(Android 14 까지).
  그 위에 `ime` 를 또 더하면 **키보드 높이만큼 빈 칸**이 생긴다 —
  삼성 일부 기기는 창이 줄었는데도 ime 를 통째로 알려준다.
- 🔒 이 계산이 **18 군데에 각자** 있었다(채팅 6 · 박람회 3 · 수첩 2 · 로그인 2 · 일정 · 정산 · 가입 · 팀 · 가격추출)
  → `presentation/util/SystemBars.kt` 의 **`keyboardClearance`** 한 곳으로.
  14 이하는 `navigationBars.exclude(ime)`, Android 15(API 35)+ 는 `ime.union(navigationBars)`
  (15+ 는 adjustResize 를 무시해 창이 안 줄어든다).
- 💡 **잘 도는 기기에선 값이 그대로**다(ime 를 0 으로 주므로) — 이상하게 주는 기기만 고쳐진다.
- ⚠️ **S9+ 에서는 원래 빈 칸이 없어 재현이 안 됐다.** 채팅 입력칸으로 **회귀 없음**만 확인했다.
  사장님 그 폰(갤노트/울트라 계열)에서 눈으로 봐야 한다.
- ✅ 빌드 EXIT=0 · 테스트폰 회귀 확인.

## 2026-09-26 18:20 · android
✍️ **숫자 위에 한 줄 + 매출 고지.** (사장님 프로토 artifact/TsopqhWzbq8 에서 고르심)
- 「올해도, 현장에서.」 · 「땀 흘린 만큼, 쌓인 매출.」 처럼 **기분 한 줄**이 달 라벨 밑에 붙는다.
  전엔 「올해 다녀온 집」 같은 설명 라벨뿐이라 SNS 에 올릴 그림치고 밋밋했다.
- 💰 돈을 고르면 숫자 밑에 **「기록한 매출 기준 · 순이익과 달라요」** — "그래서 얼마 남았냐"를 미리 막는다.
- 🔁 한 줄은 **달마다 바뀐다**(숫자마다 2개씩). 무작위가 아니라 **달로** 고르니
  미리보기와 저장본이 늘 같다. 이번 달(9월)엔 사장님이 고르신 문구가 나오게 맞췄다.
- 사실을 주장하지 않는 **기분 한 줄**만 쓴다 — 없는 얘기를 지어내면 그게 거짓말이 된다.
- 「집」은 그대로(「곳 시공」은 정확하지만 딱딱하다 — 사장님도 이의 없음).
- 그림 3갈래 + 영상 2갈래 **다섯 군데 전부**. 지도 위 알약은 글이 는 만큼 **길어진다**.
- ✅ 빌드 EXIT=0 · 단위 575개 실패 0 · 테스트폰 실측(올해 집 / 번 돈 둘 다).

## 2026-09-26 18:35 · android
🔄 **한 줄 문안을 여섯 벌로 — 반년에 한 바퀴.** (사장님 "6세트 준비하면 6달에 한번씩 반복되게")
- 자랑할 숫자 여섯 가지 × **각 여섯 줄 = 36줄**. 3월·9월이 같은 줄을 쓴다(반년 주기).
- 고르는 법: `Math.floorMod(달 - 9, 6)` — **9월이 첫 줄**. 사장님이 보고 고르신 게 그 줄이다.
- 사실을 주장하는 말은 안 넣었다(「새벽부터」 같은 건 없는 얘기가 될 수 있다) — 기분만 얹는다.
- 🧪 **한 달에 한 번만 눈에 띄는 글이라 폰으로는 확인이 안 된다** → 단위 테스트 6개:
  숫자마다 여섯 줄 다 있나 · 같은 줄 중복 없나 · 20자 넘는 줄 없나(그림에서 작아진다) ·
  9월에 고르신 줄 나오나 · 여섯 달 내내 안 겹치고 반년 뒤 제자리로 오나 · 모르는 숫자면 안 그리나.
- ✅ 빌드 EXIT=0 · 단위 **581개** 실패 0 · 테스트폰 확인.

## 2026-09-26 19:40 · android
↕️ **가격표 항목을 끌어서 순서 바꾸기.** (사장님 "시공 항목을 끌어서 순서를 올리고 싶어")
- 순서 칸(`displayOrder`)은 처음부터 있었는데 **바꿀 방법이 없었다** — 만든 순서 그대로 굳어서
  접수서에 「바닥 → 벽전체 → 폴리우레아 제거」 같은 원하는 차례로 못 넣었다.
- 줄 왼쪽 **손잡이(≡)** 를 끌면 그 자리에서 순서가 바뀌고, 손을 떼면 저장된다.
  한 번 맞춰두면 **앞으로 보내는 접수서·견적서가 전부** 그 순서로 나간다(이미 displayOrder 로 정렬 중).
- 🐞 **두 번 헛짚었다** — 줄마다 `pointerInput` 을 달았더니 **자리가 바뀌는 순간 추적이 죽었다**
  (줄이 움직이면 그 손가락 추적기도 떼였다 붙는다 · `key` 를 달아도 마찬가지).
  → **안 움직이는 묶음 카드**가 손가락을 잡고, 왼쪽 48dp 에서 시작했는지만 좌표로 가린다.
  덕분에 **목록 스크롤·줄 누르기는 그대로**(실측 확인).
- ✅ 빌드 EXIT=0 · 단위 581개 실패 0 · 테스트폰: 끌어서 1↔2 바꾸고 **앱 껐다 켜도 유지**,
  손잡이 밖을 쓸면 여전히 스크롤됨.

## 2026-09-26 19:55 · android
🤝 **협업 현장도 「내 기록」에.** (사장님 "협업일정도 똑같이 올라가져야해. 움직인건 맞으니까")
- 남의 현장에 불려 간 날도 **내가 다녀온 현장** — 지도·동네·나간 날·거리·현장 수에 들어간다.
- 💰 **일당도 매출에 합친다**(사장님이 고르심: "내가 번 돈은 맞으니까").
- ⚠️ **진짜 간 것만** — 「도착」·「완료」 표시가 있는 것. 수락만 하고 안 간 건 안 센다.
- 목록엔 번호 대신 🤝 와 **「협업 · 상호」** 딱지. 현장 번호는 **내 현장**의 차례라 안 붙인다.
- ⚠️ 협업 현장은 **서버에 있다**(앱 DB 아님) → 화면 열 때 한 번 불러온다.
  못 불러오면 조용히 내 것만 보여준다(화면은 그대로 뜬다).
- ✅ 빌드 EXIT=0 · 단위 581개 실패 0.
- 📌 아직: 테스트폰에 협업 자료가 없어 **눈으로 못 봄**. 사장님 폰에서 확인 필요.

## 2026-09-26 20:05 · android
🔧 **A/S 줄이 늘 한 자리를 먹던 것.** (사장님 "실제로 as가 많지않은데 메인으로 빼둔 느낌")
- A/S 는 **가끔** 있는 일인데 「시공 예약」 바로 밑에 **늘** 「아직 없음 · 눌러서 잡기」가 떠 있었다
  — 정보가 아니라 **빈 줄**이다.
- → **잡혀 있을 때만** 줄로 보여준다(그땐 중요하다). 없을 땐 카드 맨 아래 되돌리기 줄에
  **작은 「A/S 잡기」** 하나로 물러난다 — 길은 안 막는다.
- 시공 예약이 있을 때만 권한다 — A/S 는 시공 **뒤**의 일이다.
- ⚠️ **폰 확인 못 함** — 테스트폰이 또 빠졌다. 빌드만 확인.
- ✅ 빌드 EXIT=0.

## 2026-09-26 20:40 · android
📬 **상담함 탭을 누르면 무조건 전체.** (사장님 "무조건 전체메세지가 보여야해")
- 칩이 켜진 채로 남으면 다음에 열 때 **텅 빈 목록**(「여기 아무도 없어요」) — 문자가 다 날아간 줄 안다.
  [[reference_appscoped_tick_launchedeffect_refires]] 와 같은 부류. **앱 새로 켤 때만** 되돌리고 있었다.
- → `AppContainer.inboxResetFilter` 신호를 **탭 누를 때마다** 쏘고, 상담함이 받아 [전체]로 되돌린다.
  채팅 갔다 뒤로가기 같은 같은-화면 이동은 안 건드린다(훑던 칩이 풀리면 성가시다).
- ✅ 업무폰(잠금 해제 후 읽기만) 확인: 지금은 정상 — 그때는 칩이 남아 있던 순간이었다.
- ✅ 빌드 EXIT=0 · 단위 581개 실패 0.

## 2026-09-26 20:45 · android (조사)
🤝 **「같이 할 사장님」 리디자인 — 프로토 artifact/TRtPB7rkJ8 사장님 전체 동의.**
- 🔎 **「많이 같이 한 사람 맨 앞」은 안 들어가 있었다** — `NotebookContactDao.observeByKind` 에
  **ORDER BY 가 없다.** 등록한 순서대로 나온다.
- 할 일: ①알약→검색+줄 목록 ②「함께 N번」 정렬(협업 기준 — 사장님 확정) ③이름 옆 (함께 N번 · 마지막)
  ④등록에서 **일당 칸 삭제** ⑤「연락처에서 부르기」 = **카테고리 「일당」 분류된 사람만**
  ⑥줄 꾹 눌러 **이름 바꾸기**.
- ⚠️ 「며칠째」는 서버가 **첫 현장 날짜를 안 준다** — 횟수·마지막 날짜만 있다.

## 2026-09-26 21:20 · android
🤝 **「같이 할 사장님」 리디자인 — 프로토(artifact/TRtPB7rkJ8) 그대로.**
- 알약 → **검색 + 줄 목록**. 알약은 열 명만 넘어도 줄이 무너진다(5명 이상일 때만 검색창).
- 📊 **많이 부른 사람이 맨 위** — 전엔 `observeByKind` 에 **ORDER BY 가 없어** 등록 순서였다.
  서버 협업 집계(`partners`)의 `count` 로 줄을 세운다. ⚠️ 협업으로 부른 것만 세어진다(사장님 확인).
- 이름 옆 **「함께 12번 · 9/16」**. 없으면 「아직 같이 한 적 없어요」.
- ✏️ **꾹 누르면 이름 바꾸기 / 빼기** — 전엔 빼기만 됐다.
- 💰 등록에서 **일당 칸 삭제** — 현장마다 그때 적으니 또 물을 이유가 없다.
- 🏷️ **「＋ 내 연락처에서 부르기 · 「일당」 N명」** — 카테고리를 「일당」으로 분류해둔 **고객**을
  바로 명부에 넣는다. 전엔 전화번호를 다시 찾아 쳐야 했다
  ("카테고리의 의미가 무색해지네" — 분류해둔 보람이 여기서 난다).
  ⚠️ 카테고리는 **고객**에 붙는다(`CustomerEntity.categoryId`). 이름에 「일당」이 든 카테고리로 찾는다.
- ⚠️ **폰 확인 못 함** — 업무폰만 연결돼 있어 설치를 안 한다(Play 설치본).
- ✅ 빌드 EXIT=0 · 단위 581개 실패 0 · 손으로 적은 값 2826 → **2824**(줄었다).
- 📌 「며칠째 호흡」은 서버가 **첫 현장 날짜**를 안 줘서 못 넣었다 — 횟수·마지막 날짜만.

## 2026-09-26 21:35 · android
📍 **「함께 12번 · 마지막 강동」** — 날짜 대신 **어디였는지**. (사장님 "며칠째 호흡보다는 최근 함께한 현장")
- 날짜(9/16)보다 **동네 이름**이 훨씬 잘 떠오른다. 주소가 있으면 동네(구), 없으면 현장 이름.
- 집계(`partners`)엔 장소가 없어서 협업 현장 목록(`withMe`)에서 사장님별 **제일 최근 것**을 뽑는다.
- ✅ 빌드 EXIT=0 · 손으로 적은 값 안 늘어남.
- 📌 시트 **아래쪽**(출근 시간·메모·일당·보내기) 정리 제안 = artifact/F1ERiGmCxX (사장님 확인 대기).

## 2026-09-26 21:55 · android
📤 **부르기 보내는 자리 정리.** (프로토 artifact/F1ERiGmCxX · 사장님 "오케이 좋아 고고")
- 「고른 사장님」 섹션 **삭제** — 위 목록에 이미 체크가 보인다. 같은 말을 두 번 하지 않는다.
- 💰 **「보낼 일당」이 제 이름표**를 갖는다 — 제일 중요한 숫자가 긴 설명 밑에 묻혀 있었다.
  사람마다 한 줄. 설명도 사람마다 반복하던 걸 **아래 한 줄**로.
- 📤 **버튼이 이름을 말한다** — 「1명에게 보내기」 → **「하픽에게 보내기」** / 「하픽 외 1명에게 보내기」.
- ⚖️ 「수락해야 성립해요 · 정산엔 안 잡혀요」는 **버튼 밑**으로.
- ⏰ 출근 시간 = 자주 쓰는 넷 + **「다른 시간」**. 일곱 개가 두 줄을 먹었다.
- 「(선택)」 떼기 — 안 써도 보내진다.
- ✅ 빌드 EXIT=0 · 단위 581개 실패 0 · 손으로 적은 값 2824 → **2820**.
- ⚠️ **폰 확인 못 함** — 업무폰만 연결돼 있다.

## 2026-09-26 22:10 · android
✅ **테스트폰에서 「같이 할 사장님」 전부 눈으로 확인.**
- 목록: 검색창 + 줄(첫 글자 아바타 · 이름 · 「아직 같이 한 적 없어요」) — 새 모양으로 뜬다.
- 🏷️ **「＋ 내 연락처에서 부르기 · 「일당」 1명」** — 카테고리 찾기가 **실제로 동작**한다(1명 잡힘).
- ⏰ 출근 시간 = 오전 7/8/9/10시 + 「다른 시간」.
- 💰 「보낼 일당」 아래 **디테일라인 · 25 · 만원** 한 줄. 「이 현장에서 보낼 일당이에요.」
- 📤 버튼 **「디테일라인에게 보내기」**, 그 밑에 「수락해야 성립해요 · 정산엔 안 잡혀요」.
- 🔧 남아 있던 「전달 메모 **(선택)**」도 뗐다.
- ✅ 빌드 EXIT=0.

## 2026-09-26 22:40 · android
🏷️ **사람을 「일당」이라 부르지 않는다 → 「협업 사장님」.** (사장님 "일당의 어감이 하루살이 같아서")
- 화면 글 40여 곳이 **두 뜻**으로 갈렸다. **사람**만 바꾸고 **돈은 그대로** 뒀다
  (「그날 일당」·「받은 일당」·「일당 지급계좌」 — 돈 이름으로는 현장 말이고 낮잡는 말이 아니다. 사장님 확인).
- 바꾼 곳: 수첩 「일당사장」·설명문 · 팀 탭 · 카테고리 예시 2곳 · 「＋ 내 연락처에서 부르기 · 「협업 사장」 N명」.
- 🔑 카테고리 찾기는 **「일당」·「협업」 둘 다** 알아듣는다 — 한쪽만 보면 사장님이 그동안 분류해둔 게 안 잡힌다.
- 🛑 **지뢰 하나 제거**: `CallFollowCrmApplication` 에 2026-06-07 자 정리 코드가 있었다 —
  **「일당」으로 분류된 고객을 전부 미분류로 되돌린다.** 그땐 "'일당'은 수첩 개념" 이라 봤는데
  **지금은 정반대**(그 분류로 부른다). 이미 실행된 폰은 괜찮지만 **새로 깔면 분류가 통째로 날아간다**
  → 아예 안 돌게 막았다.
- ✅ 빌드 EXIT=0 · 단위 581개 실패 0 · 금지어 감시 통과.

## 2026-09-26 23:10 · android
🔎 **번호를 묻지 않는다 — 앱이 아는 사람 중에서 이름으로.** (프로토 artifact/ECAABRr2)
- 「직접 등록」이 번호를 물었는데 **같이 일하는 사장님 번호를 외우는 사람은 없다.**
  앱은 그 번호를 이미 안다(문자를 주고받았으니) → **이름으로 고른다.**
- 목록: ① **협업 사장으로 분류해둔 분** ② **문자를 주고받은 분**(최근 순) · 검색창 · 맨 아래 「찾는 사람이 없나요? 직접 등록」.
- 🏷️ 고르면 명부에 들어가고 **「협업 사장」 카테고리로 분류까지** 된다(없으면 만든다).
  안내문으로 유도하지 않는다 — **부르면 쌓인다.** 다음부턴 맨 위에 뜬다.
- 🐞 **처음엔 1522·1544·114 가 줄줄이 올라왔다** — 「받기만 한」 번호까지 넣어서.
  `hasOwnerReply`(내가 답장한 적 있나)를 거니 **광고·ARS 가 한 줄로 사라졌다.**
  (앞서 사장님께 "답장한 사람만"이라 말해놓고 정작 코드에 안 넣었던 것)
