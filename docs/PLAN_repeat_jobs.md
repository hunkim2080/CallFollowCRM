...
## Stage A — 코드 완료 (2026-09-11) · ⚠️ 실기 검증 대기

> 계기: 사장님 — **"인테리어 업체는 한 번호에 현장이 여러 개다. 일주일 뒤 '여기도 해주세요' 하면 새 일정을 잡는데,
> 우리 시스템은 한 번호에 일정 하나만 되는 것 같다"** → Phase 1 한계 ③(완료 전 두 번째 예약 시 덮어씀)이 실사용에서 걸림.

**달성**: 같은 고객에 **여러 날짜**를 잡아도 서로 안 덮어쓰고, 달력·일정 목록에 **건별로** 뜬다.

### 실제 구현 (DB v48 → v49)
- **`jobs` = 일정 SoT 로 승격.** 마이그레이션 `MIGRATION_48_49` 가 각 고객의 현재 예정 시공을 jobs 로 **COPY**
  (`INSERT ... SELECT ... WHERE NOT EXISTS` — 중복 방지). **customers 컬럼은 그대로 둠 = 무손실**.
- **CustomerEntity 시공필드 = "대표 건" 미러** (`JobRepository.recomputeMirror`): 오늘 이후 가장 가까운 건,
  없으면 가장 최근 건. → 홈 히어로·챗·통화전 카드·접수서·미러·브리핑 등 **CustomerEntity 리더 ~10곳 무변경**.
- **등록 = INSERT** (`ScheduleAddViewModel.submit` → `JobRepository.addJob`). `updateScheduledWorkDate` 덮어쓰기 제거.
  같은 고객·같은 날은 중복 생성 안 함(연타 가드).
- **달력/일정 = jobs 읽음** (`ScheduleViewModel.state` = combine(jobs, customers) → **건별 CustomerEntity 복사본**).
  새 타입 도입 없이 기존 UI(카드·달력 막대·배정 시트)가 그대로 건별 동작.
- **키 충돌 해소**: 목록 key·달력 lane 을 `c.id` → **`laneKeyOf(c)`=(고객,시공일)** 로. (한 고객 2건이면 key 중복으로 목록이 깨졌음)
- **건별 빼기/되돌리기**: 밀어서 삭제 = 그 **건만** 시공일 비움(`unscheduleJob`), 되돌리기는 그 건 복구(`rescheduleJob`).
- **"지난 시공 N건" 보호**: 예정 건이 jobs 에 들어오면서 섞이지 않게 `observeCompletedByCustomer`(완료건만)로 교체.

### 의도적으로 **안** 건드린 것 (Stage B 로)
- **돈**(총액·계약금·잔금·미수)은 여전히 **고객 단위**. 미러도 일정 필드만 복사 → 정산·미수금 계산 그대로.
- 알람(D-1·잔금)도 대표 건 기준 그대로. → **두 번째 날짜의 D-1 알람은 아직 안 울림** (Stage B).

### 검증 상태
- ✅ `compileDebugKotlin` 통과 (Room 코드생성·마이그레이션 등록 포함)
- ✅ **단위테스트 242건 전부 통과** — 신규 `JobRepositoryStageATest` 5건:
  두 날짜 공존 / 같은 날 중복 가드 / 대표 건 선택(미래 없으면 최근) / **미러가 돈 안 건드림(정산 보호)** / 건별 빼기
- 🔧 **겸사 수리**: 오래돼 깨져 있던 테스트 3건을 현재 규칙에 맞게 교정
  (`AutoCategoryClassifier` 2건 = 2026-08-31 `isWorkDone` 규칙 반영, `JobRepositoryArchive` 1건 = 2026-07-30 미수 가드 반영)
  + 미수 가드 회귀 테스트 1건 추가. → 스위트가 다시 초록 = 앞으로 Stage B 돈 회귀를 잡아준다.
- ⚠️ **실기 미검증**: v48→v49 마이그레이션 **실제 폰 오픈 크래시 확인 안 됨**(작업 시점 폰 미연결).
  스키마 변경 없이 행 복사만 하는 additive 마이그레이션이라 위험은 낮지만, **내부테스트 설치 후 일정 화면 진입까지 확인 필요**.

### Stage A 이후 남은 것
- **Stage B**: 정산 건별(순수함수 단위테스트 필수) + 알람 건별(두 번째 날짜도 D-1 울리게). ← 다음 차례
- **Stage C**: 고객상세 건 목록 UI + 멀티데이 lane 폴리시.

---

## Stage C UI 확정 (2026-09-17 사장님 결정) — **B안 · 탭형**

> 사장님: "B안이 괜찮다."  (프로토 artifact/KKMyncpHy1Ni7GK6dygMRY — A안 목록형 / B안 탭형 비교)

고객 상세 [일정·정산] 안에서 **건을 탭으로** 늘어놓는다. 사장님이 기억하던 "크롬 새 탭" 그림.
- 탭: `[2차 · D-28 / 10-15(화)]` `[1차 · 완료 / 9-2(수)]` `[＋ 새 시공]`
- 한 번에 **한 건만** 보여준다(화면이 짧다). 선택된 건의 일정·금액·메모·사진이 아래 패널에.
- `＋` 탭 = 이 고객으로 새 시공 잡기. 일정 탭으로 안 나가도 된다.
- 건이 많아지면 탭이 가로 스크롤.
- (안 고른 A안 = 건 카드를 위아래로 쌓고 눌러서 펼치는 목록형. 추천은 A였으나 사장님이 B 선택.)

### 순서 — **알람·정산(Stage B 잔여) 먼저, 그 다음 B안 화면**
화면부터 만들면 두 번째 시공을 쉽게 잡게 되는데, 지금은
  · 두 번째 날짜의 **D-1 알람이 안 울리고**
  · **미수금이 대표 건 기준**이라 틀린다.
= 화면이 **버그 노출을 늘린다**. 그래서 순서를 뒤집는다. (2026-09-17)

### Stage B·C 코드 완료 (2026-09-17) · ⚠️ 실기 검증 대기
- **Stage B**: D-1·잔금 알람 건별(`JobReminderCalc`, 테스트 11) + `SettlementCalc.rowOf(JobEntity)`(테스트 6).
  dedup 키 `d1j:<건>:<날>` / `settlej:<건>`. 옛 D-1 키는 업데이트 첫날만 같이 본다(중복 방지). commit 1b94de82
- **Stage C**: 고객 상세 건 탭(B안). '지난 시공 N건' 카드 → 탭으로 흡수. 지난 건은 읽기 전용. commit 3b5ddb81
- 남은 것: 정산 **화면**의 건별 합산(지금은 대표 건 기준 목록), 멀티데이 lane 폴리시, "단골" 자동 전환 규칙.
