...
### D. 본폰 뷰어 "미수금 N건" 탭 → 어디서 얼마 못 받았나 목록 (사장님 요청)
- 사장님: 미수금 4건이 있는데 탭해도 아무 변동 없음. "어느 곳에서 돈을 못 받았는지" 보이게.
- **앱이 이제 `money.receivables` 배열을 스냅샷에 실어 보냄**(commit 아래): `[{name, amount(원, 미수잔액), address?, phone?(하이픈), overdueDays?}]`, 큰 금액순.
- 요청: 뷰어의 "미수금 N건"(또는 못 받은 돈 카드)을 **탭하면 receivables 목록**을 펼쳐 표시 — 현장명 · 미수 금액(만원 표기) · (있으면) 경과일/주소. 탭 전엔 합계만.
- receivables 없거나 0건이면 탭 비활성(그대로 합계만).

### E. 본폰 뷰어 현장 카드 — 주소·전화 각각 한 셀(줄)로 (사장님 요청)
- 사장님: 주소·번호가 짧으면 한 줄에 같이 붙어 나옴 → **주소는 주소대로 한 셀, 전화는 전화대로 한 셀**로 각각 차지하게(짧아도). UI 깔끔.
- 요청: 현장 카드에서 주소 행과 전화 행을 **항상 별도 라인**으로(길이와 무관하게 한 셀씩). 뷰어 레이아웃만. 앱 무관.
