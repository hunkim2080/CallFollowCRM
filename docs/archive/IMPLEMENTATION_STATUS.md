...
>
> - 갱신: 2026-06-02 (프로토 1:1 화면 이식 진행 — android Claude)
> - 상태값: ✅ 완료 / 🔶 검수중 / 🔷 진행중 / ⬜ 시작전(프로토에만)
> - **항목 상태가 바뀌면 이 표 + status-board.html 둘 다 갱신**할 것.

---

## 🎨 프로토 1:1 화면 이식 (전면 리뉴얼) — 2026-06-02 진행 중

> 기능(아래 영역별 표)과 **별개 축**: 프로토타입 디자인(글·배치·색·흐름)을 화면별로 100% 그대로 옮기는 작업.
> 사장님 룰 = CLAUDE.md §0 (프로토 = 실전 스펙).
>
> 🎯 **요소(섹션·배너·카피·디자인 값) 단위 정밀 추적은 `docs/ONEONE_STATUS.md`(SoT) + `design-preview/oneone-board.html`(사장님 보기용).**
> 아래 표는 화면 단위 요약, 정밀판은 그쪽. 프로토 바뀌면 oneone 쪽이 "변경→반영" 체크리스트로 동작.

| 화면 | 1:1 상태 | 커밋 | 비고 / 의도적 보류 |
|---|---|---|---|
| 로그인 | ✅ | (이전 세션) | 프로토 .login 1:1 |
| 온보딩 (7장) | ✅ | b6908da | OB_SLIDES verbatim + 업종 직접입력 + 홍보 아이콘줄 |
| 홈 상담함 | ✅ | d4fbdfa~a42d09d | 오늘신규→오늘시공→대기→최근 구조 1:1. 필터칩·미수금 제거 |
| 정산 | ✅ | 9fa0c68 | 월매출 다크카드+현금흐름 인라인+srow. 과거달은 받은돈만 갱신 |
| 통계 | ✅ | e490c5b | 히어로+2x2+추이+시공종류. **전국 비교 2곳 = "모이는 중"**(서버 집계 대기) |
| 일정 | ✅ | 03a585d | cal-card+cal-hint+날짜카드(주소·입금·D-day·N일차·정산링크). **팀원 배정=99k 보류** |
| 더보기 — 막내 카드 | ✅ | aab8a73 | agent-card(레벨·말투%·stats) 1:1, 실제 카운트 |
| 더보기 — 메뉴 재배치 | 🔷 | — | 막내 카드 먼저 완료. 섹션 정렬·하위화면 정리 다음 단계 |
| 채팅 | ⬜ | — | 1:1 점검 아직 (기능은 동작) |
| 시공접수서 | ⬜ | — | 서버 §19 완료, 앱 폼 화면 1:1 대기 |

**이번 세션 핵심:** 정산·통계·일정 화면을 "비슷한 변형"에서 프로토 **그대로**로 다시 맞춤 + 더보기 막내 카드.
전국 데이터·팀 기능 의존 부분은 가짜 숫자 대신 "모이는 중"/보류로 정직하게 처리(사장님 결정).
