...
## ④ [P1] 제출 완료된 접수서 링크 = "확인용 영수증" 페이지

**증상:** 고객이 제출을 마친 `/q/{token}` 링크를 **한 번 더 열면** 지금은 "이미 제출된 접수서입니다" 만 뜸.
**요청(사장님):** 그 링크를 다시 눌러도 **"접수가 정상적으로 완료되었습니다"** + **제출된 내용을 읽기전용으로** 보여주기 — 시공일 · 견적 내용 · 주소 · 특이사항(owner_memo/고객 memo). 고객·사장님 둘 다 나중에 다시 눌러 확인하고 싶을 수 있음.

**서버:** `GET /q/{token}` (또는 `/intake/{token}`) 에서 `submitted_at_ms IS NOT NULL` 이면 "이미 제출" 메시지 대신 **완료 영수증 뷰** 렌더:
- 상단: ✅ "접수가 정상적으로 완료되었습니다" (프로토 톤/토스블루)
- 본문(읽기전용): 시공일(work_year/month/day 또는 confirmed_date_iso), 견적 항목·합계, 계약금/잔금, 고객이 입력한 주소·동/호, 특이사항(owner_memo), 고객 memo.
- ①(날짜 -1) 고친 뒤라 시공일이 정확히 표시됨.
- 편집·재제출 버튼 없음(확인 전용). 만료돼도 영수증은 보이게(선택).

---

## 우선순위
①(날짜) = P0, 한 함수 수정으로 즉효. ②③④ = P1, 사장님 편의.
- ③ 메모: 앱은 이미 `ownerMemo` 전송 배선 완료(commit 7c455b2) → 서버 `owner_memo` 컬럼+폼표시만 하면 바로 뜸.
- 서버 배포 후 SYNC 에 남겨주면 android 가 ②(링크 갱신 안내 토스트) 앱측 마무리.
