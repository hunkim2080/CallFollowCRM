...
| 셀프 등록 폼 | 🔵 | renderAddSchedule | ⚠️ **프로토와 1:1 아님 (전에 ✅ 잘못 표기 — 2026-06-02 정정).** 앱=단순화 자체 폼. 프로토엔 있는데 앱엔 없음: [내고객/거래처 토글] · [통화·문자 고객 불러오기] · [인라인 미니캘린더(앱=날짜팝업)] · [주소 검색 탭(앱=자유입력, 주소검색 기능 자체 없음)] · [총금액 만원 단위(앱=원)] · [계약금 "받았어요" 체크→펼침(앱=항상 보이는 입력)]. 시간칩·기간칩만 일치. 거래처=수첩(NotebookContact) 연결 가능, 고객 불러오기=통화/문자 기록 가능, 주소검색=Daum 우편번호 새기능 필요 |
