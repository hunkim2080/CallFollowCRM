...
| 자동문자 | 🔵 구조 다름 | 프로토(부재중·시공D-1·도착5km·정기 4블록)↔앱(통화종료후 동작·빠른액션·자동응답). D-1·도착 카드 없음 |
| 사업자정보 | 🟡 고침(폰확인) | **직인 문구(seal) 필드 추가**(a27dcbc) + 제목 '견적서·사업자 정보'·라벨·placeholder·토스트 verbatim. 유효기간 suffix UI만 남음 |
| 내 말투 학습 | 🔵 거의 미구현 | 프로토 renderTone(학습% hero·ON토글·말투특징칩·before/after·학습소스 3종·가르치기 버튼) 전부 없음, 앱=상태카드 1장 |
| 견적서(직인)·3모드 | ⬜ 미구현 | 텍스트 견적만. 직인·이미지 발송·[문자견적/접수서/견적서] 토글 없음 |
| 시공접수서(openQuote) | ⬜ 미구현 | 서버 §19 완료, 앱 화면 없음 |
| 팀 관리/팀원(99k) | ⬜ 의도적 보류 | 요금제 |

**수정 우선순위:** ①통계 그래프 버그(실제 깨짐) ②정산 현금흐름 순이익 위치 ③사업자정보 직인 필드+견적서(직인) ④재료화면(가격표·수첩·정기문자·자동문자·말투) 하나씩 ⑤홈/채팅/일정/고객 미세 DIFF.
...
| 요소 | 상태 | 메모 |
|---|---|---|
| 견적 작성 다이얼로그 | 🟡 | EstimateDialog 동작(텍스트 본문 합성). ⚠️ 프로토 견적 작성기의 **[문자 견적][시공접수서][견적서] 3모드 세그먼트 없음** — 앱은 문자 견적만 |
| **견적서(직인 정식 문서)** | ⬜ | ⚠️ **앱 미구현 (2026-06-02 확인).** 프로토 `openQuoteDoc`/`.qd-seal`/`sendQuoteDoc` = 직인 찍힌 견적서 이미지를 MMS 로 발송. 앱은 견적을 **텍스트(ACTION_SEND text/plain)** 로만 보냄 — 이미지 렌더·직인·이미지 첨부 전부 없음. 사업자정보에 **직인 문구(seal) 필드도 없음**. 필요: ①seal 필드 ②견적서 Composable(.qd-* 1:1) ③Compose→Bitmap ④MMS 이미지 첨부 ⑤모드 토글 |
| 시공접수서(openQuote) | ⬜ | 서버 §19 완료, 앱 [링크 보내기]+polling+카드 미구현 |
| 사업자정보 등록 | 🟡 | BusinessInfoScreen — 상호·대표·사업자번호·주소·연락처·유효기간. ⚠️ 프로토 **직인 문구(seal) 필드 빠짐** |
...
7. ✅ ~~고객 관리 목록(renderCustomers)~~ — 필터칩+상태태그+recent-row 1:1(efb5c26). 폰 확인.
8. ✅ ~~검색(doSearch)~~ — recent-row 아바타+문구 verbatim(e37b860). 폰 확인.
9. **시공접수서(openQuote) 앱 화면 — 유일한 "미구현" 큰 덩어리** (서버 §19 완료). [링크 보내기]+polling+카드. 서버 인터페이스 의존 → 시작 전 사장님/서버 Claude 확인.
10. 견적 작성 다이얼로그 / 가격표·리포트·템플릿·수첩 개별 1:1 미세 점검(기능은 완료).
