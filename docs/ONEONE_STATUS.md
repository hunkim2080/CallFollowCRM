...
| **통계 문의추이 그래프** | 🔴 **버그(깨짐)** | 막대 높이 계산(고정 92dp)이 컨테이너(130dp) 초과 → 요일 라벨 클립/막대 겹침. GBar 비례높이로 재작성 필요 |
| 정산 현금흐름 | 🔵 다름 | **순이익 표 위치 역전**(앱=달력 위 / 프로토=맨 아래 cc-foot), cc-head/cc-add/cc-unit/범례 괄호/소제목 누락, 직접기록 폼 분류·날짜·만원단위 누락 |
