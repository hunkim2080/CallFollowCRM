...
앱측 현재 상태(2026-07-02, 안드로이드 Claude): **줄눈은 앱에서 로컬로 이미 자동 시드**(DefaultPricingItems, isEstimated=true) → 서버 없이 동작. **타 업종만 이 서버 엔드포인트가 필요**하다. 엔드포인트가 생기면 앱이 클라이언트(StarterPricingRepository)를 붙인다(후속 작업).
