...
## 2026-08-15 사장님 요청 TODO
- [ ] 📸 **웹 사진 뷰어 — 사진 좌우 회전** — 어떤 사진이 옆으로 돌아가 있음(EXIF 방향 무시 추정). 뷰어에 회전 버튼(↺�её) + 회전값 저장. 서버 뷰어(_WEB_VIEWER_HTML) 코워크 도메인. (사진: 화장실 사진 돌아가 있음)
- [ ] 🔒 **[치명·홍보전 필수] 웹 QR 로그인 인증 강화** — `/api/web/authorize`가 owner_phone 그대로 신뢰(신원 검증 0). 티켓+번호만 알면 사장님으로 로그인 가능. 앱 딥링크는 확인창으로 최소방어 넣음(2026-08-15, MainActivity). 진짜 fix=번호 대신 로그인 토큰 검증. [[project_auth_login_no_verification]] 와 같은 뿌리 — 함께 처리.

## UX '조용한 잠금' 안티패턴 (2026-08-19 감사)
웹 2건 완료(글쓰기 잠금카드 ed035a72 · Gemini키 등록카드 02922fd). 나머지 = **docs/UX_GATING_TODO_2026-08-19.md** (앱 상2·중4·하5 + 웹 마이너2). 착수순서 1(채팅 고객아님 오탭)→2(광고증발 죽은코드 연결)→3(업종 입력칸 안내).
