...
val brandRules = listOf(
    ...
    // **줄머리 이모지** — "이 일정 지우기" 앞에 붙은 휴지통 그림 같은 것 = 아이콘 자리.
    //   폰마다 다르게 그려진다(삼성에서 달력 이모지는 "JUL 17" 미국 달력으로 나온다).
    //   ⚠️ 문장 **중간·끝**의 이모지는 잡지 않는다 — 사장님 말투이거나 고객에게 나가는 글일 수 있다.
    Triple(
        Regex("^[\\x{1F300}-\\x{1FAFF}\\x{2600}-\\x{2712}\\x{2719}-\\x{27BF}]+\\s+\\S"),
        // 진짜 이모지 영역만 본다. ▲▼▶↳ 같은 **화살표·도형은 글자**라 안 잡는다.
        //   2713~2718(✓✔✕✖✗✘)도 글자라 뺐다.
        "글 맨 앞의 이모지 = 아이콘 자리다. 폰마다 다르게 그려진다",
        "앱이 그리는 아이콘 Icon(...) 또는 글자만"
    )
)
