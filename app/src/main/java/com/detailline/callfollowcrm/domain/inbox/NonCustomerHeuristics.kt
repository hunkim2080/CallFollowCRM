...
object NonCustomerHeuristics {
    ...
    /**
     * [Web발신]/[웹발신] 헤더 — 웹 게이트웨이(대량발송·서비스·인증) 채널. (사장님 2026-07-12)
     *   "고객이 웹발신으로 보낼 리 없다" → 있으면 무조건 문자함. 개인이 폰에서 직접 친 문자엔 절대 안 붙는 머신 헤더.
     *   [국제발신]/[국외발신] 은 교포 고객 가능성이 있어 제외(그건 애매로 남김).
     */
    fun isWebSend(body: String): Boolean =
        Regex("[\\[(【]\\s*(?:web|웹)\\s*발신\\s*[\\])】]", RegexOption.IGNORE_CASE).containsMatchIn(body)
