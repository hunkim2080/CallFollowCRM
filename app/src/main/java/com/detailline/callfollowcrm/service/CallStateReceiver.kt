...
class CallStateReceiver : BroadcastReceiver() {
            ...
            scope.launch {
                try {
                    ...
                    if (phoneOk) {
                        ...
                        Log.i("PostCall", "call end phone=${LogRedact.phone(phone)} type=${recent?.type} dur=${recent?.duration} answered=$answered")
                        if (!answered) {
    ...
    private suspend fun maybeShowPostCallPicker(
        ...
        if (!isNew) { Log.i("PostCall", "skip: 이름 붙은 고객(새 번호 아님)"); return }
        if (!com.detailline.callfollowcrm.domain.inbox.NonCustomerHeuristics.isPersonalMobile(phoneNumber)) { Log.i("PostCall", "skip: 개인휴대폰(010) 아님 — ${LogRedact.phone(phoneNumber)}"); return }
        if (com.detailline.callfollowcrm.util.SpamPrefix.isSpam(phoneNumber, prefs.spamPrefixes)) { Log.i("PostCall", "skip: 스팸앞자리"); return }
        Log.i("PostCall", "SHOW picker: ${LogRedact.phone(phoneNumber)} items=${items.size}")
